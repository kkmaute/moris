        ...
        : QWidget( parent )
{
    ...
    for ( int iRoot = 0; iRoot < mProjectNames.size(); iRoot++ )
    {
        ...
        for ( int iChildren = 0; iChildren < tStringList.size(); iChildren++ )
        {
            ...
            /*
            checking if the sub-Module (i.e. GEN/Geometries) have
            any inner sub-modules
            */
            ...
            if ( tSubChildrenList.size() == 0 )
            {
                ...
                /*
                 *mScrollWidget is a blank widget to enable scrolling
                 *everything in mFormLayout is added to mScrollWidget
                 *and mScrollWidget is added to the mScrollArea
                 *and mScrollArea is then added to the main layout "mLayout"
                 *After trial and error this was the only way it seemed to work
                 */
...
{
    //"aRoot" checks the project index (i.e. OPT, GEN, FEM)
    //"aChildren" checks the outer parameter selection (i.e. GEN/Geometries or FEM/Properties)
    //"aSubChildren" checks the inner parameter selection (i.e. GEN/Geometries/Surface_Mesh)
...
QStringList Moris_Gui::get_inner_sub_parameter_list( moris::Parameter_List_Type aModule, int aIndex )
{
    ...
    switch ( aModule )
    {
        ...
        case moris::Parameter_List_Type::SOL:
            ...
            {
                case 0:
                    tInner = { "Aztec", "Amesos", "Belos", "PETSC", "EigenSolver", "ML", "Slepc_Solver" };
                    break;
...
void Moris_Gui::remove_props()
{
    ...
    else
    {
        ...
        for ( int iRemove = 0;
                ...
                iRemove++ )
        {
            mFormLayout[ tIndex[ 0 ] ][ tIndex[ 1 ] ][ tIndex[ 2 ] ]->removeRow( mFormLayout[ tIndex[ 0 ] ][ tIndex[ 1 ] ][ tIndex[ 2 ] ]->rowCount() - 1 );
