...
TEST_CASE( "IWG_Helmholtz", "[moris],[fem],[IWG_Helmholtz]" )
{
    ...
    SECTION( "IWG_Helmholtz_Bulk : check residual and jacobian" )
    {
        ...
        IWG_Helmholtz_Bulk tIWGHelmBulk( tFieldInterpolator,
                                         tkappa );
        ...
        Matrix< DDRMat > tResidualHelmBulk; tIWGHelmBulk.compute_residual( tResidualHelmBulk,
                                                                           tUnfilteredVHat );

        // check evaluation of the jacobian for IWG Helmholtz Bulk by FD
        //------------------------------------------------------------------------------
        // evaluate the jacobian from IWG_Helmholtz_Bulk
        Matrix< DDRMat > tJacobianHelmBulk; tIWGHelmBulk.compute_jacobian( tJacobianHelmBulk );
        ...
        for( uint k=0; k<tNSpaceTimeBases; k++ )
        {
            ...
            {
            	tCheckJacobianBulk = tCheckJacobianBulk && ( tJacobianRow( i ) - tJacobianHelmBulk( i, k ) < tEpsilon );
            }
        }
    ...
    SECTION( "IWG_Helmholtz_Interface : check residual and jacobian" )
    {
        ...
        IWG_Helmholtz_Interface tIWGHelmInterface( tFieldInterpolator,
                                                   tkappa );
