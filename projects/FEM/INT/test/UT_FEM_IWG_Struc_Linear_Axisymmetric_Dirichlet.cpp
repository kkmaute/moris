...
TEST_CASE( "IWG_Struc_Axi_Dirichlet_Symmetric_Nitsche",
        "[moris],[fem],[axi],[IWG_Struc_Axi_Dirichlet_Symmetric_Nitsche]" )
{
    // define an epsilon environment
    real tEpsilon = 1E-5;

    // define aperturbation relative size
    real tPerturbation = 2E-6;
    ...
    for( uint iSpaceDim = 2; iSpaceDim < 3; iSpaceDim++ )
    {
        ...
        for( uint iInterpOrder = 1; iInterpOrder < 4; iInterpOrder++ )
        {
            ...
            tIWG->mSet->mResDofAssemblyMap.resize( tDispDofTypes.size() );
            tIWG->mSet->mResDofAssemblyMap( 0 ) = { { 0, tNumDofDisp-1 } };

            // set size and fill the set jacobian assembly map
            Matrix< DDSMat > tJacAssembly = { { 0, tNumDofDisp - 1 } };
            tIWG->mSet->mJacDofAssemblyMap.resize( tDispDofTypes.size() );
...
TEST_CASE( "IWG_Struc_Axi_Dirichlet_Unsymmetric_Nitsche",
        "[moris],[fem],[axi],[IWG_Struc_Axi_Dirichlet_Unsymmetric_Nitsche]" )
{
    // define an epsilon environment
    real tEpsilon = 1E-5;
...
TEST_CASE( "IWG_Struc_Axi_Dirichlet_Mixed_Displacement_Symmetric_Nitsche",
        "[axi],[IWG_Struc_Axi_Dirichlet_Mixed_Displacement_Symmetric_Nitsche]" )
{
    // define an epsilon environment
    real tEpsilon = 1E-6;
...
TEST_CASE( "IWG_Struc_Axi_Dirichlet_Mixed_Displacement_Unsymmetric_Nitsche",
        "[axi],[IWG_Struc_Axi_Dirichlet_Mixed_Displacement_Unsymmetric_Nitsche]" )
{
    // define an epsilon environment
    real tEpsilon = 1E-6;
...
TEST_CASE( "IWG_Struc_Axi_Thermo_Elastic_Dirichlet_Symmetric_Nitsche",
        "[moris],[fem],[axi],[IWG_Struc_Axi_Thermo_Elastic_Dirichlet_Symmetric_Nitsche]" )
{
    // define an epsilon environment
    real tEpsilon = 1E-6;
    ...
    moris::Cell< moris::Cell< MSI::Dof_Type > > tDofTypes     = { tDispDofTypes( 0 ), tTempDofTypes( 0 ) };
    ...
    std::shared_ptr< fem::Property > tPropMasterCTE = std::make_shared< fem::Property >();
    tPropMasterCTE->set_parameters( {{{ 1.0 }}} );
    tPropMasterCTE->set_val_function( tConstValFunc_Elast );

    std::shared_ptr< fem::Property > tPropMasterTRef = std::make_shared< fem::Property >();
    tPropMasterTRef->set_parameters( {{{ 1.25 }}} );
    tPropMasterTRef->set_val_function( tConstValFunc_Elast );
    ...
    std::shared_ptr< fem::Constitutive_Model > tCMMasterStrucLinIso =
            tCMFactory.create_CM( fem::Constitutive_Type::STRUC_LIN_ISO );
    tCMMasterStrucLinIso->set_model_type( fem::Model_Type::AXISYMMETRIC );
    tCMMasterStrucLinIso->set_dof_type_list( tDofTypes, { "Displacement", "Temperature" } );
    tCMMasterStrucLinIso->set_property( tPropMasterEMod, "YoungsModulus" );
    tCMMasterStrucLinIso->set_property( tPropMasterNu, "PoissonRatio" );
    ...
    for( uint iSpaceDim = 2; iSpaceDim < 3; iSpaceDim++ )
    {
        ...
        for( uint iInterpOrder = 1; iInterpOrder < 4; iInterpOrder++ )
        {
            ...
            fill_phat_Elast( tMasterDofHatTemp, iSpaceDim, iInterpOrder );
            ...
            // create the field interpolator pressure
            tMasterFIs( 1 ) = new Field_Interpolator( 1, tFIRule, &tGI, tTempDofTypes( 0 ) );
...
TEST_CASE( "IWG_Struc_Axi_Thermo_Elastic_Dirichlet_Unsymmetric_Nitsche",
        "[moris],[fem],[axi],[IWG_Struc_Axi_Thermo_Elastic_Dirichlet_Unsymmetric_Nitsche]" )
{
    // define an epsilon environment
    real tEpsilon = 1E-6;
