...
namespace moris
{
    namespace fem
    {
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::G()
        {
            ...
            const Matrix< DDRMat > &tInvSpaceJacobian = get_leader_fi_manager()->get_IP_geometry_interpolator()->inverse_space_jacobian();
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::M()
        {
            ...
            real tdTaudt = get_leader_fi_manager()->get_IP_geometry_interpolator()->inverse_time_jacobian()( 0 );
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::dMdY( const uint aYind )
        {
            ...
            for ( uint iVar = 0; iVar < tNumStateVars; iVar++ )
            {
                ...
                Vector< Vector< Matrix< DDRMat > > > tdKdY;
                eval_dKdY( tPropMu, tPropKappa, get_leader_fi_manager(), iVar, tdKdY );
                ...
                for ( uint jDim = 0; jDim < this->num_space_dims(); jDim++ )
                {
                    ...
                    eval_dAdY( tMM, tCM, get_leader_fi_manager(), mResidualDofType, jDim + 1, iVar, tdAjdY );

                    for ( uint kDim = 0; kDim < this->num_space_dims(); kDim++ )
                    {
                        // get the variable derivs for the A matrices
                        Matrix< DDRMat > tdAkdY;
                        eval_dAdY( tMM, tCM, get_leader_fi_manager(), mResidualDofType, kDim + 1, iVar, tdAkdY );
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::dA0invdY( const uint aYind )
        {
            ...
            for ( uint iVar = 0; iVar < tNumStateVars; iVar++ )
            {
                ...
                eval_dAdY( tMM, tCM, get_leader_fi_manager(), mResidualDofType, 0, iVar, tdA0dY );
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::dTaudY( const Matrix< DDRMat > aVR )
        {
            ...
            // initialize
            Vector< Matrix< DDRMat > > tMinusdMinvdY( tNumStateVars );
            ...
            // initialize
            Vector< Matrix< DDRMat > > tdSqrtMinvdY( tNumStateVars );
            ...
            for ( uint iVar = 0; iVar < tNumStateVars; iVar++ )
            {
                ...
                mdTaudY( { 0, tNumStateVars - 1 }, { iVar, iVar } ) =
                        this->A0inv() * tdSqrtMinvdY( iVar ) * aVR + this->dA0invdY( iVar ) * this->SqrtMinv() * aVR;
            }
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::dLdDofY()
        {
            ...
            Vector< Matrix< DDRMat > > tdAjdY_Yj( this->num_space_dims() + 1 );

            // initialize cell containing Kij,i-matrices pre-multiplied with the state variable vector
            Vector< Vector< Matrix< DDRMat > > > tdKijidY_Yj( this->num_space_dims() );

            // initialize cell containing Kij,i-matrices pre-multiplied with the state variable vector
            Vector< Vector< Matrix< DDRMat > > > tdKijdY_Yij( this->num_space_dims() );
            for ( uint iDim = 0; iDim < this->num_space_dims(); iDim++ )
            {
            ...
            // get dA0/dY * Y,t
            eval_dAdY_VR( tMM, tCM, get_leader_fi_manager(), mResidualDofType, this->dYdt(), 0, tdAjdY_Yj( 0 ) );
            ...
            {
                // get dAj/dY * Y,j
                eval_dAdY_VR( tMM, tCM, get_leader_fi_manager(), mResidualDofType, this->dYdx( iDim ), iDim + 1, tdAjdY_Yj( iDim + 1 ) );

                // add contributions from A-matrices
                tdLdDofY += tdAjdY_Yj( iDim + 1 ) * this->W();

                // get dKij,i/dY * Y,j
                eval_dKijidY_VR( tPropMu, tPropKappa, get_leader_fi_manager(), this->dYdx( iDim ), iDim, tdKijidY_Yj( iDim ) );
                ...
                for ( uint jDim = 0; jDim < this->num_space_dims(); jDim++ )
                {
                    ...
                    // get dKij/dY * Y,ij
                    eval_dKdY_VR( tPropMu, tPropKappa, get_leader_fi_manager(), this->d2Ydx2( iDim, jDim ), iDim, jDim, tdKijdY_Yij( iDim )( jDim ) );
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::dLdDofW( const Matrix< DDRMat > &aVL )
        {
            ...
            Vector< Matrix< DDRMat > > tVLdAjdY( this->num_space_dims() + 1 );

            // initialize cell containing Kij,i-matrices pre-multiplied with VL
            Vector< Vector< Matrix< DDRMat > > > tVLdKijidY( this->num_space_dims() );

            // initialize cell containing Kij,i-matrices pre-multiplied with VL
            Vector< Vector< Matrix< DDRMat > > > tVLdKijdY( this->num_space_dims() );
            for ( uint iDim = 0; iDim < this->num_space_dims(); iDim++ )
            {
            ...
            // get VL * dA0/dY
            eval_VL_dAdY( tMM, tCM, get_leader_fi_manager(), mResidualDofType, aVL, 0, tVLdAjdY( 0 ) );
            ...
            {
                // get VL * dAj/dY
                eval_VL_dAdY( tMM, tCM, get_leader_fi_manager(), mResidualDofType, aVL, iDim + 1, tVLdAjdY( iDim + 1 ) );

                // add contributions from A-matrices
                tdLdDofW += this->dWdx_trans( iDim ) * tVLdAjdY( iDim + 1 ) * this->W();

                // get VL * dKij,i/dY
                eval_VL_dKijidY( tPropMu, tPropKappa, get_leader_fi_manager(), aVL, iDim, tVLdKijidY( iDim ) );
                ...
                for ( uint jDim = 0; jDim < this->num_space_dims(); jDim++ )
                {
                    ...
                    // get VL * dKij/dY
                    eval_VL_dKdY( tPropMu, tPropKappa, get_leader_fi_manager(), aVL, iDim, jDim, tVLdKijdY( iDim )( jDim ) );
        ...
        const Matrix< DDRMat > &IWG_Compressible_NS_Bulk::dGLSTestFuncdDof( const Matrix< DDRMat > &aVR )
        {
            ...
            Vector< Matrix< DDRMat > > tdAjdYVR( this->num_space_dims() + 1 );

            // initialize cell containing Kij,i-matrices pre-multiplied with VR
            Vector< Vector< Matrix< DDRMat > > > tdKijidYVR( this->num_space_dims() );

            // initialize cell containing Kij,i-matrices pre-multiplied with VL
            Vector< Vector< Matrix< DDRMat > > > tdKijdYVR( this->num_space_dims() );
            for ( uint iDim = 0; iDim < this->num_space_dims(); iDim++ )
            {
            ...
            // get dA0/dY * VR
            eval_dAdY_VR( tMM, tCM, get_leader_fi_manager(), mResidualDofType, aVR, 0, tdAjdYVR( 0 ) );
            ...
            {
                // get dAj/dY * VR
                eval_dAdY_VR( tMM, tCM, get_leader_fi_manager(), mResidualDofType, aVR, iDim + 1, tdAjdYVR( iDim + 1 ) );

                // add contributions from A-matrices
                tdGLSTFdDof += this->dWdx_trans( iDim ) * tdAjdYVR( iDim + 1 ) * this->W();

                // get dKij,i/dY * VR
                eval_dKijidY_VR( tPropMu, tPropKappa, get_leader_fi_manager(), aVR, iDim, tdKijidYVR( iDim ) );
                ...
                for ( uint jDim = 0; jDim < this->num_space_dims(); jDim++ )
                {
                    ...
                    // get dKij/dY * VR
                    eval_dKdY_VR( tPropMu, tPropKappa, get_leader_fi_manager(), aVR, iDim, jDim, tdKijdYVR( iDim )( jDim ) );
