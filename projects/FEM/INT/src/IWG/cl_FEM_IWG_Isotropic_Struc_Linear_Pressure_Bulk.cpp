...
namespace moris
{
    namespace fem
    {
        ...
        void IWG_Isotropic_Struc_Linear_Pressure_Bulk::compute_residual( real aWStar )
        {
            ...
            Field_Interpolator* tPressureFI     = get_leader_fi_manager()->get_field_interpolators_for_type( mResidualDofType( 0 )( 0 ) );
            ...
            moris::fem::CM_Struc_Linear_Isotropic* tCMElasticity =
                    static_cast< moris::fem::CM_Struc_Linear_Isotropic* >( get_leader_constitutive_model( IWG_Constitutive_Type::ELAST_LIN_ISO ).get() );
            ...
            mSet->get_residual()( 0 )(
                    ...
                    { 0, 0 } ) += aWStar * ( trans( tPressureFI->N() ) * ( tDisplacementFI->div() + tCMElasticity->eval_inv_bulk_modulus() * tPressureFI->val()( 0 ) ) );
        ...
        void IWG_Isotropic_Struc_Linear_Pressure_Bulk::compute_jacobian( real aWStar )
        {
            ...
            for ( uint iDOF = 0; iDOF < tNumDofDependencies; iDOF++ )
            {
                ...
                Field_Interpolator* tFIDer = get_leader_fi_manager()->get_field_interpolators_for_type( tDofType( 0 ) );
                ...
                if ( tDofType( 0 ) == mResidualDofType( 0 )( 0 ) )
                {
                    mSet->get_jacobian()(
                            { tLeaderResStartIndex, tLeaderResStopIndex },
                            { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( trans( tPressureFI->N() ) * tCMElasticity->eval_inv_bulk_modulus() * tPressureFI->N() );
                }
                ...
                if ( tDofType( 0 ) == MSI::Dof_Type::UX )
                {
                    mSet->get_jacobian()(
                            { tLeaderResStartIndex, tLeaderResStopIndex },
                            { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( trans( tPressureFI->N() ) * tFIDer->div_operator() );
                }
                ...
                if ( tCMElasticity->check_dof_dependency( tDofType ) )
                {
                    mSet->get_jacobian()(
                            { tLeaderResStartIndex, tLeaderResStopIndex },
                            { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( trans( tPressureFI->N() ) * tCMElasticity->eval_dInvBulkModulusdDOF( tDofType ) * tPressureFI->val()( 0 ) );
                }
            }
