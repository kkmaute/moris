    ...
    void IWG_Isotropic_Struc_Nonlinear_Bulk::compute_jacobian( real aWStar )
    {
        ...
        for ( uint iDOF = 0; iDOF < tNumDofDependencies; iDOF++ )
        {
            ...
            if ( tCMElasticity->check_dof_dependency( tDofType ) )
            {
                // compute the contribution to Jacobian
                tJac += aWStar * (                                                                                                            //
                                trans( tCMElasticity->testStrain( mStrainType ) ) * tCMElasticity->dFluxdDOF( tDofType, mStressType )         //
                                + trans( tCMElasticity->dTestStraindDOF( tDofType, mStrainType ) ) * tCMElasticity->flux( 1, mStressType )    //
                                          * tCMElasticity->dTestStraindDOF( tDofType, mStrainType ) );
            }
        }
        // check for nan, infinity
