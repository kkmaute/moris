...
namespace moris
{
    namespace fem
    {
        ...
        IWG_Compressible_NS_Bulk::IWG_Compressible_NS_Bulk()
        {
            ...
            init_stabilization_parameter( "GLS", IWG_Stabilization_Type::GLS );
        }
        ...
        void IWG_Compressible_NS_Bulk::compute_residual( real aWStar )
        {
            ...
            const std::shared_ptr< Stabilization_Parameter > &tSP = get_stabilization_parameter(IWG_Stabilization_Type::GLS);
        ...
        void IWG_Compressible_NS_Bulk::compute_jacobian( real aWStar )
        {
            ...
            Matrix< DDRMat > tdAdY;
            eval_dAdY_VR( tMM, tCM, get_leader_fi_manager(), mResidualDofType, this->dYdt(), 0, tdAdY );
            ...
            for ( uint iDim = 0; iDim < tNumSpaceDims; iDim++ )
            {
                ...
                eval_dAdY_VR( tMM, tCM, get_leader_fi_manager(), mResidualDofType, this->dYdx( iDim ), iDim + 1, tdAdY );
            ...
            for ( uint iDim = 0; iDim < tNumSpaceDims; iDim++ )
            {
                ...
                {
                    // get dKij/dY * Y,ij
                    eval_dKdY_VR( tPropMu, tPropKappa, get_leader_fi_manager(), this->dYdx( jDim ), iDim, jDim, dKdY );
            ...
            {
                // GLS stabilization term
                tJac += aWStar * tSP->val()( 0 ) * ( this->GLSTestFunc() * ( this->dTaudY( this->LY() ) * this->W() + this->Tau() * ( this->LW() + this->dLdDofY() ) ) + this->dGLSTestFuncdDof( this->Tau() * this->LY() ) );
            }
                        ...
                        for ( uint iCoeffRow = 0; iCoeffRow < tDerNumBases; iCoeffRow++ )
                        {
                            ...
                            for ( uint iPoint = tStartPoint; iPoint < tNumFDPoints; iPoint++ )
                            {
                                ...
                                tFDdMdu( { 0, tNumStateVars - 1 }, { tDofIndex, tDofIndex } ) +=
                                        tFDScheme( 1 )( iPoint ) * Mcol / ( tFDScheme( 2 )( 0 ) * tDeltaH );
                            }
                            // update dof counter
                            tDofCounter++;
                        }
