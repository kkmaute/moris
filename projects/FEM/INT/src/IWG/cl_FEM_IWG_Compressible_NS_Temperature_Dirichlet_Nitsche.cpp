...
namespace moris
{
    namespace fem
    {
        ...
        void IWG_Compressible_NS_Temperature_Dirichlet_Nitsche::compute_residual( real aWStar )
        {
            ...
            Field_Interpolator *tFITemp = get_leader_fi_manager()->get_field_interpolators_for_type( mDofTemperature );

            // get the imposed velocity property
           std::shared_ptr< Property > const &tPropTemperature = get_leader_property(IWG_Property_Type::PRESCRIBED_VALUE);
           ...
           std::shared_ptr< Stabilization_Parameter > const &tSPNitsche = get_stabilization_parameter(IWG_Stabilization_Type::NITSCHE_PENALTY_PARAMETER);
            ...
            mSet->get_residual()( 0 )(
                    ...
                    { 0, 0 } ) += aWStar * ( mBeta * trans( tCMFluid->testTraction( get_normal(), mResidualDofType( 0 ), CM_Function_Type::THERMAL ) ) * tTemperatureJump );
            ...
            if ( mResidualDofType( 0 )( 0 ) == mDofTemperature )
            {
                ...
                mSet->get_residual()( 0 )(
                        { tLeaderResStartIndex, tLeaderResStopIndex },
                        { 0, 0 } ) += aWStar * ( tFITemp->N_trans() * tCMFluid->traction( get_normal(), CM_Function_Type::THERMAL ) + tSPNitsche->val()( 0 ) * tFITemp->N_trans() * tTemperatureJump );
            }
        ...
        void IWG_Compressible_NS_Temperature_Dirichlet_Nitsche::compute_jacobian( real aWStar )
        {
           ...
           std::shared_ptr< Stabilization_Parameter > const &tSPNitsche = get_stabilization_parameter(IWG_Stabilization_Type::NITSCHE_PENALTY_PARAMETER);
            ...
            for ( uint iDOF = 0; iDOF < tLeaderNumDofDependencies; iDOF++ )
            {
                ...
                mSet->get_jacobian()(
                        { tLeaderResStartIndex, tLeaderResStopIndex },
                        { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( mBeta * tCMFluid->dTestTractiondDOF( tDofType, get_normal(), tTemperatureJump, mResidualDofType( 0 ), CM_Function_Type::THERMAL ) );
                ...
                if ( mResidualDofType( 0 )( 0 ) == mDofTemperature )
                {
                    ...
                    mSet->get_jacobian()(
                            { tLeaderResStartIndex, tLeaderResStopIndex },
                            { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( tFITemp->N_trans() * tCMFluid->dTractiondDOF( tDofType, get_normal(), CM_Function_Type::THERMAL ) );
                    ...
                    if ( tDofType( 0 ) == mDofTemperature )
                    {
                        ...
                        mSet->get_jacobian()(
                                { tLeaderResStartIndex, tLeaderResStopIndex },
                                { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( mBeta * trans( tCMFluid->testTraction( get_normal(), mResidualDofType( 0 ), CM_Function_Type::THERMAL ) ) * tFITemp->N() + tSPNitsche->val()( 0 ) * tFITemp->N_trans() * tFITemp->N() );
                    }
                    ...
                    if ( tPropTemperature->check_dof_dependency( tDofType ) )
                    {
                        ...
                        mSet->get_jacobian()(
                                { tLeaderResStartIndex, tLeaderResStopIndex },
                                { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( mBeta * trans( tCMFluid->testTraction( get_normal(), mResidualDofType( 0 ), CM_Function_Type::THERMAL ) ) * tPropTemperature->dPropdDOF( tDofType ) + tSPNitsche->val()( 0 ) * tFITemp->N_trans() * tPropTemperature->dPropdDOF( tDofType ) );
                    }
                    ...
                    if ( tSPNitsche->check_dof_dependency( tDofType ) )
                    {
                        ...
                        mSet->get_jacobian()(
                                { tLeaderResStartIndex, tLeaderResStopIndex },
                                { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( tFITemp->N_trans() * tTemperatureJump * tSPNitsche->dSPdLeaderDOF( tDofType ) );
                    }
                }
                //---------------------------------------------------------------------
                else    // residual dof type is density or temperature
                {
                    ...
                    if ( tDofType( 0 ) == mDofTemperature )
                    {
                        mSet->get_jacobian()(
                                { tLeaderResStartIndex, tLeaderResStopIndex },
                                { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( mBeta * trans( tCMFluid->testTraction( get_normal(), mResidualDofType( 0 ), CM_Function_Type::THERMAL ) ) * tFITemp->N() );
                    }
                    ...
                    if ( tPropTemperature->check_dof_dependency( tDofType ) )
                    {
                        ...
                        mSet->get_jacobian()(
                                { tLeaderResStartIndex, tLeaderResStopIndex },
                                { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( mBeta * trans( tCMFluid->testTraction( get_normal(), mResidualDofType( 0 ), CM_Function_Type::THERMAL ) ) * tPropTemperature->dPropdDOF( tDofType ) );
                    }
                }
            }
