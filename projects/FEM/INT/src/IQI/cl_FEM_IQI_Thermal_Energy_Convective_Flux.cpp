...
namespace moris::fem
{
    ...
    void IQI_Thermal_Energy_Convective_Flux::compute_QI( real aWStar )
    {
        ...
        // evaluate the QI
        mSet->get_QI()( tQIIndex ) += aWStar * ( tCMDiffusion->Energy()( 0 ) * tFIVelocity->val_trans() * mNormal );
    }
        ...
        for ( uint iDof = 0; iDof < tNumDofDependencies; iDof++ )
        {
            ...
            if ( tDofType( 0 ) == MSI::Dof_Type::VX )
            {
                mSet->get_residual()( tQIIndex )(
                        { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( tCMDiffusion->Energy()( 0 ) * tFIVelocity->N_trans() * mNormal );
            }
            ...
            if ( tCMDiffusion->check_dof_dependency( tDofType ) )
            {
                // compute dQIdu
                mSet->get_residual()( tQIIndex )(
                        { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( dot( tFIVelocity->val(), mNormal ) * trans( tCMDiffusion->dEnergydDOF( tDofType ) ) );
            }
        }
    }
