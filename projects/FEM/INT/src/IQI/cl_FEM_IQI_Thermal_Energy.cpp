...
namespace moris
{
    namespace fem
    {
        ...
        void IQI_Thermal_Energy::compute_QI( Matrix< DDRMat > &aQI )
        {
            ...
            // evaluate the QI
            aQI = tPropDensity->val() * tFITemp->val() * tFIVelocity->val_trans() * get_normal();
        }
        ...
        void IQI_Thermal_Energy::compute_QI( real aWStar )
        {
            ...
            // evaluate the QI
            mSet->get_QI()( tQIIndex ) += aWStar * ( tPropDensity->val() * tFITemp->val() * tFIVelocity->val_trans() * get_normal() );
        }
            ...
            for ( uint iDof = 0; iDof < tNumDofDependencies; iDof++ )
            {
                ...
                if ( tDofType( 0 ) == MSI::Dof_Type::VX )
                {
                    mSet->get_residual()( tQIIndex )(
                            ...
                            { 0, 0 } ) += aWStar * ( tPropDensity->val()( 0 ) * tFITemp->val()( 0 ) * tFIVelocity->N_trans() * get_normal() );
                }
                ...
                else if ( tDofType( 0 ) == MSI::Dof_Type::TEMP )
                {
                    mSet->get_residual()( tQIIndex )(
                            { tLeaderDepStartIndex, tLeaderDepStopIndex },
                            { 0, 0 } ) += aWStar * ( tPropDensity->val()( 0 ) * dot( tFIVelocity->val(), get_normal() ) * tFITemp->N_trans() );
                }
                ...
                if ( tPropDensity->check_dof_dependency( tDofType ) )
                {
                    ...
                    mSet->get_residual()( tQIIndex )(
                            { tLeaderDepStartIndex, tLeaderDepStopIndex },
                            { 0, 0 } ) += aWStar * ( tFITemp->val()( 0 ) * trans( tFIVelocity->val() ) * get_normal() * tPropDensity->dPropdDOF( tDofType ) );
                }
            }
        }
                ...
                Matrix< DDRMat >        &adQIdu )
        {
            ...
            {
                adQIdu +=
                        tPropDensity->val()( 0 ) * tFITemp->val()( 0 ) * tFIVelocity->N_trans() * get_normal();
            }
            // if dof type is temperature
            // FIXME protect dof type
            ...
            {
                adQIdu +=
                        tPropDensity->val()( 0 ) * tFIVelocity->val_trans() * get_normal() * trans( tFITemp->N() );
            }
            ...
            if ( tPropDensity->check_dof_dependency( aDofType ) )
            {
                // compute dQIdu
                adQIdu +=
                        tFITemp->val()( 0 ) * tFIVelocity->val_trans() * get_normal() * tPropDensity->dPropdDOF( aDofType );
            }
        }
