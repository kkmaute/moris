...
namespace moris
{
    namespace fem
    {
        ...
        void IQI_Thermal_Energy_Diffusive_Flux::compute_QI( Matrix< DDRMat > &aQI )
        {
            ...
            //       the physical flux
            aQI = -1.0 * dot( tCMDiffusion->flux(), get_normal() );
        }
        ...
        void IQI_Thermal_Energy_Diffusive_Flux::compute_QI( real aWStar )
        {
            ...
            //       the numerical diffusive flux needs to be multiplied by (-1) to obtain
            //       the physical flux
            mSet->get_QI()( tQIIndex ) += -1.0 * aWStar * dot( tCMDiffusion->flux(), get_normal() );
        }
...
Vector< MSI::Dof_Type > const &tDofType = get_requested_leader_dof_types()( iDof );
                ...
                if ( tCMDiffusion->check_dof_dependency( tDofType ) )
                {
                    ...
                    mSet->get_residual()( tQIIndex )(
                            { tLeaderDepStartIndex, tLeaderDepStopIndex } ) += aWStar * ( -1.0 * trans( tCMDiffusion->dFluxdDOF( tDofType ) ) * get_normal() );
                }
            }
        }
                ...
                Matrix< DDRMat >        &adQIdu )
        {
            ...
            {
                // compute dQIdu
                adQIdu += -1.0 * trans( tCMDiffusion->dFluxdDOF( aDofType ) ) * get_normal();
            }
        }
