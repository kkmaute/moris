...
namespace moris
{
    namespace fem
    {
        ...
        void IQI_Mass_Flow::compute_QI( Matrix< DDRMat > & aQI )
        {
            ...
            // evaluate the QI
            aQI = tPropDensity->val() * tFIVelocity->val_trans() * get_normal();
        }
        ...
        void IQI_Mass_Flow::compute_QI( real aWStar )
        {
            ...
            // evaluate the QI
            mSet->get_QI()( tQIIndex ) += aWStar * ( tPropDensity->val() * tFIVelocity->val_trans() * get_normal() );
        }
            ...
            for( uint iDof = 0; iDof < tNumDofDependencies; iDof++ )
            {
                ...
                if ( tDofType( 0 ) == MSI::Dof_Type::VX )
                {
                    mSet->get_residual()( tQIIndex )(
                            ...
                            { 0, 0 } ) += aWStar * tPropDensity->val()( 0 ) * tFIVelocity->N_trans() * get_normal();
                }
                ...
                if ( tPropDensity->check_dof_dependency( tDofType ) )
                {
                    ...
                    mSet->get_residual()( tQIIndex )(
                            { tLeaderDepStartIndex, tLeaderDepStopIndex },
                            { 0, 0 } ) += aWStar * tFIVelocity->val_trans() * get_normal() * tPropDensity->dPropdDOF( tDofType );
                }
            }
        }
                ...
                Matrix< DDRMat >             & adQIdu )
        {
            ...
            {
                adQIdu += tPropDensity->val()( 0 ) * tFIVelocity->N_trans() * get_normal();
            }
            ...
            {
                // compute dQIdu
                adQIdu += tFIVelocity->val_trans() * get_normal() * tPropDensity->dPropdDOF( aDofType );
            }
        }
