...
namespace moris
{
    namespace fem
    {
        ...
        void IQI_Mass_Flow::compute_QI( real aWStar )
        {
            ...
            // evaluate the QI
            mSet->get_QI()( tQIIndex ) += aWStar * ( tPropDensity->val() * trans( tFIVelocity->val() ) * mNormal );
        }
            ...
            for( uint iDof = 0; iDof < tNumDofDependencies; iDof++ )
            {
                ...
                if ( tDofType( 0 ) == MSI::Dof_Type::VX )
                {
                    mSet->get_residual()( tQIIndex )(
                            ...
                            { 0, 0 } ) += aWStar * tPropDensity->val()( 0 ) * trans( tFIVelocity->N() ) * mNormal;
                }
                ...
                if ( tPropDensity->check_dof_dependency( tDofType ) )
                {
                    ...
                    mSet->get_residual()( tQIIndex )(
                            { tMasterDepStartIndex, tMasterDepStopIndex },
                            { 0, 0 } ) += aWStar * trans( tFIVelocity->val() ) * mNormal * tPropDensity->dPropdDOF( tDofType );
                }
            }
        }
