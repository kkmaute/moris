                ...
                const moris::Cell< MSI::Dof_Type >& aDofTypes )
        {
            ...
            if ( tTau > mEpsilon )
            {
                ...
                if ( tNormA > mEpsilon )
                {
                    ...
                    if ( aDofTypes( 0 ) == mMasterDofVelocity )
                    {
                        ...
                        mdPPdMasterDof( tDofIndex ) +=
                                tTauA * 2.0 * trans( tModVelocity ) * tVelocityFI->N() / ( tHugn * tNormA );
                    }
                    ...
                    else if ( aDofTypes( 0 ) == mMasterDofViscosity )
                    {
                        // add contribution to mdPPdMasterDof
                        mdPPdMasterDof( tDofIndex ) -=
                                tTauA * 2.0 * mCb2 * trans( tModVelocity ) * tViscosityFI->dnNdxn( 1 ) / mSigma / ( tHugn * tNormA );
                    }
                }
                ...
                if ( tCMSATurbulence->check_dof_dependency( aDofTypes ) )
                {
                    // compute tdtauKdu
                    mdPPdMasterDof( tDofIndex ) +=
                            tTauK * 4.0 * tCMSATurbulence->ddiffusioncoeffdu( aDofTypes ) / std::pow( tHugn, 2.0 );

                    // compute tdtauSdu
                    mdPPdMasterDof( tDofIndex ) +=
                            tTauS * ( tCMSATurbulence->dwalldestructioncoeffdu( aDofTypes ) - tCMSATurbulence->dproductioncoeffdu( aDofTypes ) );
                }
