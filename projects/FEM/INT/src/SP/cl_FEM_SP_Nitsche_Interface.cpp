...
namespace moris::fem
{
    ...
    Vector< std::tuple<
            ...
            mtk::Primary_Void,
            mtk::Leader_Follower > >
    SP_Nitsche_Interface::get_cluster_measure_tuple_list()
    {
            ...
            const Vector< MSI::Dof_Type > &aDofTypes )
    {
        ...
        if ( tLeaderPropMaterial->check_dof_dependency( aDofTypes ) )
        {
            ...
            mdPPdLeaderDof( tDofIndex ).get_row( 0 ) =
                    this->val()( 0 ) * tLeaderVolume * tLeaderPropMaterial->dPropdDOF( aDofTypes ) / ( std::pow( tLeaderMaterial, 2 ) * tDeno );

            // compute leader weight derivative
            mdPPdLeaderDof( tDofIndex ).get_row( 1 ) =
                    -this->val()( 1 ) * tFollowerVolume * tLeaderPropMaterial->dPropdDOF( aDofTypes ) / ( tLeaderMaterial * tFollowerMaterial * tDeno );

            // compute follower weight derivative
            mdPPdLeaderDof( tDofIndex ).get_row( 2 ) =
                    this->val()( 2 ) * tLeaderVolume * tLeaderPropMaterial->dPropdDOF( aDofTypes ) / ( std::pow( tLeaderMaterial, 2 ) * tDeno );
        }
        else
        {
            ...
            const Vector< MSI::Dof_Type > &aDofTypes )
    {
        ...
        // reset the matrix
        mdPPdFollowerDof( tDofIndex ).set_size( 3, tFIDer->get_number_of_space_time_coefficients() );
        ...
        if ( tFollowerPropMaterial->check_dof_dependency( aDofTypes ) )
        {
            ...
            mdPPdFollowerDof( tDofIndex ).get_row( 0 ) =
                    this->val()( 0 ) * tFollowerVolume * tFollowerPropMaterial->dPropdDOF( aDofTypes ) / ( std::pow( tFollowerMaterial, 2 ) * tDeno );

            // compute leader weight SP derivative
            mdPPdFollowerDof( tDofIndex ).get_row( 1 ) =
                    this->val()( 1 ) * tFollowerVolume * tFollowerPropMaterial->dPropdDOF( aDofTypes ) / ( std::pow( tFollowerMaterial, 2 ) * tDeno );

            // compute follower weight SP derivative
            mdPPdFollowerDof( tDofIndex ).get_row( 2 ) =
                    -this->val()( 2 ) * tLeaderVolume * tFollowerPropMaterial->dPropdDOF( aDofTypes ) / ( tLeaderMaterial * tFollowerMaterial * tDeno );
        }
        else
        {
