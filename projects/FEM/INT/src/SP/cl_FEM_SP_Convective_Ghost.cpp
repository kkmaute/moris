...
namespace moris::fem
{
    ...
    Vector< std::tuple<
            ...
            mtk::Primary_Void,
            mtk::Leader_Follower > >
    SP_Convective_Ghost::get_cluster_measure_tuple_list()
    {
            ...
            const Vector< MSI::Dof_Type >& aDofTypes )
    {
        ...
        if ( aDofTypes( 0 ) == mLeaderDofVelocity )
        {
            ...
            mdPPdLeaderDof( tDofIndex ) =
                    mParameters( 0 ) * std::pow( tElementSize, 2.0 ) * tDensityProp->val()( 0 ) * tNormalDispl * trans( mNormal ) * tVelocityFI->N() / ( tAbsReal + mEpsilon );
        }
        else
        {
        ...
        if ( tDensityProp->check_dof_dependency( aDofTypes ) )
        {
            ...
            mdPPdLeaderDof( tDofIndex ) +=
                    mParameters( 0 ) * std::pow( tElementSize, 2.0 ) * tAbsReal * tDensityProp->dPropdDOF( aDofTypes );
        }
    }
