...
#include "cl_FEM_IWG_Heat_Method_Dirichlet_Nitsche.hpp"
...
namespace moris
{
    namespace fem
    {
        ...
        IWG_Heat_Method_Dirichlet_Nitsche::IWG_Heat_Method_Dirichlet_Nitsche( sint aBeta )
        {
        ...
        void IWG_Heat_Method_Dirichlet_Nitsche::compute_residual( real aWStar )
        {
#ifdef DEBUG
            ...
            mSet->get_residual()( 0 )(
                    { tMasterResStartIndex, tMasterResStopIndex } ) += aWStar * (
                            - tFI->N_trans() * tCMDiffusion->traction( mNormal )
                            + mBeta * tCMDiffusion->testTraction( mNormal, mResidualDofType ) * tJump
                            + tSPNitsche->val()( 0 ) * tFI->N_trans() * tJump );

            // check for nan, infinity
            MORIS_ASSERT( isfinite( mSet->get_residual()( 0 ) ),
                    "IWG_Heat_Method_Dirichlet_Nitsche::compute_residual - Residual contains NAN or INF, exiting!");
        }

        //------------------------------------------------------------------------------

        void IWG_Heat_Method_Dirichlet_Nitsche::compute_jacobian( real aWStar )
        {
#ifdef DEBUG
            // check field interpolators, properties, constitutive models
            this->check_field_interpolators();
            ...
            MORIS_ASSERT( isfinite( mSet->get_jacobian() ) ,
                    "IWG_Heat_Method_Dirichlet_Nitsche::compute_jacobian - Jacobian contains NAN or INF, exiting!");
        }

        //------------------------------------------------------------------------------

        void IWG_Heat_Method_Dirichlet_Nitsche::compute_jacobian_and_residual( real aWStar )
        {
            MORIS_ERROR( false, "IWG_Heat_Method_Dirichlet_Nitsche::compute_jacobian_and_residual - Not implemented." );
        }

        //------------------------------------------------------------------------------

        void IWG_Heat_Method_Dirichlet_Nitsche::compute_dRdp( real aWStar )
        {
            MORIS_ERROR( false, "IWG_Heat_Method_Dirichlet_Nitsche::compute_dRdp - Not implemented.");
        }
