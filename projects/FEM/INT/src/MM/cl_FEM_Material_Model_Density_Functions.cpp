...
namespace moris::fem
{
    ...
    const Matrix< DDRMat > &Material_Model::DensityDOF_dep( const Vector< MSI::Dof_Type > &aDofType )
    {
    ...
    // trivial operation: get values from FI
    const Matrix< DDRMat > &Material_Model::DensityDOF_triv( const Vector< MSI::Dof_Type > &aDofType )
    {
    ...
    // if thermodynamic state variable is dependent compute and retrieve values from storage
    const Matrix< DDRMat > &Material_Model::DensityDotDOF_dep( const Vector< MSI::Dof_Type > &aDofType )
    {
        // if aDofType is not an active dof type for the MM
        MORIS_ASSERT(
                this->check_dof_dependency( aDofType ),
    ...
    // trivial operation: get values from FI
    const Matrix< DDRMat > &Material_Model::DensityDotDOF_triv( const Vector< MSI::Dof_Type > &aDofType )
    {
        // check DOF deriv is wrt to own DOF-type is with
        if ( aDofType( 0 ) != mDofDensity )
        {
    ...
    // if thermodynamic state variable is dependent compute and retrieve values from storage
    const Matrix< DDRMat > &Material_Model::dnDensitydxnDOF_dep( const Vector< MSI::Dof_Type > &aDofType, uint aOrder )
    {
        // if aDofType is not an active dof type for the MM
        MORIS_ASSERT(
                this->check_dof_dependency( aDofType ),
    ...
    // trivial operation: get values from FI
    const Matrix< DDRMat > &Material_Model::dnDensitydxnDOF_triv( const Vector< MSI::Dof_Type > &aDofType, uint aOrder )
    {
        // check DOF deriv is wrt to own DOF-type is with
        if ( aDofType( 0 ) != mDofDensity )
        {
            ...
            if ( aOrder == 1 )
            {
                ...
                {
                    // initialize output matrix with zeros
                    mdDensitydxDof( tDofIndex ).set_size( mSpaceDim, mFIManager->get_field_interpolators_for_type( aDofType( 0 ) )->get_number_of_space_time_coefficients(), 0.0 );
            ...
            else if ( aOrder == 2 )
            {
                ...
                {
                    // initialize output matrix with zeros
                    md2Densitydx2Dof( tDofIndex ).set_size( 3 * mSpaceDim - 3, mFIManager->get_field_interpolators_for_type( aDofType( 0 ) )->get_number_of_space_time_coefficients(), 0.0 );
