    ...
    const Matrix< DDRMat > &Material_Model::dnPressuredxnDOF_triv( const Vector< MSI::Dof_Type > &aDofType, uint aOrder )
    {
        ...
        if ( aDofType( 0 ) != mDofPressure )
        {
            ...
            if ( aOrder == 1 )
            {
                ...
                {
                    // initialize output matrix with zeros
                    mdPressuredxDof( tDofIndex ).set_size( mSpaceDim, mFIManager->get_field_interpolators_for_type( aDofType( 0 ) )->get_number_of_space_time_coefficients(), 0.0 );
            ...
            else if ( aOrder == 2 )
            {
                ...
                {
                    // initialize output matrix with zeros
                    md2Pressuredx2Dof( tDofIndex ).set_size( 3 * mSpaceDim - 3, mFIManager->get_field_interpolators_for_type( aDofType( 0 ) )->get_number_of_space_time_coefficients(), 0.0 );
