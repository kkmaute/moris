...
namespace moris::fem
{
    ...
    const Matrix< DDRMat > &Material_Model::CvDOF_triv( const Vector< MSI::Dof_Type > &aDofTypes )
    {
        ...
        else    // no dependency of property on DoF type
        {
            ...
            // FIXME: this is slow
            mCvDof( tDofIndex ).set_size( 1, mFIManager->get_field_interpolators_for_type( aDofTypes( 0 ) )->get_number_of_space_time_coefficients(), 0.0 );
    ...
    const Matrix< DDRMat > &Material_Model::CpDOF_triv( const Vector< MSI::Dof_Type > &aDofTypes )
    {
        ...
        else    // no dependency of property on DoF type
        {
            ...
            // initialize zero matrix
            // FIXME: this is slow
            mCpDof( tDofIndex ).set_size( 1, mFIManager->get_field_interpolators_for_type( aDofTypes( 0 ) )->get_number_of_space_time_coefficients(), 0.0 );
    ...
    const Matrix< DDRMat > &Material_Model::GammaDOF_triv( const Vector< MSI::Dof_Type > &aDofTypes )
    {
        ...
        else    // no dependency of property on DoF type
        {
            ...
            // initialize zero matrix
            // FIXME: this is slow
            mGammaDof( tDofIndex ).set_size( 1, mFIManager->get_field_interpolators_for_type( aDofTypes( 0 ) )->get_number_of_space_time_coefficients(), 0.0 );
