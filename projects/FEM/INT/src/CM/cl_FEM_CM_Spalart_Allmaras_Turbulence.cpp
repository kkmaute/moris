...
namespace moris
{
    namespace fem
    {
        ...
        void CM_Spalart_Allmaras_Turbulence::set_dof_type_list(
        ...
        void CM_Spalart_Allmaras_Turbulence::build_global_dof_type_list()
        {
        ...
        void CM_Spalart_Allmaras_Turbulence::reset_eval_flags()
        {
            // call parent implementation
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_divflux()
        {
            ...
            mDivFlux = trans ( this->ddiffusioncoeffdx( 1 ) ) * tFIViscosity->gradx( 1 ) +
                    this->diffusion_coefficient() * sum( tFIViscosity->gradx( 2 )( { 0, mSpaceDim - 1 }, { 0, 0 } ) );
        }

        void CM_Spalart_Allmaras_Turbulence::eval_ddivfluxdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            ...
            if( aDofTypes( 0 ) == mDofViscosity )
            {
                ...
                mddivfluxdu( tDofIndex ) +=
                        trans( this->ddiffusioncoeffdx( 1 ) ) * tFIViscosity->dnNdxn( 1 ) +
                        this->diffusion_coefficient() * td2Ndx2;
            }
        }

        //--------------------------------------------------------------------------------------------------------------

        void CM_Spalart_Allmaras_Turbulence::eval_traction(
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_testTraction(
                const Matrix< DDRMat >             & aNormal,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dTractiondDOF(
                const moris::Cell< MSI::Dof_Type > & aDofTypes,
                const Matrix< DDRMat >             & aNormal )
        {
            ...
            mdTractiondDof( tDofIndex ).set_size(
                    1,
                    tFIDer->get_number_of_space_time_coefficients() );
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dTestTractiondDOF(
                const moris::Cell< MSI::Dof_Type > & aDofTypes,
                const Matrix< DDRMat >             & aNormal,
                const moris::Cell< MSI::Dof_Type > & aTestDofTypes )
        {
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_production_coefficient()
        {
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::production_coefficient(
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dproductioncoeffdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            ...
            mdProductionCoeffdu( tDofIndex ).set_size(
                    1,
                    tFIDer->get_number_of_space_time_coefficients() );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dproductioncoeffdu(
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::production_term(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_production_term()
        {
            // get the viscosity FI
            Field_Interpolator * tFIModViscosity =
                    mFIManager->get_field_interpolators_for_type( mDofViscosity );
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dproductiontermdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
            ...
            mdProductionTermdu( tDofIndex ).set_size(
                    1,
                    tFIDer->get_number_of_space_time_coefficients() );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dproductiontermdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_wall_destruction_coefficient()
        {
            // get the viscosity FI
            Field_Interpolator * tFIModViscosity =
                    mFIManager->get_field_interpolators_for_type( mDofViscosity );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::wall_destruction_coefficient(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dwalldestructioncoeffdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
            ...
            mdWallDestructionCoeffdu( tDofIndex ).set_size(
                    1,
                    tFIDer->get_number_of_space_time_coefficients() );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dwalldestructioncoeffdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_wall_destruction_term()
        {
            // get the viscosity FI
            Field_Interpolator * tFIModViscosity =
                    mFIManager->get_field_interpolators_for_type( mDofViscosity );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::wall_destruction_term(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dwalldestructiontermdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
            ...
            mdWallDestructionTermdu( tDofIndex ).set_size(
                    1,
                    tFIDer->get_number_of_space_time_coefficients() );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dwalldestructiontermdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_diffusion_coefficient()
        {
            // get the viscosity FI
            Field_Interpolator * tFIModViscosity =
                    mFIManager->get_field_interpolators_for_type( mDofViscosity );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::diffusion_coefficient(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_ddiffusioncoeffdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
            ...
            // initialize the matrix for dEffConddu
            mdDiffusionCoeffdu( tDofIndex ).set_size(
                    1,
                    tFIDer->get_number_of_space_time_coefficients() );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::ddiffusioncoeffdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_ddiffusioncoeffdx( uint aOrder )
        {
            ...
            else
            {
                ...
                mdDiffusionCoeffdx( aOrder - 1 ) = (
                        tFIModViscosity->gradx( 1 ) * this->fn() +
                        tFIModViscosity->val()( 0 ) * this->dfndx( 1 ) ) / mSigma;
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::ddiffusioncoeffdx(
                uint                  aOrder,
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_ddiffusioncoeffdxdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes,
                uint                                 aOrder)
        {
            // get the dof type as a uint
            ...
            // set matrix size
            mdDiffusionCoeffdxdu( aOrder - 1 )( tDofIndex ).set_size(
                    mSpaceDim,
                    tFIDer->get_number_of_space_time_coefficients() );
            ...
            else
            {
                // compute diffusion term
                mdDiffusionCoeffdxdu( aOrder - 1 )( tDofIndex ) =
                        ( tFIModViscosity->gradx( 1 ) * this->dfndu( aDofTypes ) +
                                tFIModViscosity->val()( 0 ) * this->dfndxdu( aDofTypes, 1 ) ) / mSigma;
                ...
                {
                    mdDiffusionCoeffdxdu( aOrder - 1 )( tDofIndex ) +=
                            ( tFIModViscosity->dnNdxn( 1 ) * this->fn() +
                                    this->dfndx( 1 ) * tFIModViscosity->N() ) / mSigma;
                }
            }
        }

        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::ddiffusioncoeffdxdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                uint                                 aOrder,
                enum CM_Function_Type                aCMFunctionType )
        {
        ...
        const real CM_Spalart_Allmaras_Turbulence::chi(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dchidu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dchidx(
                uint                  aOrder,
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dchidxdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                uint                                 aOrder,
                enum CM_Function_Type                aCMFunctionType )
        {
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_ft2()
        {
            // compute ft2
            mFt2 = mCt3 * std::exp( - mCt4 * std::pow( this->chi(), 2.0 ) );
        }

        const real CM_Spalart_Allmaras_Turbulence::ft2(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dft2du(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dft2du(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_w()
        {
            // get the velocity FI
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::w(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dwdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dwdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_s()
        {
            // compute W : W
            Matrix< DDRMat > tWW = trans( this->w() ) * this->w();

            // compute S
            mS = std::sqrt( 2.0 * tWW( 0 ) );

            // threshold s for consistency with derivative
            mS = std::max( mS, mEpsilon );
        }

        const real CM_Spalart_Allmaras_Turbulence::s(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dsdu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dsdu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        const real CM_Spalart_Allmaras_Turbulence::fv1(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dfv1du(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dfv2du(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_sbar()
        {
            // get the viscosity FI
            Field_Interpolator * tFIViscosity =
                    mFIManager->get_field_interpolators_for_type( mDofViscosity );
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dsbardu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dsbardu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dsmoddu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_stilde()
        {
            // init mSTilde
            mSTilde = this->s();
        ...
        const real CM_Spalart_Allmaras_Turbulence::stilde(
                enum CM_Function_Type aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
            MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
        ...
        void CM_Spalart_Allmaras_Turbulence::eval_dstildedu(
                const moris::Cell< MSI::Dof_Type > & aDofTypes )
        {
            // get the dof type as a uint
            uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
        ...
        const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dstildedu(
                const moris::Cell< MSI::Dof_Type > & aDofType,
                enum CM_Function_Type                aCMFunctionType )
        {
            // check CM function type, base class only supports "DEFAULT"
        ...
        //--------------------------------------------------------------------------------------------------------------
         ...
         void CM_Spalart_Allmaras_Turbulence::eval_r()
         {
             // get the residual dof FI (here viscosity)
             Field_Interpolator * tFIViscosity =
                     mFIManager->get_field_interpolators_for_type( mDofViscosity );
         ...
         const real CM_Spalart_Allmaras_Turbulence::r(
                 enum CM_Function_Type aCMFunctionType )
         {
             // check CM function type, base class only supports "DEFAULT"
             MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
         ...
         void CM_Spalart_Allmaras_Turbulence::eval_drdu(
                 const moris::Cell< MSI::Dof_Type > & aDofTypes )
         {
             // get the dof type as a uint
             uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
         ...
         const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::drdu(
                 const moris::Cell< MSI::Dof_Type > & aDofType,
                 enum CM_Function_Type                aCMFunctionType )
         {
             // check CM function type, base class only supports "DEFAULT"
         ...
         //--------------------------------------------------------------------------------------------------------------
          ...
          void CM_Spalart_Allmaras_Turbulence::eval_g()
          {
              // compute g
              mG = this->r() + mCw2 * ( std::pow( this->r(), 6.0 ) - this->r() );
          }

          const real CM_Spalart_Allmaras_Turbulence::g(
                  enum CM_Function_Type aCMFunctionType )
          {
              // check CM function type, base class only supports "DEFAULT"
              MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
          ...
          void CM_Spalart_Allmaras_Turbulence::eval_dgdu(
                  const moris::Cell< MSI::Dof_Type > & aDofTypes )
          {
              // get the dof type as a uint
              uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
          ...
          const Matrix< DDRMat > & CM_Spalart_Allmaras_Turbulence::dgdu(
                  const moris::Cell< MSI::Dof_Type > & aDofType,
                  enum CM_Function_Type                aCMFunctionType )
          {
              // check CM function type, base class only supports "DEFAULT"
          ...
          const real CM_Spalart_Allmaras_Turbulence::fw(
                  enum CM_Function_Type aCMFunctionType )
          {
              // check CM function type, base class only supports "DEFAULT"
              MORIS_ASSERT( aCMFunctionType == CM_Function_Type::DEFAULT,
          ...
          void CM_Spalart_Allmaras_Turbulence::eval_dfwdu(
                  const moris::Cell< MSI::Dof_Type > & aDofTypes )
          {
              // get the dof type as a uint
              uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
          ...
          void CM_Spalart_Allmaras_Turbulence::eval_dfndu(
                  const moris::Cell< MSI::Dof_Type > & aDofTypes )
          {
              // get the dof type as a uint
              uint tDofType = static_cast< uint >( aDofTypes( 0 ) );
          ...
          void CM_Spalart_Allmaras_Turbulence::eval_dfndx( uint aOrder )
          {
              // FIXME work only for 1st order
              MORIS_ERROR( aOrder == 1,
          ...
          void CM_Spalart_Allmaras_Turbulence::eval_dfndxdu(
                  const moris::Cell< MSI::Dof_Type > & aDofTypes,
                  uint                                 aOrder)
          {
              // get the dof type as a uint
              ...
              // set matrix size
              mdFndxdu( aOrder - 1 )( tDofIndex ).set_size(
                      mSpaceDim,
                      tFIDer->get_number_of_space_time_coefficients() );
