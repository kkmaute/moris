/*
 * cl_FEM_Isotropic_Struc_Linear_Ghost.cpp
 *
...
//------------------------------------------------------------------------------
        void IWG_Isotropic_Struc_Linear_Ghost::compute_residual( real aWStar )
        {
            ...
            switch ( tFIMaster->get_space_interpolation_order() )
            {
                ...
                default:
                {
                    MORIS_ERROR( false, "IWG_Isotropic_Struc_Linear_Ghost::compute_residual - order not supported");
                    break;
                }
            }
            ...
            switch( aOrderGhost )
            {
                case ( 1 ):
                {
                    switch ( tSpaceDim )
                    {
                        ...
                        default:
                        {
                            MORIS_ERROR( false, "IWG_Isotropic_Struc_Linear_Ghost::get_normal_matrix - Spatial dimensions can only be 2, 3." );
                            break;
                        }
                    }
                    break;
