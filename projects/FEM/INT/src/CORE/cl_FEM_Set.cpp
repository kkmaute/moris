...
namespace moris
{
    namespace fem
    {
                ...
                , mPerturbationStrategy( aSetInfo.get_perturbation_strategy() )
        {
            ...
            moris::Vector< mtk::Cluster const * > tMeshClusterList = mMeshSet->get_clusters_on_set();
            ...
            for ( luint iCluster = 0; iCluster < tNumMeshClusters; iCluster++ )
            {
                ...
                moris::Vector< const mtk::Cell* > tInterpolationCell;
        ...
        Set::create_unique_dof_and_dv_type_lists()
        {
            ...
            for ( const std::shared_ptr< IWG >& tIWG : mIWGs )
            {
                ...
                moris::Vector< moris::Vector< MSI::Dof_Type > >   tActiveDofType;
                moris::Vector< moris::Vector< PDV_Type > >        tActiveDvType;
                moris::Vector< moris::Vector< mtk::Field_Type > > tActiveFieldType;
        ...
        Set::create_dof_and_dv_type_lists()
        {
            ...
            for ( const std::shared_ptr< IWG >& tIWG : mIWGs )
            {
                ...
                const moris::Vector< moris::Vector< MSI::Dof_Type > >& tDofTypeLeader =
                        tIWG->get_global_dof_type_list();

                const moris::Vector< moris::Vector< PDV_Type > >& tDvTypeLeader =
                        tIWG->get_global_dv_type_list();

                const moris::Vector< moris::Vector< mtk::Field_Type > >& tFieldTypeLeader =
                ...
                const moris::Vector< moris::Vector< MSI::Dof_Type > >& tDofTypeFollower =
                        tIWG->get_global_dof_type_list( mtk::Leader_Follower::FOLLOWER );

                const moris::Vector< moris::Vector< PDV_Type > >& tDvTypeFollower =
                        tIWG->get_global_dv_type_list( mtk::Leader_Follower::FOLLOWER );

                const moris::Vector< moris::Vector< mtk::Field_Type > >& tFieldTypeFollower =
        ...
        Set::create_unique_dof_dv_and_field_type_maps()
        {
            ...
            const moris::Vector< enum MSI::Dof_Type >& tDofType = get_unique_dof_type_list();
            ...
            const moris::Vector< enum PDV_Type >& tDvType = get_unique_dv_type_list();
            ...
            const moris::Vector< mtk::Field_Type >& tFieldType = get_unique_field_type_list();
        ...
        Set::set_IWG_cluster_for_stabilization_parameters( fem::Cluster* aCluster )
        {
            ...
            for ( auto tIWG : mIWGs )
            {
                ...
                moris::Vector< std::shared_ptr< Stabilization_Parameter > >& tSPs =
        ...
        Set::create_staggered_jacobian_dof_assembly_map()
        {
            ...
            const moris::Vector< enum MSI::Dof_Type >& tSecondaryDofTypes =
        ...
        Set::create_mat_pdv_assembly_map()
        {
            ...
            moris::Vector< enum PDV_Type > tRequestedDvTypes = mUniqueDvTypeList;
            ...
            if ( mEquationModel->get_is_forward_analysis() )
            {
                ...
                for ( MSI::Dof_Type tDofType : tRequestedDofTypes )
                {
                    ...
                    for ( uint iIWG = 0; iIWG < mIWGs.size(); iIWG++ )
                    {
                        ...
                        const moris::Vector< moris::Vector< MSI::Dof_Type > >& tResDofType =
        ...
        Set::create_requested_IQI_list()
        {
            ...
            const moris::Vector< std::string >& tRequestedIQINames =
        ...
        const moris::Vector< std::shared_ptr< fem::IQI > >&
        ...
        Set::gather_requested_IQIs(
                moris::Vector< std::string > const &     aNames,
                moris::Vector< std::shared_ptr< IQI > >& aListOfRequestedIQIs,
                moris::Vector< moris_index >&            aListOfIQIGlobalIndices )
        {
            // get number of potential requested IQIs
        ...
        Set::get_ig_unique_dv_types_for_set(
                moris::Vector< enum PDV_Type >& aGeoPdvType )
        {
        ...
        void
        Set::populate_fields(
                moris::Vector< std::shared_ptr< fem::Field > >& aFieldToPopulate,
                moris::Vector< std::string > const &            aFieldIQINames )
        {
