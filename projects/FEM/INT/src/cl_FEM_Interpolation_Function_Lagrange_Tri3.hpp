...
//------------------------------------------------------------------------------
        ...
        Interpolation_Function< Interpolation_Type::LAGRANGE, 2, 3 >::get_interpolation_order() const
        {
...
//------------------------------------------------------------------------------
        ...
        template<>
        Matrix< DDRMat >
        Interpolation_Function< Interpolation_Type::LAGRANGE, 2, 3 >::get_param_coords() const
        {
            Matrix< DDRMat > tXiHat( 2, 3 );
...
//------------------------------------------------------------------------------
        ...
        template<>
        Matrix< DDRMat >
        Interpolation_Function< Interpolation_Type::LAGRANGE, 2, 3 >::eval_N( const Matrix< DDRMat > & aXi ) const
        {
...
//------------------------------------------------------------------------------
        ...
        template<>
        Matrix< DDRMat >
        Interpolation_Function< Interpolation_Type::LAGRANGE, 2, 3 >::eval_dNdXi( const Matrix< DDRMat > & aXi ) const
        {
            // make sure that input is correct
            MORIS_ASSERT( aXi.length() >= 2, "eval_shape: aXi not allocated or hat wrong size." );

            // populate output matrix
            Matrix< DDRMat > tdNdXi( 2, 3 );
            tdNdXi( 0, 0 ) = -1.0;
            tdNdXi( 1, 0 ) = -1.0;
...
//------------------------------------------------------------------------------
        ...
        template<>
        Matrix< DDRMat >
        Interpolation_Function< Interpolation_Type::LAGRANGE, 2, 3 >::eval_d2NdXi2( const Matrix< DDRMat > & aXi ) const
        {
            // make sure that input is correct
            MORIS_ASSERT( aXi.length() >= 2, "eval_shape: aXi not allocated or hat wrong size." );

            // populate output matrix
            Matrix< DDRMat > td2NdXi2( 3, 3, 0.0);
            return td2NdXi2;
        }
//------------------------------------------------------------------------------
    } /* namespace fem */
