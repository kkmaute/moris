...
namespace moris
{
    namespace fem
    {
        ...
        void IWG_Compressible_NS_Traction_Neumann::set_property(
                ...
                mtk::Master_Slave           aIsMaster )
        {
            ...
            std::string tErrMsg =
                    std::string( "IWG_Compressible_NS_Traction_Neumann::set_property - Unknown aPropertyString: " ) +
                    aPropertyString;
            MORIS_ERROR( mPropertyMap.find( aPropertyString ) != mPropertyMap.end(), tErrMsg.c_str() );

            // check no slave allowed
            MORIS_ERROR( aIsMaster == mtk::Master_Slave::MASTER,
                    "IWG_Compressible_NS_Traction_Neumann::set_property - No slave allowed." );
        ...
        void IWG_Compressible_NS_Traction_Neumann::compute_residual( real aWStar )
        {
            ...
            mSet->get_residual()( 0 )(
                    ...
                    { 0, 0 } ) += aWStar * ( -1.0 * trans( tFIVelocity->N() ) * mMasterProp( tPropTractionIndex )->val() );
        ...
        void IWG_Compressible_NS_Traction_Neumann::compute_jacobian( real aWStar )
        {
            ...
            for( uint iDOF = 0; iDOF < mRequestedMasterGlobalDofTypes.size(); iDOF++ )
            {
                ...
                if ( mMasterProp( tPropTractionIndex )->check_dof_dependency( tDepDofType ) )
                {
                    // add contribution to jacobian
                    mSet->get_jacobian()(
                            { tResStartIndex, tResStopIndex },
                            { tDepStartIndex, tDepStopIndex } ) += aWStar * ( -1.0 *
                                    trans( tFIVelocity->N() ) * mMasterProp( tPropTractionIndex )->dPropdDOF( tDepDofType ) );
                }
            }
