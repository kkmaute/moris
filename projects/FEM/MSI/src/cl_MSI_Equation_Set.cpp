...
namespace moris::MSI
{
    ...
    const Vector< sint > &Equation_Set::get_dv_type_map(
            mtk::Leader_Follower aIsLeader )
    {
        switch ( aIsLeader )
        {
            ...
            mtk::Leader_Follower aIsLeader )
    {
        switch ( aIsLeader )
        {
            case mtk::Leader_Follower::LEADER:
            {
                ...
                MORIS_ASSERT( static_cast< uint >( aDvType ) < mLeaderDvTypeMap.size(),
            ...
            {
                // check if dv type exists in map
                MORIS_ASSERT( static_cast< uint >( aDvType ) < mFollowerDvTypeMap.size(),
