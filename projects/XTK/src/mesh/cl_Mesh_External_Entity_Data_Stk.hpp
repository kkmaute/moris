/*
 * cl_Mesh_External_Entity_Data_Stk.hpp
 *
 *  Created on: Jun 20, 2017
 *      Author: ktdoble
 */

#ifndef SRC_MESH_CL_MESH_EXTERNAL_ENTITY_DATA_STK_HPP_
#define SRC_MESH_CL_MESH_EXTERNAL_ENTITY_DATA_STK_HPP_
...
class Mesh_External_Entity_Data_Stk
{
public:
    Integer INTEGER_MAX = std::numeric_limits<Integer>::max();
    Mesh_External_Entity_Data_Stk() :
            ...
            mExternalEntities((Integer)   EntityRank::END_ENUM, Cell<mesh::Entity<Real, Integer, Real_Matrix>>(0))
    {
    }
    ...
    void batch_create_new_nodes_external_data(Cell<Pending_Node<Real,Integer, Real_Matrix, Integer_Matrix>> const & aPendingNodes,
                                              bool aFieldsToAdd=false)
    {
        ...
        for(Integer i = tInitialSize; i<tAddSize+tInitialSize;i++)
        {
            ...
            Mat<Real, Real_Matrix> const & tCoords = aPendingNodes(j).get_coordinates();
            ...
            if(aFieldsToAdd)
            {
                Mat<Real, Real_Matrix> const & tFieldData = aPendingNodes(j).get_field_data();
    ...
    void batch_create_new_nodes_with_fields_external_data(Cell<Pending_Node<Real,Integer, Real_Matrix, Integer_Matrix>> const & aPendingNodes,
    ...
    void
    get_all_node_coordinates_loc_inds_external_data(Integer aStartingIndex, Mat<Real, Real_Matrix> & aCoordinates) const
    {
        Integer tEntityRankIndex = (Integer)EntityRank::NODE;
        ...
        {
            const Mat<Real,Real_Matrix> & tCoordinateRow = mExternalEntities(tEntityRankIndex)(i).get_entity_coords();
    ...
    Integer allocate_entity_ids_external_entity_data(Integer aNumIdstoAllocate, enum EntityRank aEntityRank) const
    {
        ...
        Cell<Integer> aGatheredInfo;
        Cell<Integer> tFirstId(1);
        Cell<Integer> tNumIdsRequested(1);

        tNumIdsRequested(0) = (uint)aNumIdstoAllocate;

        gather(tNumIdsRequested,aGatheredInfo);

        Cell<Integer> tProcFirstID(tProcSize);
...
private:
    Cell<Integer> mFirstExtEntityInds;

    // Owned by proc rank 0, other procs UINT_MAX
    // Mutable to preserve const in the allocate entity ids function
    mutable Cell<Integer> mFirstAvailableIds;

    // Each processor tracks this value
    Cell<Integer> mFirstAvailableInds;

    // Entity Rank outside, then entity objects inside
    Cell<Cell<mesh::Entity<Real,Integer,Real_Matrix>>>mExternalEntities;

    // Fields
    Cell<std::string> mFieldNames;

private:
    void register_fields(Cell<std::string> const & aFieldNames)
    {
...
#endif /* SRC_MESH_CL_MESH_EXTERNAL_ENTITY_DATA_STK_HPP_ */
