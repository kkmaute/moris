...
namespace moris::xtk
{
    ...
    Hole_Seeder::seed_field()
    {
        ...
        Matrix< DDRMat > tCoords( tNumNodes, 3 );
        Matrix< DDRMat > tCoordsX( tNumNodes, 1 );
        Matrix< DDRMat > tCoordsY( tNumNodes, 1 );
        Matrix< DDRMat > tCoordsZ( tNumNodes, 1 );
        for ( moris::moris_index i = 0; i < (moris::moris_index)tNumNodes; i++ )
        {
            Matrix< DDRMat > tNodeCoord = mMTKMesh->get_node_coordinate( i );
        ...
        Vector< Matrix< DDRMat > > tCenters( tNumSpheres );
        ...
        for ( moris::uint i = 0; i < mNumSpheresInX; i++ )
        {
            for ( moris::uint j = 0; j < mNumSpheresInY; j++ )
            {
                ...
                {
                    Matrix< DDRMat > tSphereCenter = { { tMinX + xOffset + i * xOffset, tMinY + j * yOffset, tMinZ + k * zOffset } };
        ...
        Vector< Matrix< DDRMat > > tSphereLSV( tNumNodes );
        //    moris::real tNodeVal = 0;
        for ( moris::uint i = 0; i < tNumNodes; i++ )
        {
            tSphereLSV( i ) = Matrix< DDRMat >( 1, tNumSpheres );
