 ...
 */
 ...
 namespace moris::xtk
 {
     ...
     Basis_Processor::perform_basis_extention()
     {
         ...
         print_as_row_vector( mMeshIndices, "mMeshIndices_" + std::to_string(par_rank()) );
         for ( const auto& iMeshIndex : mMeshIndices )
         {
             ...
             {
                 std::cout<<"calling the comm table:"<<par_rank()<<std::endl;
                 this->construct_comm_table();
                 this->construct_follower_to_leader_basis_weights_indices_mine( iMeshIndex );
                 this->construct_follower_to_leader_basis_weights_indices_neighbors( iMeshIndex ); // neighbouring processors
                 this->communicate_shared_basis( iMeshIndex );
             }
 
             this->replace_t_matrices( iMeshIndex );

             // Once replaced T-matrices, need to write to file the list of deleted basis IDs for each bulk phase for given mesh.
             std::multimap tDeletedBasis = mBasisData( iMeshIndex ).mEliminatedBasis;

             // Get total number of phases
             size_t tNumPhases = mXTKModelPtr->get_geom_engine()->get_num_phases();
             
             for ( uint iPhaseIndex = 0; iPhaseIndex < tNumPhases; iPhaseIndex++ )
             {
                // Get phase index
                moris_index tPhaseIndex = iPhaseIndex;

                auto tItr1 = tDeletedBasis.lower_bound( tPhaseIndex );
                auto tItr2 = tDeletedBasis.upper_bound( tPhaseIndex );

                std::vector< int > tDeletedBasisIDs;

                while ( tItr1 != tItr2 )    
                {
                    if ( tItr1 -> first == tPhaseIndex )
                       {
                           tDeletedBasisIDs.push_back( tItr1 -> second );
                       }        
                    tItr1++;
                }    
                
                // Write the deleted basis IDs for the given phase index to file
                hid_t  tFileID = create_hdf5_file( "Deleted_Basis_IDs_" + std::to_string( iMeshIndex ) + "_" + std::to_string( iPhaseIndex ) +".hdf5" );
                herr_t tStatus = 0;
                save_vector_to_hdf5_file( tFileID, std::string("IDs"), tDeletedBasisIDs, tStatus );
     ...
     Basis_Processor::replace_t_matrices( moris_index aMeshIndex )
     {
         ...
         Vector< Matrix< IndexMat > > const &           tSubphaseGroupIndsInEnrichedBasis = mXTKModelPtr->mEnrichment->mEnrichmentData( aMeshIndex ).mSubphaseGroupIndsInEnrichedBasis;
         Enriched_Interpolation_Mesh&                   tEnrInterpMesh                    = mXTKModelPtr->get_enriched_interp_mesh( 0 );
         ...
         for ( size_t iSPGIndex = 0; iSPGIndex < mRootSPGIds.size(); iSPGIndex++ )
         {
             ...
             else if (  mRootSPGIds( iSPGIndex ) == gNoID)
             {
                 ...
                 // Get the subphase group
                 const Subphase_Group* tSubphaseGroup = tSPGs( iSPGIndex );

                 // Get bulk phase index of subphase group
                 moris_index tSPGBulkPhaseIndex = tSubphaseGroup->get_bulk_phase();
                 ...
                 for ( const auto& iUIPCIndex : tUIPCIndices )
                 {
                     ...
                     for ( size_t iLocalVertIndex = 0; iLocalVertIndex < tXTKUIPVs.size(); iLocalVertIndex++ )
                     {
                         ...
                         Matrix< IdMat >            tBasisOwners  = tVertexEnrichment->get_owners();
                         IndexMap&                  tBasisMap     = tVertexEnrichment->get_basis_map();
                         Matrix< IdMat >            tBasisIDs     = tVertexEnrichment->get_ids();
                         tBasisMap.clear();
                         ...
                         for ( uint iBC = 0; iBC < tBasisIndices.numel(); iBC++ )
                         {
                             ...
                             moris::moris_index tBasisIndex  = tBasisIndices( iBC );
                             real&              tBasisWeight = tBasisWeights( iBC );
                             moris_id           tBasisId     = tBasisIDs( iBC );
                             ...
                             else
                             {
                              ...
                              // Append basis ID to the list of deleted basis IDs
                              mBasisData( aMeshIndex ).mEliminatedBasis.insert({ tSPGBulkPhaseIndex , tBasisId });
     ...
     Basis_Processor::construct_comm_table()
     {
         ...
         //print_as_row_vector( tProcessorsToReceiveFrom, "tProcessorsToReceiveFrom_" + std::to_string(par_rank()) );
         //std::cout << "tSize_" + std::to_string(par_rank()) + ": " << tSize << std::endl;
         ...
         barrier();
         //print_as_row_vector( tProcessorsToReceiveFromAllProcs, "tProcessorsToReceiveFromAllProcs_" + std::to_string(par_rank()) );
