...
void compute_dx_dp_with_linear_basis(moris::Mat_New<Real,Real_Matrix>  & aDPhiADp,
                                     moris::Mat_New<Real,Real_Matrix>  & aDPhiBDp,
                                     moris::Mat_New<Real,Real_Matrix>  & aEdgeCoordinates,
                                     moris::Mat_New<Real,Real_Matrix>  & aEdgeNodePhi,
                                     moris::Mat_New<Real,Real_Matrix>  & aDxDp)
{
    ...
    // Initialize
    moris::Mat_New<Real,Real_Matrix> tXa = aEdgeCoordinates.get_row(0);

    moris::Mat_New<Real,Real_Matrix> tXb = aEdgeCoordinates.get_row(1);
    ...
    moris::Mat_New<Real,Real_Matrix> tDxgDphiAMat(tDxgammaDphiA);
    moris::Mat_New<Real,Real_Matrix> tDxgDphiBMat(tDxgammaDphiB);

    // Compute dx/dp
    Real_Matrix tDxDp = aDPhiADp * moris::trans(tDxgDphiAMat) +  aDPhiBDp * moris::trans(tDxgDphiBMat);
    aDxDp = moris::Mat_New<Real,Real_Matrix>(tDxDp);
...
public:
     ...
     */
    void
    create_geometry_objects_for_background_mesh_nodes(moris::Mat_New<Real,Real_Matrix> const & aNodeCoords)
    {
        // Allocate space
        Integer tNumNodes = aNodeCoords.n_rows();
        mNodePhaseVals = moris::Mat_New<Real,Real_Matrix>(tNumNodes,mGeometry.size(),0);
        ...
        mDxDp = Cell<moris::Mat_New<Real,Real_Matrix>>(tNumNodes,moris::Mat_New<Real,Real_Matrix>(0,0,0.0));
        mNodeADVIndices = Cell<moris::Mat_New<Integer, Integer_Matrix>>(tNumNodes,moris::Mat_New<Integer, Integer_Matrix>(0,0,0));

        // Associate each geometry object with a row in phase val matrix (note phase val computed later)
        moris::Mat_New<Integer, Integer_Matrix> tNodeIndex(1,tNumNodes);
                                             ...
                                             bool aInterfaceNodes)
    {
        ...
        mDxDp.resize(tNumNewNodes+tNumCurrNodes,moris::Mat_New<Real,Real_Matrix>(0,0,0.0));
        mNodeADVIndices.resize(tNumNewNodes+tNumCurrNodes,moris::Mat_New<Integer, Integer_Matrix>(0,0,0));

        Cell<Geometry_Object<Real, Integer, Real_Matrix,Integer_Matrix>> tGeometryObjects(tNumNewNodes);

        moris::Mat_New<Integer, Integer_Matrix> tNodeIndex(1,tNumNewNodes);
        ...
        for(Integer j = 0; j<get_num_geometries(); j++)
        {
            ...
            for(Integer i = 0; i<tNumNewNodes; i++ )
            {
                ...
                moris::Mat_New<Real,Real_Matrix> const & tLocalCoordinate = aNewNodes(i).get_local_coordinate_relative_to_parent();
                moris::Mat_New<Real,Real_Matrix> tLevelSetValues(1,1);
     ...
     */
    void is_intersected(moris::Mat_New<Real,Real_Matrix> const &                                      aNodeCoords,
                        moris::Mat_New<Integer, Integer_Matrix> const &                               aNodetoEntityConn,
                        Integer                                                            aCheckType,
                        Cell<Geometry_Object<Real, Integer, Real_Matrix,Integer_Matrix>> & aGeometryObjects)
    {
        ...
        for(Integer i = 0; i < tNumEntities; i++)
        {
            ...
            moris::Mat_New<Integer, Integer_Matrix> tRow = aNodetoEntityConn.get_row(i);
            moris::Mat_New<Integer, Integer_Matrix> tNodeADVIndices;
            Cell<moris::Mat_New<Real,Real_Matrix>>tIntersectionInfo = compute_intersection_info(tRow, aNodeCoords, aCheckType,tNodeADVIndices);
    ...
    get_intersection_location(Real const &                         aIsocontourThreshold,
                              ...
                              moris::Mat_New<Real,Real_Matrix> const &        aGlobalNodeCoordinates,
                              moris::Mat_New<Real,Real_Matrix> const &        aEntityNodeVars,
                              moris::Mat_New<Integer, Integer_Matrix> const & aEntityNodeIndices,
                              moris::Mat_New<Real,Real_Matrix> &              aIntersectionLocalCoordinates,
                              moris::Mat_New<Real,Real_Matrix> &              aIntersectionGlobalCoordinates,
                              ...
                              bool                                 aComputeGlobalCoordinate = false)
    {
            ...
            if(aComputeGlobalCoordinate)
            {
                ...
                moris::Mat_New<Real,Real_Matrix> tEntityCoordinates(2,3);
    ...
    compute_dx_dp_finite_difference(Real                                 const & aPerturbationVal,
                                    moris::Mat_New<Real,Real_Matrix>                const & aGlobalNodeCoordinates,
                                    moris::Mat_New<Real,Real_Matrix>                const & aEntityNodeCoordinates,
                                    moris::Mat_New<Real,Real_Matrix>                const & aIntersectionLclCoordinate,
                                    moris::Mat_New<Integer, Integer_Matrix>         const & aEntityNodeIndices,
                                    moris::Mat_New<Real,Real_Matrix>       & aEntityNodeVars,
                                    moris::Mat_New<Real,Real_Matrix>       & aDxDp)
    {
        ...
        moris::Mat_New<Real,Real_Matrix>       tDxDp(1,3);
        moris::Mat_New<Real,Real_Matrix>       tPerturbedLocalCoordinate(1,1);
        Cell<moris::Mat_New<Real,Real_Matrix>> tPerturbedGlobCoordinates = {moris::Mat_New<Real,Real_Matrix>(1,3),
                                                                 moris::Mat_New<Real,Real_Matrix>(1,3)};
    ...
    void
    compute_dx_dp_for_an_intersection(moris::Mat_New<Integer, Integer_Matrix> const & aEntityNodeIndices,
                                      moris::Mat_New<Real,Real_Matrix> const &       aGlobalNodeCoordinates,
                                      moris::Mat_New<Real,Real_Matrix> const &       aIntersectionLclCoordinate,
                                      moris::Mat_New<Real,Real_Matrix> &             aEntityNodeVars,
                                      moris::Mat_New<Real,Real_Matrix> &             aDxDp,
                                      moris::Mat_New<Integer, Integer_Matrix> & aADVIndices)
    {
        ...
        // Initialize
        Cell<moris::Mat_New<Real,Real_Matrix>> tDPhiiDp = {moris::Mat_New<Real,Real_Matrix>(0,0),
                                                moris::Mat_New<Real,Real_Matrix>(0,0)};

        moris::Mat_New<Real,Real_Matrix> tEntityNodeCoordinates(2,3);
     ...
     */
    void get_phase_index(moris::Mat_New<Integer, Integer_Matrix> const & aNodeIndex,
                         moris::Mat_New<Integer, Integer_Matrix> & aNodePhaseIndex)
    {
        // 0 for neg 1 for pos
        Real tNodePhaseValue = 0;
        moris::Mat_New<Integer, Integer_Matrix> tPhaseOnOff(1,mGeometry.size());
     ...
     */
    Integer
    get_elem_phase_index(moris::Mat_New<Integer, Integer_Matrix> const & aElemOnOff)
    {
    ...
    get_node_adv_indices_analytic()
    {
        ...
        moris::Mat_New<Integer, Integer_Matrix> tMatrix(1,tNumDVS);
     ...
     */
    moris::Mat_New<Integer, Integer_Matrix>
    get_node_adv_indices_discrete(moris::Mat_New<Integer, Integer_Matrix> const & aEntityNodes)
    {
        moris::Mat_New<Integer, Integer_Matrix> tNodeADVIndices = ActiveGeometry().get_node_adv_indices(aEntityNodes);
     ...
     */
    void
    store_dx_dp(moris::Mat_New<Integer, Integer_Matrix> const & aDxDp,
                moris::Mat_New<Integer, Integer_Matrix> const & aNodeADVIndices)
    {
...
private:
    ...
    moris::Mat_New<Real,Real_Matrix> mNodePhaseVals;

    // Sensitivity Data
    Cell<moris::Mat_New<Real,Real_Matrix>> mDxDp;
    Cell<moris::Mat_New<Integer, Integer_Matrix>> mNodeADVIndices;
...
private:
     ...
     **/
    Cell<moris::Mat_New<Real,Real_Matrix>>
    compute_intersection_info(moris::Mat_New<Integer, Integer_Matrix> const & aEntityNodeInds,
                              moris::Mat_New<Real,Real_Matrix> const &        aNodeCoords,
                              Integer const &                      aCheckType,
                              moris::Mat_New<Integer, Integer_Matrix> &       aNodeADVIndices )
    {
        Cell<moris::Mat_New<Real,Real_Matrix>> tIntersectionInfo(3);
        ...
        moris::Mat_New<Real,Real_Matrix> tEntityNodeVars(tNumNodes, 1);
        moris::Mat_New<Real,Real_Matrix> tInterpLocationCoords(1,1);
        ...
        moris::Mat_New<Real,Real_Matrix> tIntersection(1, 2, 0.0);// Initialize as false
        ...
        if((tMax > mThresholdValue) && (tMin < mThresholdValue))
        {
            ...
            if(aCheckType == 1)
            {
                ...
                moris::Mat_New<Real,Real_Matrix> tIntersectLocalCoordinate(1,1);
                moris::Mat_New<Real,Real_Matrix> tIntersectGlobalCoordinate(1,3);
                ...
                if(mComputeDxDp)
                {
                    moris::Mat_New<Real,Real_Matrix> tDxDp(1,1,100.0);
    ...
    void
     interpolate_level_set_value_to_child_node_location(Topology<Real, Integer, Real_Matrix, Integer_Matrix> const & aParentTopology,
                                                        ...
                                                        moris::Mat_New<Real,Real_Matrix> const &                                aNodeLocalCoordinate,
                                                        moris::Mat_New<Real,Real_Matrix> &                                      aLevelSetValues)
     {

         // Get node indices attached to parent (These are indices relative to another mesh and may need to be mapped)
         moris::Mat_New<Integer, Integer_Matrix> const & tNodesAttachedToParent = aParentTopology.get_node_indices();

         // Get number of nodes attached to parent
         Integer tNumNodesAttachedToParent = tNodesAttachedToParent.n_cols();
         moris::Mat_New<Real,Real_Matrix> tNodesLevelSetValues(1, tNumNodesAttachedToParent);
         ...
         moris::Mat_New<Real,Real_Matrix> tBasisValues(1,1);
