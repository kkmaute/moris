...
namespace moris::xtk
{
    ...
    {
        Vector< std::shared_ptr< Matrix< DDRMat > > >* tCoords = aCutMesh->get_all_vertex_coordinates_loc_inds();

        // keep track of problem vertices
        Vector< moris_index > tProblemIgVertices( 0 );

        // Allocate a basis function weight matrix
        Matrix< DDRMat > tBasisWeights( 1, 8 );
        ...
        for ( moris::uint iIgCellGroup = 0; iIgCellGroup < tNumIgCellGroups; iIgCellGroup++ )
        {
            ...
            Matrix< DDRMat > tBaseCellCoords = tBaseCell->get_vertex_coords();
            ...
            for ( moris::uint iCell = 0; iCell < tIgCellGroup->mIgCellGroup.size(); iCell++ )
            {
                ...
                Vector< moris::mtk::Vertex* > tVertices = tIgCell->get_vertex_pointers();
                ...
                for ( moris::uint iV = 0; iV < tVertices.size(); iV++ )
                {
                    ...
                    Matrix< DDRMat > tInterpNodeCoord = tBasisWeights * tBaseCellCoords;
                    ...
                    if ( norm( tInterpNodeCoord - *( ( *tCoords )( tNodeIndex ) ) ) > tTol )
                    {
                        ...
                        Matrix< DDRMat > tCoords = tVertices( iV )->get_coords();
            ...
            Matrix< IndexMat > const &                                           aGoldCellConn )
    {
        ...
        for ( moris::uint iCell = 0; iCell < tCutIgMesh->get_num_elems(); iCell++ )
        {
            ...
            Matrix< IndexMat > tCellVertIds = tCell.get_vertex_ids();
