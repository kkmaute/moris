...
Real vol_tetrahedron(Mat<Real,Real_Matrix>  & aCoord)
{
    //explanation: www.colorado.edu/engineering/Aerospace/CAS/courses.d/AFEM.d/AFEM.Ch09.d/AFEM.Ch09.pdf
    Mat<Real,Real_Matrix> J(4,4,1.0); // 4 Coordinates are needed for the Jacobian-matrix.
    ...
    Real volume = (determinant(J))/6; // Volume = 1/6*det(J)
    return volume;
}
}
