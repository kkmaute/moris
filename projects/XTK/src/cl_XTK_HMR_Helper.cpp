...
namespace xtk
{
            ...
            , mBsplineMeshIndex( aMeshIndex )
    {
        ...
        mMatrices1D = moris::Vector< Matrix< DDRMat > >( mSpatialDimension, Matrix< DDRMat >( mNumberOfNodesPerDimension, mNumberOfNodesPerDimension ) );

        // intermediate variable to access the enrichment data in the next line
        moris::Vector< Enrichment_Data > const & tEnrichmentData = mXTKModelPtr->get_basis_enrichment().get_enrichment_data();
    ...
    HMR_Helper::get_enriched_basis_id_of_cell( const mtk::Cell* aCell, moris_index aSPGIndex )
    {
        ...
        moris::Vector< moris_index > const & tBGBasisIndices = mEnrichmentData->mSubphaseGroupBGBasisIndices( aSPGIndex );
        moris::Vector< moris_index > const & tBGBasisLevels  = mEnrichmentData->mSubphaseGroupBGBasisEnrLev( aSPGIndex );
        ...
        moris::Vector< Matrix< IndexMat > > const & tBasisEnrichmentIndices = mEnrichmentData->mBasisEnrichmentIndices;
