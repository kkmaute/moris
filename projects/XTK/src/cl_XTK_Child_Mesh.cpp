...
namespace moris::xtk
{
    Child_Mesh::Child_Mesh()
            ...
            , mSubPhaseBins( 0, Matrix< IndexMat >( 0, 0 ) )
    {
    }
            ...
            , mSubPhaseBins( 0, Matrix< IndexMat >( 0, 0 ) )
    {
        ...
        moris::mtk::Cell_Info const *tParentCellInfo = aParentCell->get_cell_info();

        Matrix< IndexMat > tElementToEdge = aBackgroundMesh->get_entity_connected_to_entity_loc_inds( aParentCell->get_index(), moris::mtk::EntityRank::ELEMENT, moris::mtk::EntityRank::EDGE );
        ...
        Matrix< IndexMat > tFacetoElemConnInd = aBackgroundMesh->get_entity_connected_to_entity_loc_inds(
                ...
                moris::mtk::EntityRank::ELEMENT,
                moris::mtk::EntityRank::FACE );
        ...
        mNodeParentRank         = Matrix< moris::DDSTMat >( 1, tVertexIndices.numel() );
        mElementToNode          = tVertexIndices;
        mElementEdgeParentInds  = tElementToEdge;
        mElementEdgeParentRanks = Matrix< moris::DDSTMat >( 1, tFacetoElemConnInd.numel(), 1 );
        mElementFaceParentInds  = tFacetoElemConnInd;
        mElementFaceParentRanks = Matrix< moris::DDSTMat >( 1, tFacetoElemConnInd.numel(), 2 );
        mElementInterfaceSides  = Matrix< moris::DDSTMat >( 1, aGeometryIndices.size(), std::numeric_limits< moris::size_t >::max() );
            ...
            , mSubPhaseBins( 0, Matrix< IndexMat >( 0, 0 ) )
    {
        ...
        if ( mElementTopology == moris::mtk::CellTopology::TET4 )
        {
            generate_connectivities( true, true, true );
        }
        else if ( mElementTopology == moris::mtk::CellTopology::TRI3 )
        {
    ...
    moris::size_t
    Child_Mesh::get_num_entities( moris::mtk::EntityRank aEntityRank ) const
    {
        moris::size_t tNumEntities = 0;
        if ( aEntityRank == moris::mtk::EntityRank::NODE )
        {
            tNumEntities = mNodeInds.n_cols();
        }
        else if ( aEntityRank == moris::mtk::EntityRank::EDGE )
        {
        ...
        else if ( aEntityRank == moris::mtk::EntityRank::FACE )
        {
        ...
        else if ( aEntityRank == moris::mtk::EntityRank::ELEMENT )
        {
    ...
    Child_Mesh::get_facet_rank() const
    {
        ...
        else
        {
            ...
            return moris::mtk::EntityRank::UNDEFINED;
        }
    }
    ...
    {
        Matrix< IdMat > tElementToNode = this->get_element_to_node().get_row( aCMElemIndex );
    ...
    Child_Mesh::get_node_to_element_local() const
    {
        ...
        Matrix< IndexMat > tElementToNodeLocal = this->get_element_to_node_local();
        ...
        Matrix< IndexMat > tNodeToElementLocal( this->get_num_entities( moris::mtk::EntityRank::NODE ), this->get_num_entities( moris::mtk::EntityRank::ELEMENT ) );
        tNodeToElementLocal.fill( MORIS_INDEX_MAX );

        // keep track of number of nodes a node is connected to.
        Matrix< IndexMat > tNodetoElementCounter( this->get_num_entities( moris::mtk::EntityRank::NODE ), 1, 0 );

        // Iterate through elements
        for ( moris::uint iEl = 0; iEl < this->get_num_entities( moris::mtk::EntityRank::ELEMENT ); iEl++ )
        {
        ...
        // Resize
        tNodeToElementLocal.resize( this->get_num_entities( moris::mtk::EntityRank::NODE ), tNodetoElementCounter.max() );
    ...
    {
        Matrix< IndexMat > const &tCellToFacet = this->get_element_to_facet();
    ...
    moris::moris_index
    Child_Mesh::get_entity_parent_entity_rank( moris::mtk::EntityRank aEntityRank,
            moris::moris_index                                        aCMLocIndex )
    {
        switch ( aEntityRank )
        {
            case moris::mtk::EntityRank::NODE:
            {
                return mNodeParentRank( aCMLocIndex );
                break;
            }
            case moris::mtk::EntityRank::EDGE:
            {
            ...
            case moris::mtk::EntityRank::FACE:
            {
            ...
            case moris::mtk::EntityRank::ELEMENT:
            {
                return (moris::moris_index)moris::mtk::EntityRank::ELEMENT;
                break;
            }
            default:
            {
    ...
    Child_Mesh::get_entity_parent_entity_proc_ind(
            moris::mtk::EntityRank aEntityRank,
            moris::moris_index     aCMLocIndex )
    {
        switch ( aEntityRank )
        {
    ...
    // ---------------------------------------------------------------------------------
    Matrix< IndexMat >
    Child_Mesh::convert_to_proc_local_elem_inds( Matrix< IndexMat > aConnOfElementCMIndices )
    {
        Matrix< IndexMat > tConnWithElementsProcIndices( aConnOfElementCMIndices.n_rows(), aConnOfElementCMIndices.n_cols() );
            ...
            Matrix< IndexMat > const &aEdgeIndices ) const
    {
        ...
        Matrix< IndexMat > const &tElemToEdge = get_element_to_edge();
        ...
        // Initialize output
        Matrix< IndexMat > tEdgeOrdinals( 1, tNumOrdinalstoFind );
    ...
    {
        // get the elemnt to edge connectivity
        Matrix< IndexMat > const &tElemToFacet = get_element_to_facet();
            ...
            Matrix< IndexMat >       &aChildElemOnFacetOrdinal ) const
    {
        ...
        Matrix< IndexMat > tLocFacesOnParentFace( 1, tNumFacets );

        Matrix< IndexMat > const       &tFacetParentInds  = this->get_facet_parent_inds();
        Matrix< moris::DDSTMat > const &tFacetParentRanks = this->get_facet_parent_ranks();
        Matrix< IndexMat > const       &tFacetToElement   = this->get_facet_to_element();
            ...
            Matrix< IndexMat > &aInterfacePairSideOrds ) const
    {
        ...
        Matrix< IndexMat > const &tElementToFacet = this->get_element_to_facet();
        ...
        Matrix< IndexMat > const &tFacetToElem = this->get_facet_to_element();
            ...
            Matrix< IndexMat > const &aUnzippedInterfaceNodeIds )
    {
        ...
        Matrix< IndexMat > tNodeToFaceMap = mConnectivity->get_node_to_face_map();

        // Copy starting element to node connectivity
        Matrix< IndexMat > tOldElementToNode = mElementToNode.copy();

        // Get the node to element connectivity
        Matrix< IndexMat > tNodeToElement = get_node_to_element_local();
        ...
        Matrix< IndexMat > tElementsConnectedToNodeTraversed( tNumInterfaceNodes, 1, 0 );
    ...
    void
    Child_Mesh::add_node_indices( Matrix< IndexMat > const &aNewNodeInds )
    {
        // Allocate space
    ...
    void
    Child_Mesh::add_node_ids( Matrix< IdMat > const &aNewNodeIds )
    {
        // Allocate space
    ...
    void
    Child_Mesh::set_node_ids( Matrix< IdMat > const &aNewNodeIds )
    {
        MORIS_ASSERT( aNewNodeIds.n_cols() == get_num_entities( moris::mtk::EntityRank::NODE ),
    ...
    Child_Mesh::set_child_element_ids( moris::moris_id &aElementId )
    {
        ...
        moris::size_t tNumElements = get_num_entities( moris::mtk::EntityRank::ELEMENT );
        mChildElementIds           = Matrix< IdMat >( 1, tNumElements );
    ...
    {
        moris::size_t tNumElements = get_num_entities( moris::mtk::EntityRank::ELEMENT );
        mChildElementInds          = Matrix< IndexMat >( 1, tNumElements );
    ...
    {
        MORIS_ASSERT( aCMLocNodeIndex < (moris::moris_index)this->get_num_entities( moris::mtk::EntityRank::NODE ),
            ...
            bool aGenerateElemToElem )
    {
        ...
        Matrix< IndexMat > tElementToNodeLoc = get_element_to_node_local();
    ...
    Child_Mesh::add_new_geometry_interface( moris_index aGeomIndex )
    {
        ...
        mElementInterfaceSides.resize( this->get_num_entities( moris::mtk::EntityRank::ELEMENT ), mGeometryIndex.size() );

        // last col index
        moris_index tLastCol = mGeometryIndex.size() - 1;
        for ( moris::uint i = 0; i < this->get_num_entities( moris::mtk::EntityRank::ELEMENT ); i++ )
        {
            ...
            moris::size_t      aFlag )
    {
        if ( aFlag == 0 )
        {
            aCMNodeInd = aCMNodeInd + get_num_entities( moris::mtk::EntityRank::NODE );
        }
    ...
    {
        MORIS_ASSERT( aEdgeIndex < (moris::moris_index)get_num_entities( moris::mtk::EntityRank::EDGE ), "Edge index out of bounds" );
        MORIS_ASSERT( mEdgeOnInterface.numel() == get_num_entities( moris::mtk::EntityRank::EDGE ), "mEdgeOnInterface has not been properly initialized" );
    ...
    Child_Mesh::mark_interface_faces_from_interface_coincident_faces()
    {
        if ( mHasCoincidentEdges && mSpatialDimension == 3 )
        {
            ...
            uint               tNumElements     = this->get_num_entities( moris::mtk::EntityRank::ELEMENT );
            ...
            Matrix< IndexMat > tElementEdgeOrdsOnInterface( tNumElements, tNumEdgesPerElem );
            tElementEdgeOrdsOnInterface.fill( tDummy );
            Matrix< IndexMat > tElemCounter( tNumElements, 1, 0 );

            // Get reference to edge to element connectivity
            Matrix< IndexMat > const &tEdgeToElement = get_edge_to_element();
            ...
            Matrix< IndexMat > const tEdgeOrdinalToFaceOrdMap = mConnectivity->get_edge_to_face_map();

            Matrix< DDUMat > tFaceCounter( 1, tNumFacesPerElem, 0 );
            moris_index      tGeomIndex = mGeometryIndex.size() - 1;
        ...
        {
            Matrix< IndexMat > const tEdgeToElem = this->get_edge_to_element();
            moris_index              tGeomIndex  = mGeometryIndex.size() - 1;
            moris::moris_index       tDummy      = std::numeric_limits< moris::moris_index >::max();
            ...
            Vector< moris_index > &aRepresentativeChildCellSideOrdinal ) const
    {
        ...
        Matrix< IndexMat > const &tElementSubphase = this->get_elemental_subphase_bin_membership();
        Matrix< IndexMat > const &tElementInds     = this->get_element_inds();
    ...
    Child_Mesh::construct_internal_double_sides_between_subphases()
    {
        ...
        Vector< Matrix< IndexMat > > const &tSubphaseClusters   = this->get_subphase_groups();
        ...
        Matrix< moris::DDSTMat > const &tCellInterfaceSideOrds = this->get_cell_interface_side_ords();

        // facet parent inds and ranks
        // Matrix< IndexMat > const & tFacetParentInds  = tChildMesh->get_facet_parent_inds();
        Matrix< moris::DDSTMat > const &tFacetParentRanks = this->get_facet_parent_ranks();
        Matrix< IndexMat > const       &tCellFacetInds    = this->get_element_to_facet();
            ...
            Vector< Matrix< IdMat > > &aElementCMInds ) const
    {
        ...
        aElementIds    = Vector< Matrix< IdMat > >( aNumPhases );
        aElementCMInds = Vector< Matrix< IdMat > >( aNumPhases );

        for ( moris::size_t i = 0; i < aNumPhases; i++ )
        {
            aElementIds( i )    = Matrix< IdMat >( 1, tNumElems );
            aElementCMInds( i ) = Matrix< IdMat >( 1, tNumElems );
        }
            ...
            moris_index aIndexFlag ) const
    {
        ...
        moris::size_t tNumElem = get_num_entities( moris::mtk::EntityRank::ELEMENT );

        // local geometry index
        moris_index tLocGeomIndex = this->get_local_geom_index( aGeometryIndex );

        Matrix< IdMat > tInterfaceCMInfo( tNumElem, 2, MORIS_ID_MAX );

        // Keep track of the number of interface sides
        moris::size_t tCount = 0;

        // construct a membership map in interface side set
        Matrix< IndexMat > tCellsIndexInSet( this->get_num_entities( moris::mtk::EntityRank::ELEMENT ), 1, MORIS_INDEX_MAX );
        ...
        bool               tNoPair = true;
        Matrix< IndexMat > tInterfaceElementPairs;
        Matrix< IndexMat > tInterfacePairSideOrds;
        ...
        Matrix< IndexMat > tCellsToKeepInSet( this->get_num_entities( moris::mtk::EntityRank::ELEMENT ), 1, MORIS_INDEX_MAX );
        ...
        Matrix< IdMat > tInterfaceInfo( tKeepCount, 2 );
        tCount = 0;
        for ( moris::size_t iEl = 0; iEl < tNumElem; iEl++ )
        {
    ...
    Child_Mesh::pack_interface_sides_loc_inds() const
    {
        ...
        Matrix< IdMat > tInterfaceSideSetInfo( tNumElem, 2 );
    ...
    void
    Child_Mesh::generate_face_connectivity_and_ancestry( Matrix< IndexMat > const &aElementToNodeLocal )
    {

        xtk::create_faces_from_element_to_node(
                mConnectivity.get(),
                this->get_num_entities( moris::mtk::EntityRank::NODE ),
                aElementToNodeLocal,
                mElementToFace,
                mFaceToNode,
                mNodeToFace,
    ...
    void
    Child_Mesh::generate_edge_connectivity_and_ancestry( Matrix< IndexMat > const &aElementToNodeLocal )
    {
        create_edges_from_element_to_node(
                mElementTopology,
                get_num_entities( moris::mtk::EntityRank::NODE ),
                aElementToNodeLocal,
                mElementToEdge,
                mEdgeToNode,
                mNodeToEdge,
    ...
    {
        moris::size_t tNumNodes = get_num_entities( moris::mtk::EntityRank::NODE );
    ...
    void
    Child_Mesh::add_nodes_to_map( Matrix< IndexMat > const &aNodesToAdd )
    {
        moris::size_t tNumNodes    = get_num_entities( moris::mtk::EntityRank::NODE );
    ...
    Child_Mesh::setup_face_ancestry()
    {
        ...
        moris::size_t tNumFaces          = get_num_entities( moris::mtk::EntityRank::FACE );

        mFaceParentInds  = Matrix< IndexMat >( 1, tNumFaces );
        mFaceParentRanks = Matrix< moris::DDSTMat >( 1, tNumFaces );
    ...
    Child_Mesh::setup_edge_ancestry()
    {
        ...
        moris::size_t tNumEdges          = get_num_entities( moris::mtk::EntityRank::EDGE );

        mEdgeParentInds  = Matrix< IndexMat >( 1, tNumEdges );
        mEdgeParentRanks = Matrix< moris::DDSTMat >( 1, tNumEdges );
    ...
    // ---------------------------------------------------------------------------------
    Matrix< IndexMat >
    Child_Mesh::convert_to_cm_local_indices( Matrix< IndexMat > const &EntityRankToNode ) const
    {
        moris::size_t tNumRows = EntityRankToNode.n_rows();
        moris::size_t tNumCols = EntityRankToNode.n_cols();

        Matrix< IndexMat > tLocalEntityRankToNode( tNumRows, tNumCols );

        for ( moris::size_t i = 0; i < tNumRows; i++ )
        {
            for ( moris::size_t j = 0; j < tNumCols; j++ )
            {
                auto tIter = mNodeIndsToCMInd.find( EntityRankToNode( i, j ) );
    ...
    Matrix< IndexMat >
    Child_Mesh::convert_to_proc_indices( Matrix< IndexMat > const &EntityRankToNodeLocal ) const
    {
        moris::size_t tNumRows = EntityRankToNodeLocal.n_rows();
        moris::size_t tNumCols = EntityRankToNodeLocal.n_cols();

        Matrix< IndexMat > tProcmorisEntityRankToNode( tNumRows, tNumCols );

        for ( moris::size_t i = 0; i < tNumRows; i++ )
        {
            for ( moris::size_t j = 0; j < tNumCols; j++ )
            {
                tProcmorisEntityRankToNode( i, j ) = mNodeInds( 0, EntityRankToNodeLocal( i, j ) );
            }
        }

        return tProcmorisEntityRankToNode;
    }

    // ---------------------------------------------------------------------------------

    Matrix< IdMat >
    Child_Mesh::convert_cm_loc_to_glob_ids( Matrix< IndexMat > const &EntityRankToNodeLocal ) const
    {
        moris::size_t tNumRows = EntityRankToNodeLocal.n_rows();
        moris::size_t tNumCols = EntityRankToNodeLocal.n_cols();

        Matrix< IdMat > tProcmorisEntityRankToNode( tNumRows, tNumCols );

        for ( moris::size_t i = 0; i < tNumRows; i++ )
        {
            for ( moris::size_t j = 0; j < tNumCols; j++ )
            {
                tProcmorisEntityRankToNode( i, j ) = mNodeIds( 0, EntityRankToNodeLocal( i, j ) );
            }
        }
    ...
    Matrix< IdMat >
    Child_Mesh::convert_proc_to_glob_ids( Matrix< IndexMat > const &EntityRankToNodeLocal ) const
    {
        moris::size_t tNumRows = EntityRankToNodeLocal.n_rows();
        moris::size_t tNumCols = EntityRankToNodeLocal.n_cols();
        ...
        for ( moris::size_t i = 0; i < tNumRows; i++ )
        {
            for ( moris::size_t j = 0; j < tNumCols; j++ )
            {
                moris::uint tNodeCMIndex = this->get_cm_local_node_index( EntityRankToNodeLocal( i, j ) );

                tProcmorisEntityRankToNode( i, j ) = mNodeIds( 0, tNodeCMIndex );
            }
        }
    ...
    Child_Mesh::modify_child_mesh_internal( enum TemplateType aTemplate )
    {
        if ( aTemplate == TemplateType::HIERARCHY_TET4 )
        {
            ...
            Matrix< IndexMat > tEdgeToNodeCMLoc = get_edge_to_node_local();
            Matrix< IndexMat > tElemToNodeCMLoc = get_element_to_node_local();
            Matrix< IndexMat > tElemToEdgeCMLoc = get_element_to_edge();
            ...
            for ( moris::size_t iE = 0; iE < tNumExistElem; iE++ )
            {
                if ( mIntersectConnectivity( iE, 0 ) == 3 || mIntersectConnectivity( iE, 0 ) == 4 )
                {
                    ...
                    Matrix< IndexMat > tIntersectConnRow = mIntersectConnectivity.get_row( iE );

                    // Sort nodes by edge intersection
                    Matrix< IndexMat > tSortedNodes = sort_nodes( aTemplate, tIntersectConnRow, tEdgeToNodeCMLoc, iE, tPermutationId );
                    ...
                    Matrix< IndexMat >       tElementsAncestry( { { mParentElementIndex } } );    // Not used
                    Matrix< IndexMat >       tParentEdgeInds  = mElementEdgeParentInds.get_row( iE );
                    Matrix< moris::DDSTMat > tParentEdgeRanks = mElementEdgeParentRanks.get_row( iE );
                    Matrix< IndexMat >       tParentFaceInds  = mElementFaceParentInds.get_row( iE );
                    Matrix< moris::DDSTMat > tParentFaceRanks = mElementFaceParentRanks.get_row( iE );
                ...
                else if ( mIntersectConnectivity( iE, 0 ) == 1 )
                {
                    ...
                    Matrix< IndexMat > tSortedNodes = { { mElementToNode( iE, 0 ),
                            mElementToNode( iE, 1 ),
                            mElementToNode( iE, 2 ),
                            mElementToNode( iE, 3 ),
                ...
                else if ( mIntersectConnectivity( iE, 0 ) == 2 )
                {
                    ...
                    Matrix< IdMat > const &tNodeIds = this->get_node_ids();
                    ...
                    Matrix< IndexMat > tSortedNodes = { { mElementToNode( iE, 0 ),
                            ...
                            mNodeInds( tNodeH ) } };
                    // Get parent element information
                    Matrix< IndexMat >       tElementsAncestry( { { mParentElementIndex } } );
                    Matrix< IndexMat >       tParentEdgeInds  = mElementEdgeParentInds.get_row( iE );
                    Matrix< moris::DDSTMat > tParentEdgeRanks = mElementEdgeParentRanks.get_row( iE );
                    Matrix< IndexMat >       tParentFaceInds  = mElementFaceParentInds.get_row( iE );
                    Matrix< moris::DDSTMat > tParentFaceRanks = mElementFaceParentRanks.get_row( iE );
        ...
        else if ( aTemplate == TemplateType::CONFORMAL_TRI3 )
        {
            ...
            for ( moris::size_t iE = 0; iE < tNumExistElem; iE++ )
            {
                ...
                if ( mIntersectConnectivity( iE, 0 ) == 2 )
                {
                    ...
                    Matrix< IndexMat > tEdgeOrdinals = get_edge_ordinal_from_element_and_edge_indices( iE, { { tEdge0, tEdge1 } } );
                    ...
                    Matrix< IndexMat > tNodesInTemplate( 1, 5 );
                    ...
                    // Setup template with this information
                    tTemplatesToAdd( tNumIntersected ) = Mesh_Modification_Template( tElementsAncestry( 0, 0 ),
                            iE,
                            tNodesInTemplate,
                            tParentEdgeInds,
                            tParentEdgeRanks,
                ...
                else if ( mIntersectConnectivity( iE, 0 ) == 1 )
                {
                    ...
                    Matrix< IndexMat > tEdgeOrdinals = this->get_edge_ordinal_from_element_and_edge_indices( iE, { { tEdge0 } } );
                    ...
                    // get nodes on this (proc indices)
                    Matrix< IndexMat > tNodesInTemplate( 1, 4 );
                    tNodesInTemplate( { 0, 0 }, { 0, 2 } ) = this->get_element_to_node().get_row( iE );
                    tNodesInTemplate( 3 )                  = mNodeInds( tIntersectConnRow( 1 ) );
    ...
    Child_Mesh::create_intersect_connectivity()
    {
        ...
        mIntersectConnectivity        = Matrix< IndexMat >( tNumElements, tNumEdgesToElem * 2 + 1, 0 );    // Needs to be zero for use column

        Matrix< IndexMat > const tEdgeToElem = get_edge_to_element();
        ...
        switch ( aTemplate )
        {
            case TemplateType::HIERARCHY_TET4:
            {
                if ( aIntConnectivity( 0, 0 ) == 3 )
                {
                    ...
                    Matrix< IndexMat > tHigh( { { aIntConnectivity( 0, 1 ), aIntConnectivity( 0, 7 ) } } );
                    Matrix< IndexMat > tMid( { { aIntConnectivity( 0, 1 ), aIntConnectivity( 0, 7 ) } } );
                    Matrix< IndexMat > tLow( { { aIntConnectivity( 0, 1 ), aIntConnectivity( 0, 7 ) } } );
                    ...
                    Matrix< IndexMat > tNodes13 = aEdgeToNodeCMLoc.get_row( tMid( 0, 1 ) );
                    Matrix< IndexMat > tNodes12 = aEdgeToNodeCMLoc.get_row( tLow( 0, 1 ) );
                    Matrix< IndexMat > tNodes14 = aEdgeToNodeCMLoc.get_row( tHigh( 0, 1 ) );

                    // Initialize Edge matrix for the 3 node intersection
                    Matrix< IndexMat > tEdgeIndices( { { tLow( 0, 1 ), tMid( 0, 1 ), tHigh( 0, 1 ) } } );

                    // Get edge ordinals of the edge indices
                    Matrix< IndexMat > tEdgeOrdinals = get_edge_ordinal_from_element_and_edge_indices( aElementIndex, tEdgeIndices );
                ...
                else if ( aIntConnectivity( 0, 0 ) == 4 )
                {
                    ...
                    Matrix< IndexMat > tSortedNodes( 1, 9 );
                    ...
                    Matrix< IndexMat > tRowStorage( 1, 2 );
                    Matrix< IndexMat > tRowSwapper( 1, 2 );
                    ...
                    Matrix< IndexMat > tEdgeToNode = get_edge_to_node_local();

                    Matrix< IndexMat > tNodesL  = tEdgeToNode.get_row( tNodes( 0, 1 ) );
                    Matrix< IndexMat > tNodesML = tEdgeToNode.get_row( tNodes( 1, 1 ) );
                    Matrix< IndexMat > tNodesMH = tEdgeToNode.get_row( tNodes( 2, 1 ) );
                    Matrix< IndexMat > tNodesH  = tEdgeToNode.get_row( tNodes( 3, 1 ) );
                    ...
                    // Initialize Edge matrix for the 3 node intersection
                    Matrix< IndexMat > tEdgeIndices( { { tNodes( 0, 1 ), tNodes( 1, 1 ), tNodes( 2, 1 ), tNodes( 3, 1 ) } } );
    ...
    Child_Mesh::get_intersection_permutation(
            Matrix< IndexMat > const &aOrderedEdgeOrdinals,
            moris::size_t            &aPermutation )
    {
    ...
    Child_Mesh::reindex_template_parent_information( Mesh_Modification_Template &aMeshModTemplate )
    {
        ...
        Matrix< IndexMat >       tParentElem( { { aMeshModTemplate.mParentElemInd } } );
        Matrix< moris::DDSTMat > tParentElemRank( { { 3 } } );

        // Place ptrs in cell to avoid if statement checking rank in for loop
        Vector< Matrix< IndexMat > * > tParentEntitiesInds( { &aMeshModTemplate.mParentNodeInds,
                &aMeshModTemplate.mParentEdgeInds,
                &aMeshModTemplate.mParentFaceInds,
                &tParentElem } );

        Vector< Matrix< moris::DDSTMat > * > tParentEntitiesRanks( { &aMeshModTemplate.mParentNodeRanks,
    ...
    Child_Mesh::add_intersect_conn_node_inheritance()
    {
        ...
        {
            moris::moris_index tEdgeParentInd  = get_entity_parent_entity_proc_ind( moris::mtk::EntityRank::EDGE, mIntersectedEdges( i ) );
            moris::moris_index tEdgeParentRank = get_entity_parent_entity_rank( moris::mtk::EntityRank::EDGE, mIntersectedEdges( i ) );
            ...
            Matrix< IndexMat > const &aElementSubPhase )
    {
        ...
        moris::size_t tNumElements = this->get_num_entities( moris::mtk::EntityRank::ELEMENT );
        ...
        Matrix< moris::DDSTMat > tBinSizeCounter( 1, tNumBins, 0 );
        mSubPhaseBinId                = Matrix< IndexMat >( 1, tNumBins );
        mSubPhaseBinIndices           = Vector< moris::moris_index >( tNumBins );
        mSubPhaseBins                 = Vector< Matrix< IndexMat > >( tNumBins );
        ...
        {
            mSubPhaseBins( i ) = Matrix< IndexMat >( 1, tNumElements );
    ...
    void Child_Mesh::identify_hanging_nodes( const Vector< moris_index > &aTransitionFacetIndices )
    {
        ...
        for ( uint Ik = 0; Ik < aTransitionFacetIndices.size(); Ik++ )
        {
            ...
            {
                if ( ( this->get_entity_parent_entity_rank( moris::mtk::EntityRank::NODE, Ii ) == (moris_index)this->get_facet_rank() ) && ( this->get_entity_parent_entity_proc_ind( moris::mtk::EntityRank::NODE, Ii ) == tFacetIndex ) )
                {
