...
namespace xtk
{
    ...
    Model::decompose(Cell<enum Subdivision_Method> aMethods)
    {
        ...
        if(mDiagnostics)
        {
            ...
            // no access to the isocontour tolerances currently from gen, hardcoded 
            Matrix<DDRMat> tIsocontourTolerance(1,mGeometryEngine->get_num_geometries(),1e-12);
            Matrix<DDRMat> tIsocontourThreshold(1,mGeometryEngine->get_num_geometries(),0.0);
            if(verify_interface_vertices(this,tIsocontourThreshold,tIsocontourTolerance))
            {
                MORIS_LOG_INFO("Interpolated Coordinate Check: Pass");
            }
            else
