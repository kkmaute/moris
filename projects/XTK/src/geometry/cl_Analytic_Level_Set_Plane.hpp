...
public:
    ...
    Real evaluate_field_value_with_coordinate(moris::Matrix< Real_Matrix > const & aCoordinates) const
    {
        return mA*(aCoordinates(0,0)-mXo) + mB*(aCoordinates(0,1)-mYo) + mC*(aCoordinates(0,2) - mZo);
    }


    void  get_plane_normal(moris::Matrix< Real_Matrix > & aPlaneNormal)
    {
    ...
    std::shared_ptr<Matrix_Base<Real, Real_Matrix>> evaluate_sensitivity_dphi_dp_with_coordinate(moris::Matrix< Real_Matrix > const & aCoordinates) const
    {
        return NULL;
    }
