...
namespace moris::xtk
{
            ...
            Integration_Mesh_Generator       *aMeshGenerator )
    {
        ...
        aDecompositionData->tCMNewNodeParamCoord = Vector< Vector< Matrix< DDRMat > > >( tNumChildMeshes );
        ...
        for ( auto &iCell : aMeshGenerationData->mAllIntersectedBgCellInds )
        {
            ...
            for ( moris::uint iV = 0; iV < tNumVerts; iV++ )
            {
                ...
                std::shared_ptr< Matrix< DDRMat > > tVertexLocalCoords = std::make_shared< Matrix< DDRMat > >( tVertexParamCoords.get_row( iV ) );
                    ...
                    if ( tVertexAncestry->get_vertex_parent_rank( iV ) == mtk::EntityRank::ELEMENT )
                    {
                        ...
                        else
                        {
                            MORIS_ASSERT( moris::norm( tNewCoordinate - mDecompData->tNewNodeCoordinate( tNewNodeIndexInSubdivision ) ) < 1e-12,
                                    "Vertex coordinate mismatch, could be a hashing collision, parent rank: ELEMENT , index: %i  hash: %i",
                                    tCell.get_index(),
                                    ( *tVertexHashes )( iV ) );
                        }
                    }
