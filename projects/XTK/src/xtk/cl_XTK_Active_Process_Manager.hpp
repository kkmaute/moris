...
public:
    ...
    mActiveProcTracker(aNumProcessors-1, 2, 0)
    {
        ...
        mActiveInfoToCommunicate = Cell<moris::Mat_New<Integer, Integer_Matrix>>(aNumProcessors-1);

        for( Integer i = 0; i<aNumProcessors-1; i++)
        {
            mActiveInfoToCommunicate(i) = moris::Mat_New<Integer, Integer_Matrix>(0, 0, 0);
        }
    ...
    void set_communication_info(Integer aEntityIndex, Integer aSecondaryTag, Integer aFirstAvailableID, Integer aOtherProcRank)
    {
        ...
        if (mProcTracker(aOtherProcRank, 0) == 0)
        {
            ...
            if (mNumChildren == 1)
            {
                moris::Mat_New<Integer, Integer_Matrix> tCommunicationAllocator(3, tNumInCommListInd + 1, INTEGER_MAX);
                mActiveInfoToCommunicate(mActiveCount) =  tCommunicationAllocator;
            }

            else
            {
                moris::Mat_New<Integer, Integer_Matrix> tCommunicationAllocator(3, (tNumInCommListInd + 1) * mNumChildren, INTEGER_MAX);
                mActiveInfoToCommunicate(mActiveCount) =  tCommunicationAllocator;
            }
...
private:
    ...
    moris::Mat_New<Integer, Integer_Matrix> mProcTracker;
    moris::Mat_New<Integer, Integer_Matrix> mActiveProcTracker;
    Cell<moris::Mat_New<Integer, Integer_Matrix>> mActiveInfoToCommunicate;
