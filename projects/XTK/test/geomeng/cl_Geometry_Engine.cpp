...
// Create an analytical levelset sphere function directly
    ...
    moris::Mat_New<real, Default_Matrix_Real> tTestCoordinate(1, 3);
...
//    std::shared_ptr<Matrix_Base<real,Default_Matrix_Real>> tSensitivityInformation = tLevelsetSphere.evaluate_sensitivity_dx_dp_with_coordinate(tTestCoordinate);
    ...
    moris::Mat_New<real,Default_Matrix_Real> tExpectedSensitivity({{-0.326991257596857, 0.3910309435028875, -0.6575959492214292}, {1, 0, 0}, {0, 1, 0}, {0, 0, 1}});
...
TEST_CASE("Geometry Engine is intersected analytic level set sphere","[GEOMENG][ANALYTIC]")
{
    ...
    // Coordinates
    moris::Mat_New<real, Default_Matrix_Real> tNodeCoordinate(4, 3);
    ...
    moris::Mat_New<size_t, Default_Matrix_Integer> tNodetoElementConnectivity(1, 4);
    ...
    moris::Mat_New<size_t, Default_Matrix_Integer> tNodetoEdgeConnectivity(4, 2);
...
TEST_CASE("Geometry Engine is intersected discrete level set sphere","[GEOMENG][DISCRETE_2D]")
{
    ...
    // Coordinates
    moris::Mat_New<real, Default_Matrix_Real> tNodeCoordinate = tLevelSetMesh.get_level_set_mesh()->get_all_node_coordinates_loc_inds();
    tGeometryEngine.create_geometry_objects_for_background_mesh_nodes(tNodeCoordinate);
...
TEST_CASE("Geometry Engine is intersected discrete level set sphere on a 3d mesh","[GEOMENG][DISCRETE_3D]")
{
    ...
    SECTION("Intersection check with no interface information_ and an analytic level set sphere on a 3d mesh"){
        ...
        moris::Mat_New<real, Default_Matrix_Real> tNodeCoordinate = tLevelSetMeshData->get_all_node_coordinates_loc_inds();
        moris::Mat_New<size_t, Default_Matrix_Integer> tNodetoElemConnectivity(tNumEdge, 8);
        moris::Mat_New<size_t, Default_Matrix_Integer> tNodetoElemConnRow (1, 8);
        // Get level set mesh
...
TEST_CASE("Compute dx/dp","[DxDp]")
{
    ...
    moris::Mat_New<real, Default_Matrix_Real> tDPhiADp(2,3);
    tDPhiADp.fill(10.0);
    tDPhiADp(0,0) = 0.0;
    tDPhiADp(1,0) = 3.0;

    moris::Mat_New<real, Default_Matrix_Real> tDPhiBDp(2,3);
    tDPhiBDp.fill(10.0);
    tDPhiBDp(0,0) = 3.0;
    tDPhiBDp(1,0) = 0.0;

   // Set edge node coordinates
    moris::Mat_New<real, Default_Matrix_Real> tEdgeNodeCoordinates(2,3);
    ...
    // Set edge node coordinates
     moris::Mat_New<real, Default_Matrix_Real> tEdgeNodePhi(2,1);
     (tEdgeNodePhi)(0,0) = -2;  (tEdgeNodePhi)(1,0) = 1;
     moris::Mat_New<real, Default_Matrix_Real> tDxDp(2,3);
