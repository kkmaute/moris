...
namespace xtk
{
  TEST_CASE("Sensitivity Data Structure", "[SENSITIVITY]")
    ...
    moris::Mat_New<size_t,Default_Matrix_Integer> tADVIndicesN1({{1,3}});

    // Sensitivity Data related to node 1
    moris::Mat_New<real, Default_Matrix_Real> tDxDpN1({{10.0,33.0,4.4},{3.0,1.1,2.4}});
    moris::Mat_New<real, Default_Matrix_Real> tDxDpN2({{4.0,3.0,2.4},{1.1,33,40}});
    ...
    moris::Mat_New<size_t,Default_Matrix_Integer> tExpN1Map({{1, 3}, {0, 0}});
    moris::Mat_New<size_t,Default_Matrix_Integer> tExpN2Map({{1, 3}, {1, 1}});
   ...
   TEST_CASE("Finite differencing check on a single tet mesh","[FD_TRI]")
   {
    ...
    Cell<moris::Mat_New<real,Default_Matrix_Real>> tInterfaceNodeLocation(2);

    // computed sensitivity
    moris::Mat_New<real,Default_Matrix_Real> tDxDp(1,1);
    moris::Mat_New<size_t,Default_Matrix_Integer> tDxDpInds(1,1);
    ...
    moris::Mat_New<real,Default_Matrix_Real> tDxDpComp(3,1,10.0);
    ...
    moris::Mat_New<real,Default_Matrix_Real> tDxDpFD(tDxDpFdMat);

    moris::Mat_New<real,Default_Matrix_Real> tDxDpRow = tDxDp.get_row(0);
...
//    CHECK(t2Norm < tTol);
   ...
   TEST_CASE("Finite differencing check on a single hex mesh","[FD_HEX]")
   {
    ...
    moris::Mat_New<real,Default_Matrix_Real> tDxDpRow = tDxDp.get_row(1);
//    real t2Norm = (tDxDpFD.matrix_data()-tDxDpRow.matrix_data()).squaredNorm();
//    CHECK(t2Norm < tTol);
