...
namespace moris::xtk
{
    ...
    {
        aNodeCoordinates = moris::Matrix< DDRMat >( 4, 2 );

        aNodeCoordinates( 0, 0 ) = 0.0;
        aNodeCoordinates( 0, 1 ) = 0.0;
        aNodeCoordinates( 1, 0 ) = 1.0;
        aNodeCoordinates( 1, 1 ) = 0.0;
        aNodeCoordinates( 2, 0 ) = 1.0;
        aNodeCoordinates( 2, 1 ) = 1.0;

        moris::Matrix< DDRMat > tEdgeNodeCoordinates( 1, 2, 75 );
        get_midside_coordinate( tEdge0, aNodeCoordinates, tEdgeNodeCoordinates );
        aNodeCoordinates( 3, 0 ) = tEdgeNodeCoordinates( 0, 0 );
        aNodeCoordinates( 3, 1 ) = tEdgeNodeCoordinates( 0, 1 );
    }
    ...
    setup_node_coordinates( moris::moris_index const &tEdge0,
            ...
            moris::Matrix< DDRMat >                  &aNodeCoordinates )
    {
        aNodeCoordinates = moris::Matrix< DDRMat >( 5, 2 );
    ...
    TEST_CASE( " 2-D Conformal Decomposition Templates ", "[2D_Conf_Temp]" )
    {
        ...
        moris::Matrix< IndexMat >       tNodeIndex( { { 0, 1, 2 } } );
        ...
        moris::Matrix< IndexMat >       tParentEdgeInds( { { 0, 1, 2 } } );
        ...
        for ( size_t iPerm = 0; iPerm < tNumPermutations; iPerm++ )
        {
            ...
            moris::Matrix< IdMat > tElementIds  = tChildMesh.get_element_ids();
            moris::Matrix< IdMat > tElementNode = tChildMesh.get_element_to_node_global();
        }
    }
    ...
    TEST_CASE( " 2-D Conformal Coincident Decomposition Templates ", "[2D_Conf_Coin_Temp]" )
    {
        // base cell information
        moris::Matrix< IndexMat >       tNodeIndex( { { 0, 1, 2 } } );
        moris::Matrix< IdMat >          tNodeIds( { { 1, 2, 3, 4 } } );
        moris::Matrix< IndexMat >       tElementsAncestry( { { 0 } } );    // Not used
        moris::Matrix< moris::DDSTMat > tElementNodeParentRanks( 1, 3, 0 );
        moris::Matrix< IndexMat >       tParentEdgeInds( { { 0, 1, 2 } } );
        moris::Matrix< moris::DDSTMat > tParentEdgeRanks( 1, 3, 1 );
