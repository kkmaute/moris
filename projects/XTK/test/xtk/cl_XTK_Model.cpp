...
TEST_CASE("XFEM TOOLKIT CORE TESTING SERIAL","[XTK][SERIAL]")
{
    ...
    if(tProcSize==1)
    {
        ...
        SECTION("Regular Subdivision Method","[XTK] [REGULAR_SUBDIVISION]"){
            ...
            moris::Matrix<real, Default_Matrix_Real> tNodeCoordinates = tMeshData->get_all_node_coordinates_loc_inds();
            moris::Matrix<real, Default_Matrix_Real> tExpectedNodeCoordinates(
            {{0, 0, 0},
                {1, 0, 0},
                {0, 1, 0},
                {1, 1, 0},
        ...
        SECTION("Regular Subdivision and Nodal Hierarchy Subdivision","[XTK][CONFORMAL]"){
           ...
           {{+0.000000000000000e+00,  +0.000000000000000e+00,  +0.000000000000000e+00},
            ...
            moris::Matrix<real, Default_Matrix_Real> tSingleCoord(1,3);
            moris::Matrix<real, Default_Matrix_Real> tLevelSetValues(1,tNumNodesAfterDecompositionXTK);
            ...
            moris::Matrix<real, Default_Matrix_Real> tExpectedLevelSetValues({{1.9375, 0.9375, 0.9375, -0.0625, 2.9375, 1.9375, 1.9375, 0.9375, 5.9375, 4.9375, 4.9375, 3.9375, 1.4375, 0.4375, 0.4375, 1.4375, 0.4375, 1.4375, 0.6875, -0.05859375, -0.05859375, -0.05859375}});
        ...
        for(size_t iX = 0; iX<tXIts; iX++)
        {
            ...
            for(size_t iY = 0; iY<tYIts; iY++)
            {
                ...
                for(size_t iZ = 0; iZ<tZIts; iZ++)
                {
                    ...
                    moris::Matrix<size_t,Default_Matrix_Integer> tElementFaces = tMeshData->get_entity_connected_to_entity_loc_inds(0, EntityRank::ELEMENT, EntityRank::FACE);
                    moris::Matrix<size_t,Default_Matrix_Integer> tElementEdges = tMeshData->get_entity_connected_to_entity_loc_inds(0, EntityRank::ELEMENT, EntityRank::EDGE);
