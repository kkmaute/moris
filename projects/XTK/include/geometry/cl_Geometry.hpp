...
public:
     ...
     */
    virtual moris::Matrix<Real,Real_Matrix> evaluate_sensitivity_dphi_dp_with_coordinate(Integer const & aRowIndex,
                                                                                moris::Matrix<Real,Real_Matrix> const & aCoordinates) const
    {
     ...
     *  NOTE: THIS CURRENTLY REQUIRES THE BACKGROUND MESH AND GEOMETRY ARE COINCIDENT (NEEDS AN OBJECT THAT MAPS BETWEEN THE TWO)
     */
    virtual moris::Matrix<Real,Real_Matrix> evaluate_sensitivity_dphi_dp(moris::Matrix<Real,Real_Matrix> const & aLocalCoordinate, Integer aEntityIndex, enum EntityRank aEntityRank)
    {
//        XTK_ERROR<<" evaluate_sensitivity_dx_dp not implemented. This could be due to a geometry not being based on a mesh.";
        return moris::Matrix<Real,Real_Matrix>(1,1,0);
    }

    /*
     * Returns the advs that the provided node indices are dependent on
     */
     virtual moris::Matrix<Integer, Integer_Matrix> get_node_adv_indices(moris::Matrix<Integer, Integer_Matrix> const & aNodeIndices)
    {
         XTK_ERROR<<" get_node_adv_indices not implemented, This could be due to a geometry not being based on a mesh.";
         return moris::Matrix<Integer, Integer_Matrix>(1,1,0);
    }
