...
#if ( defined _MSC_VER && _MSC_VER > 1400 && !defined __EDGE__ ) || ( defined __WAVE__ && __WAVE_HAS_VARIADICS ) || ( defined __GNUC__ && __GNUC__ >= 3 ) || ( !defined __cplusplus && __STDC_VERSION__ >= 199901L || __cplusplus >= 201103L )
...
#else
#define CATCH_BREAK_INTO_DEBUGGER()                        \
    if ( Catch::isDebuggerActive() ) { __asm__( "int $3\n" \
                                                :          \
                                                : ); }
#endif
#endif
...
#define CATCH_BREAK_INTO_DEBUGGER() \
    if ( Catch::isDebuggerActive() ) { __debugbreak(); }
...
namespace Clara
{
    ...
    namespace Detail
    {
        ...
        {
            BoundUnaryMethod( void ( C::*_member )( M ) )
                    : member( _member )
            {
            }
            virtual void set( C& p, std::string const & stringValue ) const
        ...
        {
            BoundNullaryMethod( void ( C::*_member )() )
                    : member( _member )
            {
            }
            virtual void set( C& p, std::string const & stringValue ) const
        ...
        {
            BoundUnaryFunction( void ( *_function )( C& ) )
                    : function( _function )
            {
            }
        ...
        {
            BoundBinaryFunction( void ( *_function )( C&, T ) )
                    : function( _function )
            {
            }
            virtual void set( C& obj, std::string const & stringValue ) const
    ...
    class CommandLine
    {
      ...
      public:
        ...
        void optUsage( std::ostream& os, std::size_t indent = 0, std::size_t width = Detail::consoleWidth ) const
        {
            ...
            for ( it = itBegin; it != itEnd; ++it )
            {
                Detail::Text usage( it->commands(), Detail::TextAttributes().setWidth( maxWidth + indent ).setIndent( indent ) );
        ...
        std::vector< Parser::Token > populateOptions( std::vector< Parser::Token > const & tokens, ConfigT& config ) const
        {
            ...
            for ( std::size_t i = 0; i < tokens.size(); ++i )
            {
                ...
                for ( ; it != itEnd; ++it )
                {
                    ...
                    try
                    {
                        if ( ( token.type == Parser::Token::ShortOpt && arg.hasShortName( token.data ) ) || ( token.type == Parser::Token::LongOpt && arg.hasLongName( token.data ) ) )
                        {
                            if ( arg.takesArg() )
                            {
...
namespace Catch
{
    ...
    inline std::size_t listTags( Config const & config )
    {
                ...
                ++countIt )
        {
            ...
            Text wrapper( countIt->second.all(), TextAttributes().setInitialIndent( 0 ).setIndent( oss.str().size() ).setWidth( CATCH_CONFIG_CONSOLE_WIDTH - 10 ) );
    ...
    inline std::size_t listReporters( Config const & /*config*/ )
    {
        ...
        for ( it = itBegin; it != itEnd; ++it )
        {
            Text wrapper( it->second->getDescription(), TextAttributes().setInitialIndent( 0 ).setIndent( 7 + maxNameLen ).setWidth( CATCH_CONFIG_CONSOLE_WIDTH - maxNameLen - 8 ) );
            std::cout << "  "
                      << it->first
                      << ":"
...
namespace Catch
{
    ...
    Colour::Colour( Colour const & _other )
            : m_moved( false )
    {
...
namespace Catch
{
    ...
    std::string ResultBuilder::reconstructExpression() const
    {
        ...
        {
            if ( m_exprComponents.lhs.size() + m_exprComponents.rhs.size() < 40 && m_exprComponents.lhs.find( "\n" ) == std::string::npos && m_exprComponents.rhs.find( "\n" ) == std::string::npos )
