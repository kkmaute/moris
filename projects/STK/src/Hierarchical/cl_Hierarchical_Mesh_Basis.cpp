...
uint
Hierarchical_Mesh_Basis::give_number_of_basis(
        uint const & aLevel,
        ...
        Mat<uint> const & aNumberOfElementsPerDirection)
{
    ...
    if( aModelDim == 1 )
    {
        ...
        {
            tBasisNumber += ( pow( 2, i ) * aNumberOfElementsPerDirection( 0 ) + aPolynomial );
        }
    }
    else if( aModelDim == 2 )
    {
        ...
        {
            tBasisNumber += ( pow( 2, i ) * aNumberOfElementsPerDirection( 0 ) + aPolynomial )
                                            * ( pow( 2, i ) * aNumberOfElementsPerDirection( 1 ) + aPolynomial );
        }
    }
    else if( aModelDim == 3 )
    {
        for( uint i = 0; i < aLevel + 1 ; i++ )
        {
            tBasisNumber += ( pow( 2, i ) * aNumberOfElementsPerDirection( 0 ) + aPolynomial )
                                            * ( pow( 2, i ) * aNumberOfElementsPerDirection( 1 ) + aPolynomial )
                                            * ( pow( 2, i ) * aNumberOfElementsPerDirection( 2 ) + aPolynomial );
        }
    }
    return tBasisNumber;
}
...
Mat<uint>
Hierarchical_Mesh_Basis::give_neighbor_of_basis(
        uint const & aBasisId,
        uint const & aModelDim,
        uint const & aPolynomial,
        uint const & aBuffer,
...
/* @TODO give_basis_level: clean formulas */
uint
Hierarchical_Mesh_Basis::give_basis_level(
        uint const & aBasisId,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection) const
{
    ...
    uint tPowLevel = 1; //pow( 2, tBasisLevel - 1 );
    if( aModelDim == 1 )
    {
        // temporary variable for the number of basis per level
        uint tNumberOfBasis = ( tPowLevel * aNumberOfElementsPerDirection( 0 ) + aPolynomial );
        while(tLevel>0)
        {
            ...
            else
            {
                ...
                tNumberOfBasis += ( tPowLevel * aNumberOfElementsPerDirection( 0 ) + aPolynomial);
            }
        }
        tBasisLevel--;
    }
    else if( aModelDim == 2 )
    {
        // temporary variable for the number of basis per level
        uint tNumberOfBasis = ( tPowLevel * aNumberOfElementsPerDirection( 0 ) + aPolynomial )
                                        * ( tPowLevel * aNumberOfElementsPerDirection( 1 ) + aPolynomial );
        while( tLevel > 0 )
        {
            // Check if it fits in the level
            tLevel = floor( (real)( aBasisId + 1 ) / tNumberOfBasis );
    ...
    else if( aModelDim == 3 )
    {
        ...
        uint tNumberOfBasis = ( tPowLevel * aNumberOfElementsPerDirection( 0 ) + aPolynomial)
                                        * ( tPowLevel * aNumberOfElementsPerDirection( 1 ) + aPolynomial )
                                        * ( tPowLevel * aNumberOfElementsPerDirection( 2 ) + aPolynomial );
        while( tLevel > 0 )
        {
            // Check if it fits in the level
            tLevel = floor( (real)( aBasisId + 1 ) / tNumberOfBasis );
...
/* @TODO give_position_of_basis: clean formulas */
Mat<uint>
Hierarchical_Mesh_Basis::give_position_of_basis(
        uint const & aBasisId,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection) const
{
    ...
    if( aModelDim == 2 )
    {
        ...
        tBasisPosition( 1 ) = ceil( (aBasisId + 1 - tNumberOfBasis )
                / ( ( pow( 2, tBasisLevel ) * aNumberOfElementsPerDirection( 0 ) + aPolynomial ) ) );
        // Calculate the x-direction with the calculated y-position
        tBasisPosition( 0 ) = aBasisId +1 - tNumberOfBasis
                + ( pow( 2, tBasisLevel ) * aNumberOfElementsPerDirection( 0 ) + aPolynomial )
                - tBasisPosition( 1 ) * ( pow( 2, tBasisLevel ) * aNumberOfElementsPerDirection( 0 ) + aPolynomial );
    ...
    else if( aModelDim == 3 )
    {
        ...
        uint tConst0 = tPow2*aNumberOfElementsPerDirection( 0 ) + aPolynomial;
        uint tConst1 = tPow2*aNumberOfElementsPerDirection( 1 ) + aPolynomial;
        // Calculate the position in z-direction
        tBasisPosition( 2 ) = ceil( ( aBasisId + 1 - tNumberOfBasis ) / ( (real)( tConst0 * tConst1 ) ) );
        // Calculate the y-direction with the calculated z-position
        tBasisPosition( 1 ) = ceil( ( aBasisId + 1 - tNumberOfBasis - ( tBasisPosition( 2 ) - 1 ) * tConst0 * tConst1 ) / (real)tConst0 );
...
uint
Hierarchical_Mesh_Basis::give_basis_of_position(
        uint const & aLevel,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection,
        Mat<uint> const & aIJKPosition) const
{
    ...
    {
        tY_Position = aIJKPosition( 1 )*( aNumberOfElementsPerDirection( 0 ) + aPolynomial );
        if( aLevel>0 )
        {
            //Give number of basis functions for the last level to have the initial number at position 0,0
            tBasisId = give_number_of_basis( aLevel-1, aModelDim, aPolynomial, aNumberOfElementsPerDirection );
            // Position of the Element in y-direction for a specific level
            tY_Position = aIJKPosition( 1 ) * ( aPolynomial + aNumberOfElementsPerDirection( 0 ) * pow( 2, aLevel ) );
        }
    ...
    {
        tY_Position = aIJKPosition( 1 ) * ( aPolynomial + aNumberOfElementsPerDirection( 0 ) )
                           + aIJKPosition( 2 ) * ( aPolynomial + aNumberOfElementsPerDirection( 0 ) )
                           * ( aPolynomial + aNumberOfElementsPerDirection( 1 ) );
        if( aLevel > 0 )
        {
            ...
            // Position of the Element in y-direction for a specific level
            tY_Position = aIJKPosition( 1 ) * ( aPolynomial + aNumberOfElementsPerDirection( 0 ) * pow( 2, aLevel ) )
                                            + aIJKPosition( 2 ) * ( aPolynomial + aNumberOfElementsPerDirection( 0 ) *pow( 2, aLevel ) )
                                            * ( aPolynomial + aNumberOfElementsPerDirection( 1 ) * pow( 2, aLevel ) );
        }
...
uint
Hierarchical_Mesh_Basis::give_basis_of_parent(
        uint const & aBasis,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection) const
{
    MORIS_ASSERT(aPolynomial == 1, "Parent of a basis function can only determined for a linear approach");
...
Mat<uint>
Hierarchical_Mesh_Basis::give_element_of_basis(
        uint const & aBasis,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection) const
{
    ...
    uint tBasisLevel = give_basis_level( aBasis, aModelDim, aPolynomial, aNumberOfElementsPerDirection );
    // Compute Position of basis function
    Mat<uint> tBasisPosition = give_position_of_basis( aBasis, aModelDim, aPolynomial, aNumberOfElementsPerDirection );
    // Elements, which have support with the basis function aBasis
    Mat<uint> tElements( pow( ( aPolynomial + 1 ), aModelDim ) , 1 , UINT_MAX );
    ...
    int tDomainRangeCheck0 = (int)aNumberOfElementsPerDirection( 0 ) * tPowVar;
    if( aModelDim == 1 )
    {
        // Elements are hardcoded for aPolynomial = 1 & 2. aPolynomial > 2 is generated by a loop
        if( aPolynomial == 1 )
        {
            if( (int)( tBasisPosition( 0 ) ) >= 0
                    && (int)( tBasisPosition( 1 ) ) >= 0
                    && (int)( tBasisPosition( 0 ) ) < tDomainRangeCheck0 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 );
                tElements( 1 ) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)( tBasisPosition( 0 ) - 1 ) >= 0
                    && (int)( tBasisPosition( 1 ) ) >= 0
                    && (int)( tBasisPosition( 0 ) - 1 ) < tDomainRangeCheck0 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 )-1;
                tElementPosition( 1 ) = tBasisPosition( 1 );
                tElements( 0 ) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
        }
        if( aPolynomial == 2 )
        {
                    ...
                    && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 );
                tElements( 2 ) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )-1) >= 0
                    && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 0 )-1) < tDomainRangeCheck0 )
            ...
            if( (int)(tBasisPosition( 0 )-2) >= 0
                    && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 0 )-2) < tDomainRangeCheck0 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 )-2;
                tElementPosition( 1 ) = tBasisPosition( 1 );
                tElements( 0 ) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
        }
        ...
        {
            // The elements will be calculated, if higher order polynomials are needed
            // Temporary variable for the for loops
            uint tVar=0;
            for( int i=-aPolynomial; i < 1; i++ )
            {
                if( (int)(tBasisPosition( 0 ) + i ) >= 0
                        && (int)(tBasisPosition( 0 ) + i ) < tDomainRangeCheck0 )
                {
                    tElementPosition( 0 ) = tBasisPosition( 0 ) + i;
    ...
    {
        //Temporary variable for if clause conditions, it is the maximum number of elements in a specific direction
        int tDomainRangeCheck1 = (int)aNumberOfElementsPerDirection( 1 ) * tPowVar;
        // Elements are hardcoded for aPolynomial = 1 & 2. aPolynomial > 2 is generated by a loop
        if( aPolynomial == 1 )
        {
            if( (int)(tBasisPosition( 0 )) >= 0
                    && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 );
                tElements( 3 ) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 ) - 1) >= 0  && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 0 ) - 1 ) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 1;
            ...
            if((int)(tBasisPosition( 0 )) >= 0 && (int)(tBasisPosition( 1 )-1) >= 0
                    && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )-1) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 ) - 1;
                tElements( 1 ) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if((int)(tBasisPosition( 0 )-1)>=0 && (int)(tBasisPosition( 1 )-1)>=0
                    && (int)(tBasisPosition( 0 )-1) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )-1) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 1;
        ...
        if( aPolynomial == 2 )
        {
            if( (int)(tBasisPosition( 0 )) >= 0 && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 );
                (tElements( 8 ))=mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )-1) >= 0 && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 0 ) - 1) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 1;
                tElementPosition( 1 ) = tBasisPosition( 1 );
                (tElements( 7 ))=mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )-2) >= 0 && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 0 ) - 2) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 2;
                tElementPosition( 1 ) = tBasisPosition( 1 );
                (tElements( 6 ))=mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )) >= 0 && (int)(tBasisPosition( 1 ) - 1)>=0
                    && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 ) - 1) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 ) - 1;
                (tElements( 5 ))=mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )-1) >= 0 && (int)(tBasisPosition( 1 )-1) >= 0
                    && (int)(tBasisPosition( 0 ) - 1) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 ) - 1) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 1;
                tElementPosition( 1 ) = tBasisPosition( 1 ) - 1;
                (tElements( 4 ))=mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )-2) >= 0 && (int)(tBasisPosition( 1 ) - 1) >= 0
                    && (int)(tBasisPosition( 0 ) - 2) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 ) - 1) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 2;
            ...
            if( (int)(tBasisPosition( 0 )) >= 0 && (int)(tBasisPosition( 1 ) - 2) >= 0
                    && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 ) - 2) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 ) - 2;
                (tElements( 2 ))=mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 ) - 1) >= 0 && (int)(tBasisPosition( 1 ) - 2) >= 0
                    && (int)(tBasisPosition( 0 ) - 1) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 ) - 2) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 1;
            ...
            if( (int)(tBasisPosition( 0 ) - 2) >= 0 && (int)(tBasisPosition( 1 ) - 2) >= 0
                    && (int)(tBasisPosition( 0 ) - 2) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 ) - 2) < tDomainRangeCheck1 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 2;
            ...
            for( int j = -aPolynomial; j < 1; j++ )
            {
                for( int i = -aPolynomial; i < 1; i++ )
                {
                    if( (int)(tBasisPosition( 0 ) + i) >= 0 && (int)(tBasisPosition( 1 ) + j) >= 0
                            && (int)(tBasisPosition( 0 ) + i) < tDomainRangeCheck0
                            && (int)(tBasisPosition( 1 ) + j) < tDomainRangeCheck1 )
                    {
                        tElementPosition( 0 ) = tBasisPosition( 0 ) + i;
                        tElementPosition( 1 ) = tBasisPosition( 1 ) + j;
                        (tElements(tVar)) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
                        tVar++;
                    }
                }
    ...
    else if( aModelDim == 3 )
    {
        //Temporary variable for if clause conditions, it is the maximum number of elements in a specific direction
        int tDomainRangeCheck1 = (int)aNumberOfElementsPerDirection( 1 ) * tPowVar;
        int tDomainRangeCheck2 = (int)aNumberOfElementsPerDirection( 2 ) * tPowVar;
        // Elements are hardcoded for aPolynomial = 1. aPolynomial > 1 is generated by a loop
        if( aPolynomial == 1 )
        {
            if( (int)(tBasisPosition( 0 )) >= 0 && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 2 )) >= 0 && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1
                    && (int)(tBasisPosition( 2 )) < tDomainRangeCheck2 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 );
                tElementPosition( 2 ) = tBasisPosition( 2 );
                (tElements( 7 )) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )-1) >= 0 && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 2 )) >= 0 && (int)(tBasisPosition( 0 )-1) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1
                    && (int)(tBasisPosition( 2 )) < tDomainRangeCheck2 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 1;
            ...
            if( (int)(tBasisPosition( 0 )) >= 0 && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 2 ) - 1) >= 0 && (int)(tBasisPosition( 0 )) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1
                    && (int)(tBasisPosition( 2 ) - 1) < tDomainRangeCheck2 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 );
                tElementPosition( 1 ) = tBasisPosition( 1 );
                tElementPosition( 2 ) = tBasisPosition( 2 ) - 1;
                (tElements( 3 )) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
            }
            if( (int)(tBasisPosition( 0 )-1) >= 0 && (int)(tBasisPosition( 1 )) >= 0
                    && (int)(tBasisPosition( 2 ) - 1) >= 0 && (int)(tBasisPosition( 0 ) - 1) < tDomainRangeCheck0
                    && (int)(tBasisPosition( 1 )) < tDomainRangeCheck1
                    && (int)(tBasisPosition( 2 ) - 1) < tDomainRangeCheck2 )
            {
                tElementPosition( 0 ) = tBasisPosition( 0 ) - 1;
        ...
        else if( aPolynomial > 1) // The elements will be calculated, if higher order polynomials are needed
        {
                ...
                for(int j=-aPolynomial; j<1; j++)
                {
                    for(int i=-aPolynomial; i<1; i++)
                    {
                        if((int)(tBasisPosition( 0 ) + i) >= 0 && (int)(tBasisPosition( 1 ) + j) >= 0
                                ...
                                && (int)(tBasisPosition( 0 ) + i) < tDomainRangeCheck0
                                && (int)(tBasisPosition( 1 ) + j) < tDomainRangeCheck1
                                && (int)(tBasisPosition( 2 ) + k) < tDomainRangeCheck2 )
                        {
                            tElementPosition( 0 ) = tBasisPosition( 0 ) + i;
                            tElementPosition( 1 ) = tBasisPosition( 1 ) + j;
                            tElementPosition( 2 ) = tBasisPosition( 2 ) + k;
                            (tElements(tVar)) = mBaseElement.give_element_of_position( tBasisLevel, aModelDim, aNumberOfElementsPerDirection, tElementPosition );
                            tVar++;
                        }
                    }
...
Mat<real>
Hierarchical_Mesh_Basis::give_coordinate_from_basis(
        uint const & aBasisId,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection,
        ...
        Mat<real> const & aDomainRangeOffset) const
{
    ...
    real tConst = 0.5 * (aPolynomial-1);
    for( uint k = 0; k < aModelDim; k++ )
    {
        tCoordinates(k) = aDomainRangeOffset(k) + (tBasisPosition(k) - tConst )
                                        * aDomainRange(k) / ( tPow2Level * aNumberOfElementsPerDirection(k) );
    }
    return tCoordinates;
}
...
uint
Hierarchical_Mesh_Basis::give_basis_proc_owner(
        uint const & aBasisId,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection,
...
Mat<uint>
Hierarchical_Mesh_Basis::give_basis_share(
        uint const & aBasis,
        uint const & aModelDim,
        uint const & aPolynomial,
        Mat<uint> const & aNumberOfElementsPerDirection,
        ...
        Mat<uint> const& aDecomp) const
{
    ...
    Mat<uint> tBasisPosition( aModelDim , 1 , 0 );
    uint tVar = 0;
    // Arbitrary number for possible sharing elements ( It will find with the algorithm multiple times the same neighbors and unique will kick them out)
...
uint
Hierarchical_Mesh_Basis::give_symmetry_master_of_basis(
        const uint      & aBasisId,
        ...
        const Mat<uint> & aNumberOfBasisPerDirection ) const
{
    ...
    Mat< uint > tIJK = give_position_of_basis(
            aBasisId,
            aModelDim,
            aPolynomial,
    ...
    if ( tIJK( aSymmetryPlane ) >= tScale*aSymmetryIndex )
    {
        // reflect position at defined plane
        tIJK( aSymmetryPlane ) =  tScale*
                aNumberOfBasisPerDirection( aSymmetryPlane )
                - tIJK( aSymmetryPlane );

        // return ID OF reflected basis
        return give_basis_of_position(
                tLevel,
                aModelDim,
                aPolynomial,
                aNumberOfElementsPerDirection,
...
uint
Hierarchical_Mesh_Basis::give_symmetry_slave_of_basis(
        const uint      & aBasisId,
        const uint      & aModelDim,
        const uint      & aPolynomial,
        const uint      & aSymmetryPlane,
...
bool
Hierarchical_Mesh_Basis::basis_is_symmetry_master(
        const uint      & aBasisId,
        const uint      & aModelDim,
        const uint      & aPolynomial,
        const uint      & aSymmetryPlane,
        ...
        const Mat<uint> & aNumberOfElementsPerDirection ) const
{
    ...
    // calculate scaling factor of basis
    uint tScale = pow( 2, tLevel );
    ...
    return  tIJK( aSymmetryPlane ) <= tScale*aSymmetryIndex;
}
