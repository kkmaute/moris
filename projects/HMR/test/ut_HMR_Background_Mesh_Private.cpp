    ...
    TEST_CASE( "HMR_Background_Mesh_Private", "[moris],[mesh],[hmr][Background_Mesh_private]" )
    {
        ...
        if ( par_size() == 1 || par_size() == 2 || par_size() == 4 )
        {
            ...
            SECTION( "Background mesh 2D: test calc_child_index() function" )
            {
                ...
                Matrix< DDUMat > tInputTree( gMaxNumberOfLevels - 1, 1 );
                uint tChildIndex = 0;
                ...
                for ( uint l = 0; l < gMaxNumberOfLevels - 1; ++l )
                {
                    ...
                    const luint* tIJ = tElement->get_ijk();
            ...
            SECTION( "Background mesh 3D: test calc_child_index() function" )
            {
                ...
                for ( uint l = 0; l < gMaxNumberOfLevels - 1; ++l )
                {
                    ...
                    const luint* tIJK = tElement->get_ijk();
            ...
            SECTION( "Background mesh 2D: test neighborhood calculation" )
            {
                ...
                for ( luint k = 0; k < tElements.length(); ++k )
                {
                    // pick an element
                    Background_Element_Base* tElement = tBackgroundMesh->get_element( tElements( k ) );

                    // get ijk position of element
                    const luint* tElIJ = tElement->get_ijk();

                    // calculate ij positions for neighbor elements
                    Matrix< DDLUMat > tIJ( 2, 8 );
                    ...
                    for ( uint e = 0; e < 8; ++e )
                    {
                        ...
                        Background_Element_Base* tNeighbor = tElement->get_neighbor( e );

                        // calculate expected id of element
                        luint tDomainID = tBackgroundMesh->calc_domain_id_of_element( tLevel, tIJ( 0, e ),
                                                                                             tIJ( 1, e ) );
            ...
            SECTION( "Background mesh 3D: test neighborhood calculation" )
            {
                ...
                Background_Element_Base* tElement = tBackgroundMesh->get_element_from_proc_domain_including_aura(0 );
                ...
                for ( luint k = 0; k < tNumberOfElementsOnProc; ++k )
                {
                ...
                for ( uint e = 0; e < 26; ++e )
                {
                    ...
                    // calculate expected id of element
                    luint tDomainID = tBackgroundMesh->calc_domain_id_of_element( tLevel, tIJK( 0, e ),
                                                                                         tIJK( 1, e ), tIJK( 2, e ) );
        ...
        if ( par_size() == 1 )
        {
            ...
            SECTION( "Background mesh 2D: test get_neighbors_from_same_level() function" )
            {
                ...
                luint tNumberOfActiveElements = tBackgroundMesh->get_number_of_active_elements_on_proc();
                ...
                for ( uint tOrder = 1; tOrder <= 2; ++tOrder )
                {
                    ...
                    luint tNumberOfNeighbors = std::pow( 2 * tOrder + 1,
                                                                tParameters->get_number_of_dimensions() ) - 1;
                    ...
                    Matrix< DDLUMat > tExpectedIDs( tNumberOfNeighbors, 1 );
                    ...
                    // sort
                    Matrix< DDLUMat > tExpectedUniqueIDs;
                    unique( tExpectedIDs, tExpectedUniqueIDs );

                    Matrix< DDLUMat > tCalculatedIDs( tNumberOfNeighbors, 1 );
                    ...
                    // make result unique
                    Matrix< DDLUMat > tCalculatedUniqueIDs;
                    unique( tCalculatedIDs, tCalculatedUniqueIDs );
            ...
            SECTION( "Background mesh 3D: test get_neighbors_from_same_level() function" )
            {
                ...
                for ( uint tOrder = 1; tOrder <= 2; ++tOrder )
                {
                    ...
                    // sort
                    Matrix< DDLUMat > tUniqueIDs;
                    unique( tExpectedIDs, tUniqueIDs );
