...
//-------------------------------------------------------------------------------
        ...
        SECTION( "Background mesh 2D: test calc_child_index() function")
        {
            ...
            moris::Mat<moris::luint> tNumberOfElements = { {6}, {4} };
            ...
            moris::Mat< moris::uint > tInputTree( moris::hmr::gMaxNumberOfLevels-1, 1 );
     ...
     SECTION( "Background mesh 3D: test calc_child_index() function")
        {
            ...
            // set number of elements
            moris::Mat<moris::luint> tNumberOfElements = { {4}, {6}, {4} };
            tParameters->set_number_of_elements_per_dimension( tNumberOfElements );
       ...
       SECTION( "Background mesh 2D: test neighborhood calculation")
        {
            ...
            // in this matrix
            moris::Mat< moris::luint > tElements;
       ...
       SECTION( "Background mesh 3D: test neighborhood calculation")
        {
                ...
                // set number of elements
                moris::Mat<moris::luint> tNumberOfElements = { {4}, {2}, {3} };
                ...
                moris::Mat<moris::luint> tIJK( 3, 26 );
...
//-------------------------------------------------------------------------------
        ...
        SECTION( "Background mesh 2D: test get_neighbors_from_same_level() function")
        {
            ...
            for( uint tOrder=1; tOrder<=2; ++tOrder )
            {
                ...
                moris::Mat< moris::luint > tExpectedIDs( tNumberOfNeighbors, 1 );
                ...
                // sort
                tExpectedIDs = moris::unique( tExpectedIDs );

                moris::Mat< moris::luint > tCalculatedIDs( tNumberOfNeighbors, 1 );
                ...
                // make result unique
                tCalculatedIDs = moris::unique( tCalculatedIDs );

                // make sure that IDs are correct
                bool tOK = true;
                for( moris::luint k=0; k<tNumberOfNeighbors; ++k )
                {
                    tOK = tOK &&  tCalculatedIDs( k ) == tExpectedIDs( k );
                }
