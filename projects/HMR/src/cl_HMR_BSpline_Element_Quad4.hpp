...
namespace moris::hmr
{
    ...
    template<>
    mtk::Geometry_Type
    BSpline_Element< 1, 1, 0 >::get_geometry_type() const
    {
    ...
    template<>
    void
    BSpline_Element< 1, 1, 0 >::get_ijk_of_basis_function(
            uint   aBasisNumber,
            luint* aIJK )
    {
     ...
     */
    template<>
    luint BSpline_Element< 1, 1, 0 >::refine_basis( uint aBasisNumber )
    {
        // Start basis counter
                    ...
                    for ( luint i = tIMin; i < tIMax; ++i )
                    {
                        ...
                        if ( tBasis->get_child( tChildIndex ) == nullptr )
                        {
                            ...
                            tBasis->insert_child( tChildIndex,
                                    new BSpline< 1, 1, 0 >( tIJ, tLevel, gNoProcOwner ) );
     ...
     */
    template<>
    luint BSpline_Element< 1, 1, 0 >::refine( moris::Cell< Element* >& aAllElementsOnProc )
    {
        // Start basis counter
        luint tBasisCounter = 0;
        ...
        Basis_Function* tBasis[ 9 ] = { nullptr };
