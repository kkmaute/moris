...
namespace moris
{
            ...
            , m_parameter( a_param )
    {
        ...
        if ( m_parameter.get_value< std::string >().empty() )
        {
            ...
            // std::shared_ptr< fem::Constitutive_Model > tCM  = tCMFactory.create_CM( fem::Constitutive_Type::STRUC_LIN_ISO );
            ...
            for ( const auto &iMap : tMap )
            {
                ...
                mRows[ tKey ] = tLineEdit;

                connect( mRows[ tKey ], &QLineEdit::textChanged, this, &Moris_Group_Box::on_line_edit_text_changed );
