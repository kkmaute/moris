    ...
    void TestWindow::saveInputsToXML( const std::string &filePath )
    {
        ...
        for ( auto paramEntry : parameterList )
        {
            ...
            else if ( paramName == "pairBox" )
            {
                ...
                //     pairValue.get_name()  = pairBox->moris_pair_combo_box->currentText().toStdString();
                //     pairValue.get_parameter() = pairBox->moris_pair_line_edit->text().toStdString();
                //     xmlParser.set( "UserInputs.PairBox.LineEdit", pairValue.get_parameter() );
                //     xmlParser.set( "UserInputs.PairBox.ComboBox", pairValue.get_name() );
                // }
            }
        }
