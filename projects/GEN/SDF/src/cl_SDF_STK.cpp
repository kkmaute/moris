...
namespace moris::sdf
{
            ...
            const double                aTimeStep )
    {
        ...
        uint tNumberOfNodesPerElement = tMesh->get_mtk_cell( 0 ).get_number_of_vertices();
        ...
        if ( mMesh.is_verbose() )
        {
            ...
            // print output
            std::fprintf( stdout, "%s Created MTK output object.\n               Mesh has %lu elements and %lu nodes.\n               Creation took %5.3f seconds.\n\n", proc_string().c_str(), (long unsigned int)tNumberOfElements, (long unsigned int)tNumberOfNodes, (double)tElapsedTime / 1000 );
        }
    }
