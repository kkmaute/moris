...
//     MORIS_ASSERT( aPoint.numel() == aSurfaceMesh.get_spatial_dimension(),
//             "fn_SDF_Raycast: get_region_from_raycast() - Dimension mismatch. Point dimension = %lu\tObject Dimension = %d",
//             aPoint.numel(),
//             aSurfaceMesh.get_spatial_dimension() );

//     // flag that marks if rotation was called
//     bool tRotation = false;
//     while ( tPointIsInside == UNSURE )
//     {
//         tPointIsInside = voxelize( aSurfaceMesh, aPoint, 0 );

//         if ( tPointIsInside == UNSURE )
//         {
//             tPointIsInside = voxelize( aSurfaceMesh, aPoint, 1 );
//             if ( aSurfaceMesh.get_spatial_dimension() == 3 and tPointIsInside == UNSURE )
//             {
//                 tPointIsInside = voxelize( aSurfaceMesh, aPoint, 2 );
//             }
...
//                 tRotation = true;
//                 random_rotation( aSurfaceMesh, aPoint );
//             }
//         }
//     }
...
//     if ( tRotation )
//     {
//         aSurfaceMesh.reset_coordinates();
//     }
...
// {
//     MORIS_ASSERT( aSurfaceMesh.get_spatial_dimension() == aPoint.numel(),
...
//     switch ( aSurfaceMesh.get_spatial_dimension() )
//     {
//         case 2:
//         {
//             // preselect lines in the aAxis direction
//             // NOTE: tCertainIntersectedFacets are intersected lines whose bounding box does not contain the cast point,
//             // aIntersectedFacets are lines whose bounding box contains the cast point
//             Vector< uint > tCertainIntersectedFacets;
//             tPreselection = preselect_lines( aSurfaceMesh, aPoint, aAxis, tCertainIntersectedFacets, aIntersectedFacets );

//             // append the candidates to the intersected facets to compute intersection
//             aIntersectedFacets.insert( aIntersectedFacets.size(), tCertainIntersectedFacets.begin(), tCertainIntersectedFacets.end() );

//             // compute intersection locations
//             tIntersectionCoordinates = intersect_ray_with_facets( aSurfaceMesh, aIntersectedFacets, aPoint, aAxis, tPreselection, true );
...
//             tIntersectionCoordinates = intersect_triangles_moller_trumbore( tCandidateFacets, aSurfaceMesh, aPoint, aAxis, aIntersectedFacets );
//             break;
//         }
//         default:
//         {
//             MORIS_ERROR( false, "SDF-Raycast: Unsupported dimension of %d provided.", aSurfaceMesh.get_spatial_dimension() );
//             return {};
//         }
//     }
...
//     real tIntersectionTolerance = aSurfaceMesh.get_intersection_tolerance();
...
//             Vector< uint >      tCandidateFacets;
//             Preselection_Result tPreselection = preselect_lines( aSurfaceMesh, aPoint, aAxis, tIntersectedFacets, tCandidateFacets );
...
//                     moris::Vector< real > tIntersectionCoords = intersect_ray_with_facets( aSurfaceMesh, tCandidateFacets, aPoint, aAxis, tPreselection );

//                     // check if the node is inside the polygon
//                     return check_if_node_is_inside_lines( aSurfaceMesh, tIntersectionCoords, tIntersectedFacets, aPoint, aAxis );
//                 }
//                 default:
//                 {
...
//             Vector< real > tIntersectionCoords = intersect_triangles_moller_trumbore( tCandidateFacets, aSurfaceMesh, aPoint, aAxis, tIntersectedFacets );

//             return check_if_node_is_inside_triangles( tIntersectionCoords, aPoint, aAxis, aSurfaceMesh.get_intersection_tolerance() );
...
//             //         // Vector< Facet* > tIntersectedFacets = intersect_triangles( tCandidateFacets, aSurfaceMesh, aPoint, aAxis );
...
//     real tEpsilon        = aSurfaceMesh.get_intersection_tolerance();
//     real tEpsilonScaling = 10.0;

//     uint           tCountJ = 0;
//     Vector< uint > tCandJ( aSurfaceMesh.get_number_of_facets() );
//     for ( uint iFacetIndex = 0; iFacetIndex < aSurfaceMesh.get_number_of_facets(); ++iFacetIndex )
//     {
//         // Get the difference of the cast point and the facet min and max coords in the J direction
//         real tJDirectionPreselection = ( aPoint( tFirstAxis ) - aSurfaceMesh.get_facet_min_coord( iFacetIndex, tFirstAxis ) )
//                                      * ( aSurfaceMesh.get_facet_max_coord( iFacetIndex, tFirstAxis ) - aPoint( tFirstAxis ) );
...
//             {
//                 const Vector< std::shared_ptr< sdf::Facet_Vertex > >& tVertices = aSurfaceMesh.get_facet( iFacetIndex ).get_facet_vertex_pointers();
...
//                 }
//                 else if ( aSurfaceMesh.get_facet( iFacetIndex ).get_distance_to_point( aPoint ) < tEpsilon )
//                 {
//                     aCandidateFacets.resize( 1 );
//                     aCandidateFacets( 0 ) = iFacetIndex;
//                     return FAIL_ON_EDGE;
...
//     aCandidateFacets.resize( aSurfaceMesh.get_number_of_facets() );
...
//         real tKDirectionPreselection = ( aPoint( tSecondAxis ) - aSurfaceMesh.get_facet_min_coord( tCandJ( k ), tSecondAxis ) )
//                                      * ( aSurfaceMesh.get_facet_max_coord( tCandJ( k ), tSecondAxis ) - aPoint( tSecondAxis ) );
...
//             {
//                 const Vector< std::shared_ptr< sdf::Facet_Vertex > >& tVertices = aSurfaceMesh.get_facet( tCandJ( k ) ).get_facet_vertex_pointers();
...
//                     return FAIL_ON_VERTEX;
//                 }
//                 else if ( aSurfaceMesh.get_facet( tCandJ( k ) ).get_distance_to_point( aPoint ) < tEpsilon )
//                 {
//                     aCandidateFacets.resize( 1 );
//                     aCandidateFacets( 0 ) = tCandJ( k );
//                     return FAIL_ON_EDGE;
...
//     aIntersectedFacets.resize( aSurfaceMesh.get_number_of_facets() );
//     aCandidateFacets.resize( aSurfaceMesh.get_number_of_facets() );
...
//     for ( uint iLineIndex = 0; iLineIndex < aSurfaceMesh.get_number_of_facets(); iLineIndex++ )
//     {
//         // Get a reference to this line and the Object's intersection tolerance
//         real   tEpsilon = aSurfaceMesh.get_intersection_tolerance();
//         Facet& tLine    = aSurfaceMesh.get_facet( iLineIndex );
...
//                 aCandidateFacets( tCandidateCount ) = iLineIndex;
//                 tCandidateCount++;
//             }
//         }
//     }
...
//         Facet& tFacet = aSurfaceMesh.get_facet( iCandidateFacetIndex );
...
//     Vector< uint > tIntersectedFacets( tCount );
...
//     {
//         // get reference to triangle
//         Facet& tFacet = aSurfaceMesh.get_facet( aCandidateFacets( iCandidateFacetIndex ) );

//         if ( tFacet.is_flagged() )
//         {
//             // add triangle to list
//             tIntersectedFacets( tCount++ ) = aCandidateFacets( iCandidateFacetIndex );
...
// Vector< real > intersect_triangles_moller_trumbore(
//         Vector< uint >&         aCandidateFacets,
//         mtk::Surface_Mesh&      aSurfaceMesh,
//         const Matrix< DDRMat >& aPoint,
//         uint                    aAxis,
...
//             aIntersectedFacets( tCount++ ) = iCandidateFacet;
//         }
//     }
...
//     Vector< uint > tIntersectedFacetsSorted( aIntersectedFacets.size() );
...
//                 // aMesh->intersect_with_coordinate_axis( aIntersectedFacets( iFacetIndex ), aPoint, aAxis, tCoordK, tError );
...
//                     aIntersectedFacets( iFacetIndex ) = std::numeric_limits< uint >::quiet_NaN();
//                 }
...
//     auto tItFacet = std::remove_if( aIntersectedFacets.begin(), aIntersectedFacets.end(), []( uint aFacet ) { return aFacet == std::numeric_limits< uint >::quiet_NaN(); } );
...
//     {
//         if ( std::abs( aIntersectionCoords( iIntersectionIndex ) - aPoint( aAxis ) ) < 10.0 * aSurfaceMesh.get_intersection_tolerance() )
//         {
//             return INTERFACE;
//         }
...
//     if ( aSurfaceMesh.get_spatial_dimension() == 2 )
//     {
...
//         Matrix< DDRMat > tAxis = random_axis( aSurfaceMesh.get_spatial_dimension() );
//         tRotation              = rotation_matrix( tAxis, tAngle );
//     }

//     aSurfaceMesh.append_rotation( tRotation );
