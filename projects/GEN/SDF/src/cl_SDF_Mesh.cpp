...
namespace moris::sdf
{
            ...
            , mMaxCoord( 3, 1 )
    {
        ...
        // pick first element
        Matrix< IndexMat > tNodeIndices = aMesh->get_nodes_connected_to_element_loc_inds( 0 );
        // determine interpolation order of mesh
        ...
        for ( uint k = 0; k < tNumberOfNodes; ++k )
        {
            mNodeIDs( k ) = aMesh->get_glb_entity_id_from_entity_loc_index( k, mtk::EntityRank::NODE );
        }
    }

    //-------------------------------------------------------------------------------

    Mesh::Mesh( const std::shared_ptr< mtk::Mesh >& aMesh, bool aVerbose )
            : Mesh( aMesh.get(), aVerbose )
    {
    }
