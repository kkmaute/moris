...
public:
    ...
    GEN_CylinderWithEndCaps( uint aNumberOfFibers,
                             bool aFibersForThesis,     // switch for if I am using the data file for thesis problem
                             real aRadii ) :
        mNumberOfFibers( aNumberOfFibers ),
        mFileHasBeenRead( false ),
        mFibersForThesis( aFibersForThesis ),
        mRadii( aRadii )
    {
            mAllFibers.resize( aNumberOfFibers );
    }
...
//        const real KSbeta=-10.0;
    ...
    //    aLength(2) = aMidPoint(2) - tFiberEnd00(2) + aRadius + aBuffer;
        aLength(0) = -aMidPoint(0) + tFiberEnd01(0) + mRadii + aBuffer;
        aLength(1) = -aMidPoint(1) + tFiberEnd01(1) + mRadii + aBuffer;
        aLength(2) = -aMidPoint(2) + tFiberEnd01(2) + mRadii + aBuffer;
    }
...
private:    // private functions
    ...
    void readFiberData(  )
    {
        std::string tMorisRoot;
        std::string tHdf5FilePath;
