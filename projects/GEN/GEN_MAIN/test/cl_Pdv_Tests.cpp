...
//------------------------------------------------------------------------------
    TEST_CASE("pdv_test_00","[GE],[pdv_check_00]")
    {
        ...
        if(par_size()<=1)
        {
            ...
            //------------------------------------------------------------------------------
            //------------------------------------------------------------------------------
            Cell< enum PDV > tPdvList(3);
            tPdvList(0) = PDV::DENSITY;
            tPdvList(1) = PDV::DENSITY1;
            tPdvList(2) = PDV::DENSITY2;
...
//------------------------------------------------------------------------------
    ...
    TEST_CASE("pdv test 01","[GE],[pdv_check_01]")
    {
        ...
        if(par_size()<=1)
        {
            ...
            Cell< enum PDV > tList(2);
            tList(0) = PDV::X_COORDINATE;  tList(1) = PDV::Y_COORDINATE;
...
//------------------------------------------------------------------------------
    TEST_CASE("pdv_test_02","[GE],[pdv_check_02]")
    {
        ...
        if(par_size()<=1)
        {
            ...
            tGeometryEngine.mark_ig_dv_type_as_unchanging( PDV::X_COORDINATE );
            //------------------------------------------------------------------------------
            // get the vertex indices of the circle to use below
            Matrix< IndexMat > tAllVertIndices;
            moris::mtk::Set* tTempSet = tEnrIntegMesh.get_set_by_name( "HMR_dummy_c_p0" );
            ...
            {
                REQUIRE( tIPChangingList(i) != PDV::DENSITY );
            }
            // ------------------------ end ------------------------

            // -------- check the IG list of changing types --------
            Cell< enum PDV > tIGChangingList;
            ...
            {
                REQUIRE( tIGChangingList(i) != PDV::X_COORDINATE );
            }
            // ------------------------ end ------------------------
        }
    }
