...
//    std::cout<<*mSolutionVector->get_vector()<<std::endl;
    ...
    if( mListOfDofTypes.size() == 1)
    {
        ...
        aElementRHS(0,0)=(0.4 - 10*mMySolVec( 0, 0 ) - 0.4*std::pow(mMySolVec( 1, 0 ),3) + 5*std::pow(mMySolVec( 1, 0 ),2));
        aElementRHS(1,0)=(0.15 - 0.4*std::pow(mMySolVec( 0, 0 ),3) + 3*std::pow(mMySolVec( 0, 0 ),2) - 10*mMySolVec( 1, 0 ));
    }
