...
namespace moris
{
    TEST_CASE( "testing stuff", "[Epetraaa Mat],[tested]" )
    {
        ...
        if ( tMyPID == 0 )
        {
            tSuccess = A.InsertGlobalValues( 0, 2, moris::Vector< double >{ 1.0, 2.0 }.memptr(), moris::Vector< int >{ 0, 1 }.memptr() );
            tSuccess = A.InsertGlobalValues( 1, 2, moris::Vector< double >{ 2.0, 3.0 }.memptr(), moris::Vector< int >{ 0, 2 }.memptr() );
        }
        else
        {
            tSuccess = A.InsertGlobalValues( 2, 2, moris::Vector< double >{ 4.0, 5.0 }.memptr(), moris::Vector< int >{ 1, 2 }.memptr() );
        }
