...
namespace moris::mig
{
    ...
    Periodic_3D::perform()
    {
        ...
        for ( uint tPairCount = 0; tPairCount < mMeshSideSetPairs.size(); tPairCount++ )
        {
            ...
            for ( const auto &tIter : mBackgroundCellToSideClusterMap1( tPairCount ) )
            {
                ...
                moris::Vector< moris_index > const &tSideClustersIndices2 = mBackgroundCellToSideClusterMap2( tPairCount )[tIter.first];
    ...
    void
    Periodic_3D::create_dbl_sided_cluster( moris::Vector< Matrix< DDRMat > > tP,
    ...
    Periodic_3D::create_leader_ig_cell( Matrix< IndexMat > const &tVertexIndex, moris::mtk::Cell const &aLeaderInterpCell, uint aPairCount )
    {
        ...
        // cell of Ig vertices
        moris::Vector< moris_index > tIgCellVertices( 4 );
    ...
    void
    Periodic_3D::offset_vector( moris::Matrix< DDRMat > &tOffsetVector, moris::Vector< std::string > &tFirstSideSetNames, moris::Vector< std::string > &tSecondSideSetNames, uint aPairCount )
    {
        // Integration mesh
        moris::mtk::Integration_Mesh *tIntegrationMesh = mMeshManager->get_integration_mesh( mMeshIndex );

        // get all the sidesets
        moris::Vector< moris::mtk::Side_Set * > const &tListOfSideSets = tIntegrationMesh->get_side_sets();

        moris::Vector< std::string > tSetNames( tListOfSideSets.size() );
    ...
    void
    Periodic_3D::generate_identifier( moris::Vector< mtk::Cluster const * >    &aSideClusters,
        uint                                                                 &aPairCount,
