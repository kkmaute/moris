...
namespace moris::mtk
{
    ...
    Periodic_Boundary_Condition_Helper::setup_periodic_boundary_conditions()
    {
        ...
        Vector< Vector< Cluster const * > > tDoubleSideSetClusters( mMeshSideSetPairs.size() );

        // Double sided side set
        Vector< mtk::Double_Side_Set* > tDblSideSet( mMeshSideSetPairs.size() );
        ...
        for ( uint tPairCounter = 0; tPairCounter < mMeshSideSetPairs.size(); tPairCounter++ )
        {
            ...
            Vector< moris::Matrix< moris::IdMat > > tFirstIds( tSetClusters.size() );
            ...
            Vector< moris::mtk::Cluster const * > tSetClusters2 = tSet2->get_clusters_on_set();

            // Cell to store 2nd side IDs of each integration cell - each member of the cell holds one cell
            Vector< moris::Matrix< moris::IdMat > > tSecondIds( tSetClusters.size() );
            ...
            switch ( tSpatialDim )
            {
                case 3:
                {
                    ...
                    for ( uint tClusterPairCounter = 0; tClusterPairCounter < tSetClusters.size(); tClusterPairCounter++ )
                    {
                        ...
                        const moris::Matrix< moris::IdMat >& tSecondPairIds = tSecondIds( tClusterPairCounter );

                        // vertex pairing for individual clusters
                        Vector< moris::mtk::Vertex const * > tVertexPairing( tNumVertexinCluster );
