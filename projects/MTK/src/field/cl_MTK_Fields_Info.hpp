...
namespace moris::mtk
{
    ...
    struct MtkFieldsInfo
    {
        ...
        print()
        {
            std::cout << "Number of Real Scalar Fields: " << std::setw( 6 ) << mRealScalarFields.size() << '\n';
            std::cout << "Number of Real Matrix Fields: " << std::setw( 6 ) << mRealMatrixFields.size() << '\n';
            std::cout << "Number of Int  Scaler Fields: " << std::setw( 6 ) << mSintScalarFields.size() << '\n';
            std::cout << "Number of Int  Matrix Fields: " << std::setw( 6 ) << mSintMatrixFields.size() << '\n';

            std::cout << " Real Scalar Field Information:" << '\n';
            for ( moris::uint i = 0; i < mRealScalarFields.size(); i++ )
            {
                std::cout << "    Name: " << mRealScalarFields( i )->get_field_name() << " | Entity Rank: " << (uint)mRealScalarFields( i )->get_field_entity_rank() << '\n';
            }
        }
    };
