...
namespace moris::mtk
{
    ...
    Ray_Cones Surface_Mesh::build_ray_cone_directions( real aConeAngle, uint aNumPolarRays, uint aNumAzimuthRays ) const
    {
        ...
        Ray_Cones tRayCones( tNumVertices, aNumPolarRays * aNumAzimuthRays, tDimension );

        // Get all the current vertex coordinates and normals
        ...
        switch ( tDimension )
        {
            case 2:
            {
                ...
                {
                    Matrix< DDRMat > tVertexNormal     = tVertexNormals.get_column( iV );    // Note: Fixed to use iV instead of iNumRays
                    auto const &     tLocalVertexIndex = iV;                                 // aLocalVertexIndex( iV );
                    ...
                    for ( uint iR = 0; iR < aNumPolarRays; ++iR )
                    {
                        ...
                        Matrix< DDRMat > tTempRay = tRotation * tVertexNormal;

                        Matrix< DDRMat > tConeRay;

                        if ( dot( tTempRay, tVertexNormal ) > 0 )
                        {
                        ...
                        tRayCones.mRayDirections( tLocalVertexIndex ).set_column( iR, tConeRay );
                    }
                }
            ...
            case 3:
            {
                ...
                for ( uint iV = 0; iV < tNumVertices; iV++ )
                {
                    ...
                    Matrix< DDRMat > tVertexNormal = tVertexNormals.get_column( iV );    // This is the `e3` vector

                    // Normalize the vertex normal to use as `e3`
                    Matrix< DDRMat > e3 = tVertexNormal / norm( tVertexNormal );
                    ...
                    for ( uint iPolar = 0; iPolar < aNumPolarRays; ++iPolar )
                    {
                        ...
                        for ( uint iAzimuth = 0; iAzimuth < aNumAzimuthRays; ++iAzimuth )
                        {
                            ...
                            if ( dot( tConeRay, tVertexNormal ) > 0 )
                            {
                            ...
                            {
                                // Check that the weight is consistent across all vertices
                                MORIS_ASSERT( std::abs( tRayCones.mDirectionWeights( iAzimuth + iPolar * aNumAzimuthRays ) - ( 1.0 / std::acos( dot( tConeRay, e3 ) / ( norm( tConeRay ) * norm( e3 ) ) ) ) ) < 1e-12,
                                        "Inconsistent ray weight for vertex %d, polar %d, azimuth %d",
                                        iV,
                                        iPolar,
                                        iAzimuth );
                            }
                            ...
                            tRayCones.mRayDirections( iV ).set_column( iAzimuth + iPolar * aNumAzimuthRays, tConeRay );
                        }
                    }
                }
    ...
    Vector< Intersection_Vector > Surface_Mesh::determine_nearest_nontrivial_intersections( Vector< Vector< Intersection_Vector > >& aAllIntersections ) const
    {
        ...
        // Loop over vertices
        for ( uint iV = 0; iV < aAllIntersections.size(); iV++ )
        {
            MORIS_ASSERT( aAllIntersections( iV ).size() == aAllIntersections( 0 ).size(), "Inconsistent number of rays per vertex in the input intersections." );

            // Loop over rays for this vertex
            for ( uint iR = 0; iR < aAllIntersections( iV ).size(); iR++ )
            {
                // Get the facets connected to this vertex
                Vector< moris_index > tConnectedFacets = this->get_vertexs_facet_indices( iV );

                MORIS_ASSERT( aAllIntersections( iV )( iR ).size() > 0, "No intersections found for vertex %d, ray %d. Is the normal correct?", iV, iR );

                // Loop over the intersections and find the first intersection that is not with a connected facet
                for ( uint iI = 0; iI < aAllIntersections( iV )( iR ).size(); iI++ )
                {
                    if ( std::find( tConnectedFacets.begin(), tConnectedFacets.end(), aAllIntersections( iV )( iR )( iI ).second ) == tConnectedFacets.end() )
                    {
                        tNearestIntersections( iV )( iR ) = aAllIntersections( iV )( iR )( iI );
                        break;
                    }

                    MORIS_ASSERT( iI < aAllIntersections( iV )( iR ).size() - 1, "No non-trivial intersection found for vertex %d, ray %d.", iV, iR );
                }
            }
        }
    ...
    Shape_Diameter_Distances Surface_Mesh::compute_nodal_shape_diameter_distances( real aConeAngle, uint aNumPolarRays, uint aNumAzimuthRays ) const
    {
        ...
        Vector< Vector< bool > >                tWarnings;
        Vector< Vector< Intersection_Vector > > tIntersections = this->cast_batch_of_rays( tVertexCoordinates, tRayCones.mRayDirections, tWarnings );
    ...
    Vector< real > Surface_Mesh::compute_nodal_shape_diameter()
    {
        ...
        // Get the number of rays per vertex
        uint tNumRays = tConeDistances.mDirectionWeights.size();

        // Check that we got a distance for every vertex and ray
        MORIS_ASSERT( tConeDistances.mDistances.size() == tNumVertices, "Inconsistent number of vertices in the shape diameter distances." );

        // Initialize return variable
        Vector< real > tShapeDiameterValues( tNumVertices, MORIS_REAL_MAX );

        // Resize the derivative to store for later
        mdShapeDiameterdVertex.resize( tNumVertices, this->get_spatial_dimension() );

        // Loop through every vertex in this surface mesh
        for ( uint iV = 0; iV < tNumVertices; iV++ )
        {
            MORIS_ASSERT( tConeDistances.mDistances( iV ).size() == tConeDistances.mDirectionWeights.size(), "Inconsistent number of rays in cone for vertex %d (Expected %d, Got %ld)", iV, tNumRays, tConeDistances.mDistances( iV ).size() );

            // Get the ray distances for this vertex
            Intersection_Vector& tRaysOnVertex = tConeDistances.mDistances( iV );

            Intersection_Vector tSortedRaysOnVertex = tRaysOnVertex;    // FIXME annoying copy

            // Compute the median ray distance for this vertex
            std::sort( tSortedRaysOnVertex.begin(), tSortedRaysOnVertex.end() );
            real tMedian;
            if ( tNumRays % 2 == 0 )
            {
                tMedian = ( tSortedRaysOnVertex( tNumRays / 2 - 1 ).second + tSortedRaysOnVertex( tNumRays / 2 ).second ) / 2;
            }
            else
            {
                tMedian = tSortedRaysOnVertex( tNumRays / 2 ).second;
            }

            // Compute the mean ray distance for this vertex
            real tMean = 0.0;
            for ( const auto& tDistance : tSortedRaysOnVertex )
            {
                tMean += tDistance.second;
            }
            tMean = tMean / tNumRays;

            // Compute the standard deviation ray distances for this vertex
            real tVariance = 0.0;
            for ( const auto& tDistance : tSortedRaysOnVertex )
            {
                tVariance += ( tDistance.second - tMean ) * ( tDistance.second - tMean );
            }
            real tStdDeviation = std::sqrt( tVariance / tNumRays );

            // Compute the weighted sum for entries within one standard deviation from the median
            real tWeightedSum       = 0.0;
            ...
            // FIXME I think this is the same for every vertex, so could be moved outside the loop
            for ( uint iR = 0; iR < tNumRays; iR++ )
            {
                if ( std::abs( tRaysOnVertex( iR ).second - tMedian ) <= tStdDeviation )
                {
                    tSumOfValidWeights += std::abs( tConeDistances.mDirectionWeights( iR ) );
                }
            }
            MORIS_ERROR( tSumOfValidWeights > 0.0, "Sum of valid weights is zero for vertex %d. Check ray directions and intersections.", iV );

            for ( uint iR = 0; iR < tNumRays; iR++ )
            {
                if ( std::abs( tRaysOnVertex( iR ).second - tMedian ) <= tStdDeviation )
                {
                    tWeightedSum += tRaysOnVertex( iR ).second * std::abs( tConeDistances.mDirectionWeights( iR ) );

                    // distribute correct order of each node indices
                    // x_k : the origin point from which the ray is shot
                    // {x_k-1,x_k+1} : neighboring node indices at origin point
                    // x_p : the intersected point
                    // {x_p-1,x_p+1} : vertices associated with the intersection facet
                    moris_index tOriginIndex           = iV;
                    moris_index tOriginNeighbor1       = this->get_vertex_neighbors( iV )( 0 );
                    moris_index tOriginNeighbor2       = this->get_vertex_neighbors( iV )( 1 );
                    moris_index tIntersectionNeighbor1 = this->get_facets_vertex_indices( tConeDistances.mDistances( iV )( iR ).first )( 0 );
                    moris_index tIntersectionNeighbor2 = this->get_facets_vertex_indices( tConeDistances.mDistances( iV )( iR ).first )( 1 );

                    Matrix< DDRMat > xm  = this->get_vertex_coordinates( tOriginIndex );
                    real             xm1 = xm( 0, 0 );
                    real             xm2 = xm( 1, 0 );

                    Matrix< DDRMat > xl  = this->get_vertex_coordinates( tOriginNeighbor1 );
                    real             xl1 = xl( 0, 0 );
                    real             xl2 = xl( 1, 0 );

                    Matrix< DDRMat > xr  = this->get_vertex_coordinates( tOriginNeighbor2 );
                    real             xr1 = xr( 0, 0 );
                    real             xr2 = xr( 1, 0 );

                    Matrix< DDRMat > yl  = this->get_vertex_coordinates( tIntersectionNeighbor1 );
                    real             yl1 = yl( 0, 0 );
                    real             yl2 = yl( 1, 0 );

                    Matrix< DDRMat > yr  = this->get_vertex_coordinates( tIntersectionNeighbor2 );
                    real             yr1 = yr( 0, 0 );
                    real             yr2 = yr( 1, 0 );

                    // angle is in radians
                    real tet = 1.0 / tConeDistances.mDirectionWeights( iR );

                    real dThicknessdxm1 = ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ), 2.0 ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ), 2.0 ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) );

                    real dThicknessdxm2 = -( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ), 2.0 ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ), 2.0 ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) );

                    dthk_dxm1 = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdxm1;
                    dthk_dxm2 = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdxm2;

                    real dThicknessdxl1 = -1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) + sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) - cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) - ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) - cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) - 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) + sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) - cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) - ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) + sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl1 - xm1, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xm1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) );

                    real dThicknessdxl2 = 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) + cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) - sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) + ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) - sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) + 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) + cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) - sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) - ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) + cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xl2 - xm2, 2.0 ) / ( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) - sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xm2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xl1 - xm1 ), 2.0 ) + pow( std::abs( xl2 - xm2 ), 2.0 ) ) ) ) ) );
                    dthk_dxl1           = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdxl1;
                    dthk_dxl2           = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdxl2;

                    real dThicknessdxr1 = 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) + sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) - cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) - ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) - cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) + 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) + sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) - cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) - ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl1 - xr1, 2.0 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) + sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm1 - xr1, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm1 - xr1, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) );

                    real dThicknessdxr2 = -1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) + cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) - sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) + ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) - sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) - 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) + cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( cos( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) - sin( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) - ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( sin( tet ) * ( 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) - 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xl2 - xr2, 2.0 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) + cos( tet ) * 1.0 / pow( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * ( xl1 - xr1 ) * ( xl2 - xr2 ) * ( pow( xm2 - xr2, 2.0 ) / ( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) - sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) * ( 1.0 / pow( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ), 3.0 / 2.0 ) * pow( xm2 - xr2, 2.0 ) - 1.0 / sqrt( pow( std::abs( xm1 - xr1 ), 2.0 ) + pow( std::abs( xm2 - xr2 ), 2.0 ) ) ) ) ) );
                    dthk_dxr1           = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdxr1;
                    dthk_dxr2           = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdxr2;

                    real dThicknessdyl1 = -( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ), 2.0 ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + 1.0 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ), 2.0 ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + 1.0 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) );

                    real dThicknessdyl2 = ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ), 2.0 ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + 1.0 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ), 2.0 ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + 1.0 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) );

                    dthk_dyl1 = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdyl1;
                    dthk_dyl2 = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdyl2;

                    real dThicknessdyr1 = ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ), 2.0 ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ), 2.0 ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( yl2 - yr2 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) );

                    real dThicknessdyr2 = -( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ), 2.0 ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( 1.0 / sqrt( pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) + sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) + pow( std::abs( ( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ) ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ), 2.0 ) ) * sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * pow( cos( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl1 - xr1 ) - sin( tet ) * 1.0 / sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xl2 - xr2 ), 2.0 ) * ( ( ( xm1 - yl1 ) * ( xl1 * cos( tet ) - xr1 * cos( tet ) - xl2 * sin( tet ) + xr2 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) + ( ( xm2 - yl2 ) * ( xl2 * cos( tet ) - xr2 * cos( tet ) + xl1 * sin( tet ) - xr1 * sin( tet ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) * ( yl1 - yr1 ) * ( ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm1 - yl1 ) * ( yl2 - yr2 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) - ( sqrt( pow( std::abs( xl1 - xr1 ), 2.0 ) + pow( std::abs( xl2 - xr2 ), 2.0 ) ) * ( xm2 - yl2 ) * ( yl1 - yr1 ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) ) ) ) / ( xl1 * yl1 * cos( tet ) + xl2 * yl2 * cos( tet ) - xl1 * yr1 * cos( tet ) - xr1 * yl1 * cos( tet ) - xl2 * yr2 * cos( tet ) - xr2 * yl2 * cos( tet ) + xr1 * yr1 * cos( tet ) + xr2 * yr2 * cos( tet ) + xl1 * yl2 * sin( tet ) - xl2 * yl1 * sin( tet ) - xl1 * yr2 * sin( tet ) + xl2 * yr1 * sin( tet ) - xr1 * yl2 * sin( tet ) + xr2 * yl1 * sin( tet ) + xr1 * yr2 * sin( tet ) - xr2 * yr1 * sin( tet ) );

                    dthk_dyr1 = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdyr1;
                    dthk_dyr2 = std::abs( tConeDistances.mDirectionWeights( iR ) ) * dThicknessdyr2;

                    // Store sensitivitiy
                    mdShapeDiameterdVertex( tOriginIndex, 0 ) += dthk_dxm1 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tOriginNeighbor1, 0 ) += dthk_dxl1 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tOriginNeighbor2, 0 ) += dthk_dxr1 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tIntersectionNeighbor1, 0 ) += dthk_dyl1 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tIntersectionNeighbor2, 0 ) += dthk_dyr1 / tSumOfValidWeights;

                    mdShapeDiameterdVertex( tOriginIndex, 1 ) += dthk_dxm2 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tOriginNeighbor1, 1 ) += dthk_dxl2 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tOriginNeighbor2, 1 ) += dthk_dxr2 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tIntersectionNeighbor1, 1 ) += dthk_dyl2 / tSumOfValidWeights;
                    mdShapeDiameterdVertex( tIntersectionNeighbor2, 1 ) += dthk_dyr2 / tSumOfValidWeights;
                }
            }

            // Divide by the sum of weights to get the weighted average
            tShapeDiameterValues( iV ) = tWeightedSum / tSumOfValidWeights;
        }
