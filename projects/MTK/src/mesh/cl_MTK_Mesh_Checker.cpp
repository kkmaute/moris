...
namespace moris::mtk
{
    ...
    Mesh_Checker::verify_vertex_ownership( Serialized_Mesh_Data* aSerializedMesh )
    {
        ...
        if ( par_rank() == 0 )
        {
            ...
            for ( moris::uint iV = 0; iV < tVertexOwners.numel(); iV++ )
            {
                ...
                {
                    std::cout << "Vertex Ownership issue 2" << '\n';
                    tValidInd = 0;
                }
            }
        }
    ...
    Mesh_Checker::verify_cell_ownership( Serialized_Mesh_Data* aSerializedMesh )
    {
        ...
        if ( par_rank() == 0 )
        {
                ...
                for ( moris::uint iV = 0; iV < aSerializedMesh->mCollectCellIds( iP ).numel(); iV++ )
                {
                    ...
                    else
                    {
                        ...
                        if ( aSerializedMesh->mCollectCellOwners( iP )( tProcVertInd ) != tCellOwners( tSerialVertInd ) )
                        {
                            ...
                            std::cout << aSerializedMesh->mCollectCellOwners( iP )( tProcVertInd ) << " | expected owner = " << tCellOwners( tSerialVertInd ) << '\n';
                            tValidInd = 0;
                        }
                    }
                }
            ...
            for ( moris::uint iV = 0; iV < tCellOwners.numel(); iV++ )
            {
                ...
                {
                    std::cout << "Cell Ownership issue 2" << '\n';
                    tValidInd = 0;
                }
            }
        }
    ...
    Mesh_Checker::print_diagnostics()
    {
        if ( par_rank() == 0 )
        {
            ...
            std::cout << "Mesh Checking Diagnostics:" << '\n';
            std::cout << std::setw( tFirstColWidth ) << "" << std::setw( tSecondColWidth ) << "Interp Mesh" << std::setw( tSecondColWidth ) << " | Integ Mesh" << '\n';
            std::cout << std::setw( tFirstColWidth ) << "Vertex Ids: " << std::setw( tSecondColWidth ) << this->bool_to_string( mIpVertexDiag ) << " |" << std::setw( tSecondColWidth ) << this->bool_to_string( mIgVertexDiag ) << '\n';
            std::cout << std::setw( tFirstColWidth ) << "Vertex Coords: " << std::setw( tSecondColWidth ) << this->bool_to_string( mIpVertexDiag ) << " |" << std::setw( tSecondColWidth ) << this->bool_to_string( mIgVertexDiag ) << '\n';
            std::cout << std::setw( tFirstColWidth ) << "Vertex Owners: " << std::setw( tSecondColWidth ) << this->bool_to_string( mIpVertexOwnerDiag ) << " |" << std::setw( tSecondColWidth ) << this->bool_to_string( mIgVertexOwnerDiag ) << '\n';
            std::cout << std::setw( tFirstColWidth ) << "Cell Owners: " << std::setw( tSecondColWidth ) << this->bool_to_string( mIpCellOwnerDiag ) << " |" << std::setw( tSecondColWidth ) << this->bool_to_string( mIgCellOwnerDiag ) << '\n';
        }
    }
    //--------------------------------------------------------------------------------
    ...
    {
        // gather vertex
        moris::gatherv_mats( aSerializedMesh->mVertexIds, aSerializedMesh->mCollectVertexIds );
        moris::gatherv_mats( aSerializedMesh->mVertexOwners, aSerializedMesh->mCollectVertexOwners );
        moris::gatherv_mats( aSerializedMesh->mVertexCoordinates, aSerializedMesh->mCollectVertexCoords );

        // gather cells
        moris::gatherv_mats( aSerializedMesh->mCellIds, aSerializedMesh->mCollectCellIds );
        moris::gatherv_mats( aSerializedMesh->mCellOwners, aSerializedMesh->mCollectCellOwners );
    }
    ...
    Mesh_Checker::gather_serialized_ip_mesh( Serialized_Mesh_Data* aSerializedMesh )
    {
        ...
        moris::gatherv_mats( tVertexTMatrixWeightsData, tGatheredFlattenedWeightsData );
        moris::gatherv_mats( tVertexTMatrixWeightsOffsets, tGatheredFlattenedWeightsOffsets );
        ...
        moris::gatherv_mats( tVertexTMatrixIdsData, tGatheredFlattenedBasisIdData );
        moris::gatherv_mats( tVertexTMatrixIdsOffsets, tGatheredFlattenedBasisIdOffsets );
