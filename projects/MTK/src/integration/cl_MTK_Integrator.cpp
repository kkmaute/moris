...
namespace moris::mtk
{
            ...
            , mNumOfTimePoints( mTimeCoeffs->get_number_of_points() )
    {
        ...
        if ( aMeshSet->get_set_type() == mtk::SetType::BULK )
        {
            ...
            m_compute_cluster_integration_points_and_weights_perturbed =
                    aUseMomentFitting ?
                            &Integrator::compute_bulk_cluster_integration_points_and_weights_moment_fitting_perturbed:
                            nullptr;
        }
    ...
    void Integrator::compute_bulk_cluster_integration_points_and_weights_moment_fitting( const Cluster *aCluster )
    {
        ...
        for ( uint iFacetIndex = 0; iFacetIndex < tBoundaryFacetElementOrdinals.n_rows(); iFacetIndex++ )
        {
            ...
            [[maybe_unused]] real tNx = tFacetNormal( 0, 0 );
            ...
            else 
            {
                ...
                for ( uint iQuadPtIndex = 0; iQuadPtIndex < tIntWeightsFacet.numel(); iQuadPtIndex++ )
                {
                    ...
                    if ( tDim == 2 )
                    {
                        ...
                        if ( tOrder == 1 )
                        {
                            tAntiDerivIntOrder = mtk::Integration_Order::BAR_4;
                        }
                        if ( tOrder == 2 )
                        {
                            tAntiDerivIntOrder = mtk::Integration_Order::BAR_4;
                        ...
                        mtk::Interpolation_Rule           tMomentInterpolationRule( mtk::Geometry_Type::QUAD, mtk::Interpolation_Type::LAGRANGE, tInterpOrder, mtk::Geometry_Type::LINE, mtk::Interpolation_Type::LAGRANGE, mtk::Interpolation_Order::LINEAR );
                        ...
                        for ( uint iAntiDerivQuadPoint = 0; iAntiDerivQuadPoint < tIntWeightsAntiDeriv.numel(); iAntiDerivQuadPoint++ )
                        {
                            ...
                            real tSigma_y = 0.5 * ( 1.0 + tAntiDerivQuadPoint( 0 ) ) * ( tYm );

                            // Get final point coordinates for the moment functions
                            Matrix< DDRMat > tAntiDerivativeEvalPoint_y = { { tXm }, { tSigma_y } };

                            // Evaluate Moment fitting polynomial at this point
                            Matrix< DDRMat > tMomentValue_y;

                            tMomentInterp->eval_N( tAntiDerivativeEvalPoint_y, tMomentValue_y );

                            // Geometric jacobian will be 0.5 * tZm
                            // Compute value of moment
                            tMomentFittingRHS += 0.25 * trans( tMomentValue_y ) * tD * 0.5 * tYm * tNy * tQuadWeight * tIntWeightsAntiDeriv( iAntiDerivQuadPoint );
                        }
                    }
                    else if ( tDim == 3 )
                    {
                    ...
                    //     [[maybe_unused]] real tt4 = std::pow( tYm, 2 );
                    //     [[maybe_unused]] real tt5 = std::pow( tYm, 3 );
                    ...
                    //     tMomentFittingRHS( 0, 0 ) += 2.0*0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 2.7e+1 / 2.56e+2 ) + tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) - tt3 * ( 9.0 / 5.12e+2 ) + tXm / 5.12e+2 - 1.0 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 - tt3 * 7.91015625e-2 + tXm * 8.7890625e-3 - 8.7890625e-3 ) - tYm * ( tt2 * ( 9.0 / 2.56e+2 ) - tt3 * ( 9.0 / 2.56e+2 ) + tXm / 2.56e+2 - 1.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 1, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 2.7e+1 / 2.56e+2 ) - tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) + tt3 * ( 9.0 / 5.12e+2 ) - tXm / 5.12e+2 - 1.0 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 + tt3 * 7.91015625e-2 - tXm * 8.7890625e-3 - 8.7890625e-3 ) - tYm * ( tt2 * ( 9.0 / 2.56e+2 ) + tt3 * ( 9.0 / 2.56e+2 ) - tXm / 2.56e+2 - 1.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 2, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 2.7e+1 / 2.56e+2 ) - tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) + tt3 * ( 9.0 / 5.12e+2 ) - tXm / 5.12e+2 - 1.0 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 + tt3 * 7.91015625e-2 - tXm * 8.7890625e-3 - 8.7890625e-3 ) - tYm * ( tt2 * ( 9.0 / 2.56e+2 ) + tt3 * ( 9.0 / 2.56e+2 ) - tXm / 2.56e+2 - 1.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 3, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 2.7e+1 / 2.56e+2 ) + tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) - tt3 * ( 9.0 / 5.12e+2 ) + tXm / 5.12e+2 - 1.0 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 - tt3 * 7.91015625e-2 + tXm * 8.7890625e-3 - 8.7890625e-3 ) - tYm * ( tt2 * ( 9.0 / 2.56e+2 ) - tt3 * ( 9.0 / 2.56e+2 ) + tXm / 2.56e+2 - 1.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 4, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 8.1e+1 / 2.56e+2 ) + tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) - tt3 * ( 2.7e+1 / 5.12e+2 ) + tXm * ( 2.7e+1 / 5.12e+2 ) - 9.0 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 - tt3 * 2.373046875e-1 + tXm * 2.373046875e-1 - 7.91015625e-2 ) + tYm * ( tt2 * ( 9.0 / 2.56e+2 ) - tt3 * ( 2.7e+1 / 2.56e+2 ) + tXm * ( 2.7e+1 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 5, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 8.1e+1 / 2.56e+2 ) - tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) + tt3 * ( 2.7e+1 / 5.12e+2 ) - tXm * ( 2.7e+1 / 5.12e+2 ) - 9.0 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 + tt3 * 2.373046875e-1 - tXm * 2.373046875e-1 - 7.91015625e-2 ) + tYm * ( tt2 * ( 9.0 / 2.56e+2 ) + tt3 * ( 2.7e+1 / 2.56e+2 ) - tXm * ( 2.7e+1 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 6, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 2.7e+1 / 2.56e+2 ) - tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) + tt3 * ( 2.43e+2 / 5.12e+2 ) - tXm * ( 2.7e+1 / 5.12e+2 ) - 2.7e+1 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 + tt3 * 2.373046875e-1 - tXm * 2.63671875e-2 - 2.63671875e-2 ) + tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) + tt3 * ( 8.1e+1 / 2.56e+2 ) - tXm * ( 9.0 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 7, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 2.7e+1 / 2.56e+2 ) - tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) + tt3 * ( 2.43e+2 / 5.12e+2 ) - tXm * ( 2.7e+1 / 5.12e+2 ) - 2.7e+1 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 + tt3 * 2.373046875e-1 - tXm * 2.63671875e-2 - 2.63671875e-2 ) + tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) + tt3 * ( 8.1e+1 / 2.56e+2 ) - tXm * ( 9.0 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 8, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 8.1e+1 / 2.56e+2 ) - tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) + tt3 * ( 2.7e+1 / 5.12e+2 ) - tXm * ( 2.7e+1 / 5.12e+2 ) - 9.0 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 + tt3 * 2.373046875e-1 - tXm * 2.373046875e-1 - 7.91015625e-2 ) + tYm * ( tt2 * ( 9.0 / 2.56e+2 ) + tt3 * ( 2.7e+1 / 2.56e+2 ) - tXm * ( 2.7e+1 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 9, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 8.1e+1 / 2.56e+2 ) + tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 9.0 / 5.12e+2 ) - tt3 * ( 2.7e+1 / 5.12e+2 ) + tXm * ( 2.7e+1 / 5.12e+2 ) - 9.0 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 7.91015625e-2 - tt3 * 2.373046875e-1 + tXm * 2.373046875e-1 - 7.91015625e-2 ) + tYm * ( tt2 * ( 9.0 / 2.56e+2 ) - tt3 * ( 2.7e+1 / 2.56e+2 ) + tXm * ( 2.7e+1 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 10, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 2.7e+1 / 2.56e+2 ) + tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) - tt3 * ( 2.43e+2 / 5.12e+2 ) + tXm * ( 2.7e+1 / 5.12e+2 ) - 2.7e+1 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 - tt3 * 2.373046875e-1 + tXm * 2.63671875e-2 - 2.63671875e-2 ) + tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) - tt3 * ( 8.1e+1 / 2.56e+2 ) + tXm * ( 9.0 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 11, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( -std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 2.7e+1 / 2.56e+2 ) + tXm * ( 3.0 / 2.56e+2 ) - 3.0 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) - tt3 * ( 2.43e+2 / 5.12e+2 ) + tXm * ( 2.7e+1 / 5.12e+2 ) - 2.7e+1 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 - tt3 * 2.373046875e-1 + tXm * 2.63671875e-2 - 2.63671875e-2 ) + tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) - tt3 * ( 8.1e+1 / 2.56e+2 ) + tXm * ( 9.0 / 2.56e+2 ) - 9.0 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 12, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 8.1e+1 / 2.56e+2 ) + tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) - tt3 * ( 7.29e+2 / 5.12e+2 ) + tXm * ( 7.29e+2 / 5.12e+2 ) - 2.43e+2 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 - tt3 * 7.119140625e-1 + tXm * 7.119140625e-1 - 2.373046875e-1 ) - tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) - tt3 * ( 2.43e+2 / 2.56e+2 ) + tXm * ( 2.43e+2 / 2.56e+2 ) - 8.1e+1 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 13, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 8.1e+1 / 2.56e+2 ) - tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) + std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) + tt3 * ( 7.29e+2 / 5.12e+2 ) - tXm * ( 7.29e+2 / 5.12e+2 ) - 2.43e+2 / 5.12e+2 ) - std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 + tt3 * 7.119140625e-1 - tXm * 7.119140625e-1 - 2.373046875e-1 ) - tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) + tt3 * ( 2.43e+2 / 2.56e+2 ) - tXm * ( 2.43e+2 / 2.56e+2 ) - 8.1e+1 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 14, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) + tt3 * ( 8.1e+1 / 2.56e+2 ) - tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) + tt3 * ( 7.29e+2 / 5.12e+2 ) - tXm * ( 7.29e+2 / 5.12e+2 ) - 2.43e+2 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 + tt3 * 7.119140625e-1 - tXm * 7.119140625e-1 - 2.373046875e-1 ) - tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) + tt3 * ( 2.43e+2 / 2.56e+2 ) - tXm * ( 2.43e+2 / 2.56e+2 ) - 8.1e+1 / 2.56e+2 ) );
                    ...
                    //     tMomentFittingRHS( 15, 0 ) += 2.0 * 0.5 * 0.5 * tQuadWeight * tD * tNy * ( std::pow( tYm, 3 ) * ( tt2 * ( 2.7e+1 / 2.56e+2 ) - tt3 * ( 8.1e+1 / 2.56e+2 ) + tXm * ( 8.1e+1 / 2.56e+2 ) - 2.7e+1 / 2.56e+2 ) - std::pow( tYm, 2 ) * ( tt2 * ( 2.43e+2 / 5.12e+2 ) - tt3 * ( 7.29e+2 / 5.12e+2 ) + tXm * ( 7.29e+2 / 5.12e+2 ) - 2.43e+2 / 5.12e+2 ) + std::pow( tYm, 4 ) * ( tt2 * 2.373046875e-1 - tt3 * 7.119140625e-1 + tXm * 7.119140625e-1 - 2.373046875e-1 ) - tYm * ( tt2 * ( 8.1e+1 / 2.56e+2 ) - tt3 * ( 2.43e+2 / 2.56e+2 ) + tXm * ( 2.43e+2 / 2.56e+2 ) - 8.1e+1 / 2.56e+2 ) );
                    // }
                }
    ...
    //------------------------------------------------------------------------------
    void Integrator::compute_bulk_cluster_integration_points_and_weights_moment_fitting_perturbed( const Cluster *aCluster, const moris_index aNodeIndex, const Matrix< DDRMat > aPerturbation, const Matrix< DDRMat > aPhysicalPerturbation )
    {
        // Drop const and static cast into tCellCluster because we know func is for cell clusters and because we need to modify it
        Cell_Cluster *tCellCluster = static_cast< Cell_Cluster * >( const_cast< Cluster * >( aCluster ) );
        ...
        uint tOrder = tInterpOrder == Interpolation_Order::LINEAR ? 1 : tInterpOrder == Interpolation_Order::QUADRATIC ? 2
                                                              : tInterpOrder == Interpolation_Order::CUBIC             ? 3
                                                                                                                       : 0;
        ...
        for ( uint iFacetIndex = 0; iFacetIndex < tBoundaryFacetElementOrdinals.n_rows(); iFacetIndex++ )
        {
            ...
            for ( uint iNodeIndex = 0; iNodeIndex < tNodeToFacetMap.n_cols(); iNodeIndex++ )
            {
                ...
                // Check if IDs associated with the any of the vertices are the same as the node ID for which we are computing the perturbed quadrature rule, and if so perturb the vertex coordinates by the specified perturbation amount in a random direction
                if ( tVertexPointers( iNodeIndex )->get_index() == aNodeIndex )
                {
                    // Get the coordinates of the vertex with respect to the interpolation cell
                    tFacetVertexCoordinates.set_row( iNodeIndex, aPerturbation);
                }
                else
                {
                    // If the vertex is not the one to be perturbed, simply get its coordinates with respect to the interpolation cell and place them in tFacetVertexCoordinates
                    tFacetVertexCoordinates.set_row( iNodeIndex, tCellCluster->get_vertex_local_coordinate_wrt_interp_cell( tVertexPointers( iNodeIndex ) ) );
                }
            }
            ...
            //compute facet normal from the tFacetVertexCoordinates
            Matrix< DDRMat > tFacetNormal;

            // Get the nodes which need to be used to compute normal
            moris::Matrix< moris::IndexMat > tEdgeNodesForNormal = tCell->get_cell_info()->get_node_map_outward_normal( tElementOrdinal );

            // get all the vertex indices of the cell
            moris::Matrix< moris::IndexMat > tCellVertexIndices = tCell->get_vertex_inds();

            // initialize the output normal
            Matrix< DDRMat > tOutwardNormal;

            // get the vertex coordinates
            moris::Matrix< moris::DDRMat > tVertexCoords = tCell->get_vertex_coords();
            ...
            if ( tVertexCoords.n_cols() == 2 )
            {
                ...
                // if the index of either the first node or the second node is the one that is perturbed, do that
                if ( tCellVertexIndices( tFirstNode ) == aNodeIndex )
                {
                    real tX1       = aPhysicalPerturbation( 0 );
                    real tX2       = tVertexCoords( tSecondNode, 0 );
                    real tY1       = aPhysicalPerturbation( 1 );
                    real tY2       = tVertexCoords( tSecondNode, 1 );
                    tOutwardNormal = { { -tY2 + tY1 }, { tX2 - tX1 } };
                }
                else if ( tCellVertexIndices( tSecondNode ) == aNodeIndex )
                {
                    real tX1       = tVertexCoords( tFirstNode, 0 );
                    real tX2       = aPhysicalPerturbation( 0 );
                    real tY1       = tVertexCoords( tFirstNode, 1 );
                    real tY2       = aPhysicalPerturbation( 1 );
                    tOutwardNormal = { { -tY2 + tY1 }, { tX2 - tX1 } };
                }
                else
                {
            ...
            // Normalize
            tFacetNormal = tOutwardNormal / moris::norm( tOutwardNormal );
            //Matrix< DDRMat > tFacetNormal = tCell->compute_outward_side_normal( tElementOrdinal );
    ...
    //------------------------------------------------------------------------------
    void
    Integrator::restore_quadrature_weights_and_points( const Cluster *aCluster, Matrix< DDRMat > &aQuadraturePoints, Matrix< DDRMat > &aQuadratureWeights )
    {
        // Implementation for restoring quadrature weights and points
        // This function would typically restore the original quadrature points and weights
        // for a cluster after they have been modified for sensitivities or other purposes.
        // Drop const and static cast into tCellCluster because we know func is for cell clusters and because we need to modify it
        Cell_Cluster *tCellCluster = static_cast< Cell_Cluster * >( const_cast< Cluster * >( aCluster ) );
        tCellCluster->set_quadrature_points( aQuadraturePoints );
        tCellCluster->set_quadrature_weights( aQuadratureWeights );
    }
