...
namespace moris
{
    namespace mtk
    {
        ...
        Matrix< DDRMat >
        Cell_Info_Tet4::get_vertex_loc_coord( moris_index const &aVertexOrdinal ) const
        {
        ...
        Cell_Info_Tet4::compute_cell_size_deriv( moris::mtk::Cell const *aCell, uint aLocalVertexID, uint aDirection ) const
        {
            ...
            // calculating volume
            return 1.0 / 6.0 * std::pow( -1.0, aLocalVertexID ) * ( tNodeCoords20( tDirIndexMap( aDirection ) ) * tNodeCoords30( tDirIndexMap( aDirection + 1 ) ) - tNodeCoords20( tDirIndexMap( aDirection + 1 ) ) * tNodeCoords30( tDirIndexMap( aDirection ) ) );
        }
