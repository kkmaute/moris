...
namespace moris
{
    namespace mtk
    {
            ...
            for ( uint tPairCount = 0; tPairCount < mMeshSideSetPairs.size(); tPairCount++ )
            {
                ...
                for ( const auto &tIter : tBackgroundCellToSideClusterMap1 )
                {
                    ...
                    else
                    {
                        ...
                        Vector< moris::Matrix< DDRMat > > tParamCoordsCell1( tIGCellNum, moris::Matrix< DDRMat >( 2, 3 ) );
                        ...
                        Vector< moris::Matrix< DDRMat > > tParamCoordsCell2( tIGCellNum, moris::Matrix< DDRMat >( 2, 3 ) );
        ...
        void
        Intersection_Detect::create_dbl_sided_cluster( Vector< Matrix< DDRMat > > &tP,
                ...
                moris_index                                                            &aPhaseToPhase )
        {
            ...
            Vector< moris::mtk::Vertex * > tLeaderVerticesCell;
            ...
            Vector< moris::mtk::Vertex * > tFollowerVerticesCell;
            ...
            Vector< moris::mtk::Vertex const * > tLeaderVerticesConst( tLeaderVerticesCell.size() - 1, nullptr );
            Vector< moris::mtk::Vertex const * > tFollowerVerticesConst( tLeaderVerticesCell.size() - 1, nullptr );
        ...
        Intersection_Detect::create_leader_ig_cell( Vector< moris::mtk::Vertex * > tLeaderVertices, Matrix< IndexMat > tVertexIndex, moris::mtk::Cell const &aLeaderInterpCell, uint aPairCount )
        {
            ...
            // cell of Ig vertices
            Vector< moris::mtk::Vertex * > tIgCellVertices( 4 );
        ...
        moris::mtk::Cell const *
        Intersection_Detect::create_follower_ig_cell( Vector< moris::mtk::Vertex * > tFollowerVertices, Matrix< IndexMat > tVertexIndex, moris::mtk::Cell const &aFollowerInterpCell, uint aPairCount )
        {
            // Sort the nodes such that it is consistent with Exodus convention
            Matrix< IndexMat > tVertexIndexCounterClockWise;
        ...
        void
        Intersection_Detect::offset_vector( moris::Matrix< DDRMat > &tOffsetVector, Vector< std::string > &tFirstSideSetNames, Vector< std::string > &tSecondSideSetNames, uint aPairCount )
        {
            // Integration mesh
            moris::mtk::Integration_Mesh *tIntegrationMesh = mMeshManager->get_integration_mesh( mMeshIndex );

            // get all the sidesets
            Vector< std::string > tSetNames = tIntegrationMesh->get_set_names( EntityRank::FACE );
        ...
        void
        Intersection_Detect::create_leader_vertices( Vector< moris::mtk::Vertex * > &aLeaderVertices, Matrix< DDRMat > &tPhysicalCoordinates3D, moris::mtk::Cell const &aLeaderInterpCell, uint aPairCount, Matrix< DDRMat > &tNewNodeCoordinates )
        {
        ...
        void
        Intersection_Detect::create_follower_vertices( Vector< moris::mtk::Vertex * > &aFollowerVertices, Matrix< DDRMat > &tPhysicalCoordinates3D, moris::mtk::Cell const &aFollowerInterpCell, uint aPairCount, Matrix< DDRMat > &tNewNodeCoordinates )
        {
            uint tNumSurfaceNodes = tPhysicalCoordinates3D.n_rows();
                ...
                moris::Matrix< moris::IdMat > &aLocalElemToNode )
        {
            ...
            Vector< moris::mtk::Cell const * > const &tIntegCells = aSideCluster->get_primary_cells_in_cluster();
            ...
            for ( uint k = 0; k < tIntegCells.size(); k++ )
            {
                ...
                Vector< moris::mtk::Vertex const * > tVertices = tIntegCells( k )->get_vertices_on_side_ordinal( tCellOrdinals( k ) );
                ...
                moris::Matrix< moris::IndexMat > const &aSecondTRIConnect )
        {
            ...
            Vector< moris::Matrix< moris::DDRMat > > allIntersectionTriangles;
            ...
            Vector< uint > bd( aSecondTRIConnect.n_rows() + 1, 0 );
            ...
            while ( bl.size() > 0 )
            {
                ...
                Vector< uint > al( 1, bil( 0 ) );
                bil.erase( 0 );
                bil.shrink_to_fit();

                // same as for bd
                Vector< uint > ad( aFirstTRIConnect.n_rows() + 1, 0.0 );
        ...
        void
        Intersection_Detect::generate_identifier( Vector< mtk::Cluster const * > &aSideClusters,
                uint                                                                  &aPairCount,
