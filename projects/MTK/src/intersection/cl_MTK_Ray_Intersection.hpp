...
namespace moris::mtk
{
    class Ray_Intersection
    {
      ...
      protected:
        ...
        {
            MORIS_ASSERT( aIntersectionFactors.size() == mTargetSpan.n_cols(),
                    "Intersection factors should have the same number of elements as the target span vectors" );
        ...
        static int get_target_span_dimension( Geometry_Type const &aGeometryType )
        {
            switch ( aGeometryType )
            {
                ...
                default:
                    MORIS_ERROR( false,
                            "Ray_Intersection::get_target_span_dimension: invalid geometry type" );
                    return 0;
            }
        }
