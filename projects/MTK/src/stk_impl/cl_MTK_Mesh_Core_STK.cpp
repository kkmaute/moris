...
namespace moris::mtk
{
            ...
            const bool   aCreateFacesAndEdges )
    {
        ...
        this->build_mesh( std::move( aFileName ), aSupMeshData, aCreateFacesAndEdges );
    }
    ...
    Mesh_Core_STK::Mesh_Core_STK(
            ...
            : mSTKMeshData( std::move( aSTKMeshData ) )
    {
    }
            ...
            const bool   aCreateFacesAndEdges )
    {
        ...
        mSTKMeshData->mMeshReader->add_mesh_database( std::move( aFileName ), stk::io::READ_MESH );
            ...
            const moris_index aIndex ) const
    {
        ...
        moris_id            tId          = this->get_glb_entity_id_from_entity_loc_index( aEntityIndex, aInputEntityRank );
    ...
    // ----------------------------------------------------------------------------
    Vector< moris::mtk::Vertex const * >
    Mesh_Core_STK::get_vertices_in_vertex_set_no_aura( const std::string& aSetName ) const
    {
        enum EntityRank tEntityRank = EntityRank::NODE;
            ...
            enum EntityRank            aFieldEntityRank ) const
    {
        ...
        stk::mesh::Field< real >* tField =
                mSTKMeshData->mMtkMeshMetaData->get_field< real >( tEntityRank, aFieldName );
        ...
        if ( tField == nullptr )
        {
            ...
            // throw error
            MORIS_ERROR( tField != nullptr, "Could not find %s field %s", +tRank.c_str(), aFieldName.c_str() );
        }
            ...
            Matrix< DDRMat > const & aFieldData )
    {
        ...
        // Get Field
        stk::mesh::EntityRank     tEntityRank = this->get_stk_entity_rank( aFieldEntityRank );
        stk::mesh::Field< real >* tField      = mSTKMeshData->mMtkMeshMetaData->get_field< real >( tEntityRank, aFieldName );
        for ( size_t i = 0; i < tNumEntities; i++ )
        {
            ...
            MtkMeshData& aMeshData )
    {
        ...
        if ( ( aMeshData.NodeProcOwner == nullptr ) && ( tProcSize == 1 ) )
        {
            // Do nothing. This information is not needed in serial
        }
        else if ( aMeshData.NodeProcOwner != nullptr )
        {
            // Verify sizes
            ...
            MtkMeshData& aMeshData )
    {
        ...
        {
            stk::mesh::Field< real >* tCoord_field =
                    &mSTKMeshData->mMtkMeshMetaData->declare_field< real >( stk::topology::NODE_RANK, "coordinates", aMeshData.SpatialDim[ 0 ] );

            stk::mesh::put_field_on_mesh(
                    *tCoord_field,
                    mSTKMeshData->mMtkMeshMetaData->universal_part(),
                    aMeshData.SpatialDim[ 0 ],
                    nullptr );
        }
        ...
        if ( aMeshData.FieldsInfo != nullptr )
        {
            ...
            for ( uint iF = 0; iF < tNumRealScalarFields; iF++ )
            {
                ...
                stk::mesh::Field< real >& tSTKRealScalarField =
                        mSTKMeshData->mMtkMeshMetaData->declare_field< real >( this->get_stk_entity_rank( tFieldEntityRank ), tFieldName, 1 );

                stk::mesh::put_field_on_mesh( tSTKRealScalarField, tFieldPart, 1, nullptr );
    ...
    Mesh_Core_STK::add_supplementary_fields_to_declare_at_output( MtkMeshData& aMeshData )
    {
        ...
        for ( uint iF = 0; iF < tNumRealScalarFields; iF++ )
        {
            ...
            stk::mesh::Field< real >* tField      = mSTKMeshData->mMtkMeshMetaData->get_field< real >( tEntityRank, tFieldName );
            ...
            uint                       aNumCols )
    {
        ...
        switch ( tNumFieldComp )
        {
            ...
            {
                // Declare fields
                mSTKMeshData->mField1CompVecsReal.push_back(
                        &mSTKMeshData->mMtkMeshMetaData->declare_field< real >( this->get_stk_entity_rank( aFieldRank ), aFieldName, tNumFieldComp, nullptr ) );
                stk::mesh::put_field_on_mesh( *mSTKMeshData->mField1CompVecsReal.back(), aFieldPart, tNumFieldComp, nullptr );
            ...
            {
                // Declare fields
                mSTKMeshData->mField2CompVecsReal.push_back(
                        &mSTKMeshData->mMtkMeshMetaData->declare_field< real >( this->get_stk_entity_rank( aFieldRank ), aFieldName, tNumFieldComp, nullptr ) );
                stk::mesh::put_field_on_mesh( *mSTKMeshData->mField2CompVecsReal.back(), aFieldPart, tNumFieldComp, nullptr );
            ...
            {
                // Declare fields
                mSTKMeshData->mField3CompVecsReal.push_back(
                        &mSTKMeshData->mMtkMeshMetaData->declare_field< real >( this->get_stk_entity_rank( aFieldRank ), aFieldName, tNumFieldComp, nullptr ) );
                stk::mesh::put_field_on_mesh( *mSTKMeshData->mField3CompVecsReal.back(), aFieldPart, tNumFieldComp, nullptr );
            ...
            {
                // Declare fields
                mSTKMeshData->mField4CompVecsReal.push_back(
                        &mSTKMeshData->mMtkMeshMetaData->declare_field< real >( this->get_stk_entity_rank( aFieldRank ), aFieldName, tNumFieldComp, nullptr ) );
                stk::mesh::put_field_on_mesh( *mSTKMeshData->mField4CompVecsReal.back(), aFieldPart, 2, 2, nullptr );
            ...
            {
                // Declare fields
                mSTKMeshData->mField9CompVecsReal.push_back(
                        &mSTKMeshData->mMtkMeshMetaData->declare_field< real >( this->get_stk_entity_rank( aFieldRank ), aFieldName, tNumFieldComp, nullptr ) );
                stk::mesh::put_field_on_mesh( *mSTKMeshData->mField9CompVecsReal.back(), aFieldPart, 3, 3, nullptr );
