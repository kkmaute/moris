...
//##############################################
    ...
    Mesh_Core_STK::get_elements_connected_to_element_and_face_ord_loc_inds(moris_index aElementIndex) const
    {
        ...
        for ( uint faceIt = 0; faceIt < tNumFaces; ++faceIt )
        {
            ...
            {
                if ( mSTKMeshData->mMtkMeshBulkData->identifier(tDummyConnectivity[0]) !=
            ...
            {
                if ( mSTKMeshData->mMtkMeshBulkData->identifier(tDummyConnectivity[1]) !=
                     mSTKMeshData->mMtkMeshBulkData->identifier(  tStkEntity ) )
                {
...
//##############################################
    ...
    Matrix< IdMat >
    Mesh_Core_STK::generate_unique_entity_ids( uint            aNumEntities,
...
//##############################################
    ...
    Matrix< IdMat >
    Mesh_Core_STK::get_processors_whom_share_entity_glob_ids(moris_id        aEntityId,
                                                        enum EntityRank aEntityRank) const
    {
        // Convert index to ID
...
//        }
    ...
    Matrix< IndexMat >
    Mesh_Core_STK::get_set_entity_loc_inds( enum EntityRank aSetEntityRank,
    ...
    void
    Mesh_Core_STK::add_mesh_field_real_scalar_data_loc_inds(std::string      const & aFieldName,
...
//##############################################
    ...
    void
    Mesh_Core_STK::add_element_cmap_to_exodus(std::string  & aFileName,
...
//##############################################
    ...
    Mesh_Core_STK::get_shared_info_by_entity( uint aNumActiveSharedProcs, enum EntityRank  aEntityRank )
    {
    ...
    uint
    Mesh_Core_STK::get_num_of_entities_shared_with_processor(moris_id        aProcessorRank,
                                                        enum EntityRank aEntityRank,
