...
namespace moris::mtk
{
    ...
    MappingResult QuadraturePointMapper_Ray::initialize_source_points( moris_index aSourceMeshIndex, Matrix< DDRMat > const &aParametricCoordinates ) const
    {
        ...
        {    // Kurt should be const & and no vertex normals
            ...
            std::map< moris_index, Matrix< DDRMat > > const &tIPElementDisplacementsMap = tSurfaceMesh.get_ip_element_displacement();
            auto                                             tIt                        = tIPElementDisplacementsMap.find( tIPElementIndex );

            if ( tIt != tIPElementDisplacementsMap.end() )
            {
                tIPElementDisplacements = tIt->second;
            }
            else
            {
