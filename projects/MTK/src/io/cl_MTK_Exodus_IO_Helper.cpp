...
namespace moris::mtk
{
    ...
    Exodus_IO_Helper::get_eb_info_global()
    {
        if ( par_size() > 1 && mBuildGlobal )
        {
            ...
            {
                std::cout << "[" << par_rank() << "] "
                          << "Global Element Block IDs and Counts:" << '\n';
                for ( std::size_t bn = 0; bn < mGlobalElemBlkIds.size(); ++bn )
                {
                    std::cout << "  [" << par_rank() << "] "
                              << "global_elem_blk_ids[" << bn << "]=" << mGlobalElemBlkIds[ bn ] << ", global_elem_blk_cnts[" << bn << "]=" << mGlobalElemBlkCnts[ bn ] << '\n';
                }
            }
        }
    }
    ...
    Exodus_IO_Helper::get_ns_param_global()
    {
        if ( par_size() > 1 && mBuildGlobal )
        {
            if ( mNumNodeSetsGlobal > 0 )
            {
                ...
                {
                    std::cout << "[" << par_rank() << "] "
                              << "Global Nodeset IDs, Node Counts, and DF counts:" << '\n';
                    for ( std::size_t bn = 0; bn < mGlobalNodesetIds.size(); ++bn )
                    {
                        std::cout << "  [" << par_rank() << "] "
                                  << "global_nodeset_ids[" << bn << "]=" << mGlobalNodesetIds[ bn ] << ", num_global_node_counts[" << bn << "]=" << mNumGlobalNodeCounts[ bn ] << ", num_global_node_df_counts[" << bn << "]=" << mNumGlobalNodeDfCounts[ bn ] << '\n';
                    }
                }
            }
        }
    ...
    {
        MORIS_ERROR( mTimeStepIndex < mNumTimeSteps, "Requested time step %d does not exist in file %s (contains %d time steps).", mTimeStepIndex, mTitle.c_str(), mNumTimeSteps );
    ...
    uint
    Exodus_IO_Helper::get_field_index_by_name( const std::string &aFileName )
    {
        if ( mNumNodalVars > 0 )
        {
