...
namespace moris
{
    namespace mtk
    {
        ...
        void
        Mesh::get_Mesh_GEN_map( moris::Vector< moris_index >& aMesh_GEN_map )
        {
        ...
        Set*
        Mesh::get_set_by_name( std::string aSetLabel ) const
        {
        ...
        uint
        Mesh::get_base_node_index( uint aNodeIndex )
        {
        ...
        Mesh::get_communication_vertex_pairing() const
        {
            ...
            return moris::Vector< Matrix< IdMat > >( 0 );
        }
                ...
                uint                    aDiscretizationIndex )
        {
            ...
            for ( uint Ik = 0; Ik < aNodeIndices.numel(); Ik++ )
            {
                ...
                {
                    tCounter += this->get_mtk_vertex( aNodeIndices( Ik ) ).get_interpolation( aDiscretizationIndex )->get_indices().numel();
                }
            }

            moris::Vector< uint > tIndices;
            ...
            for ( uint Ik = 0; Ik < aNodeIndices.numel(); Ik++ )
            {
                if ( this->get_mtk_vertex( aNodeIndices( Ik ) ).has_interpolation( aDiscretizationIndex ) )
                {
                    Matrix< IndexMat > tInterpolationIndices = this->get_mtk_vertex( aNodeIndices( Ik ) ).get_interpolation( aDiscretizationIndex )->get_indices();

                    Matrix< IdMat > tOwner = this->get_mtk_vertex( aNodeIndices( Ik ) ).get_interpolation( aDiscretizationIndex )->get_owners();
        ...
        Matrix< DDUMat >
        Mesh::get_owned_discretization_coefficient_indices(
                const Matrix< DDUMat >& aNodeIndices,
                uint                    aDiscretizationIndex )
        {
            uint tCounter = 0;
            ...
            for ( uint Ik = 0; Ik < aNodeIndices.numel(); Ik++ )
            {
                if ( this->get_mtk_vertex( aNodeIndices( Ik ) ).has_interpolation( aDiscretizationIndex ) )
                {
                    tCounter += this->get_mtk_vertex( aNodeIndices( Ik ) ).get_interpolation( aDiscretizationIndex )->get_ids().numel();
                }
            }
        ...
        Mesh::get_set_cells( std::string aSetLabel ) const
        {
            ...
            moris::Vector< Cell const * > tBlockSetCells;
        ...
        Mesh::get_block_set_cells( std::string aSetName ) const
        {
            ...
            moris::Vector< Cell const * > tBlockSetCells( tBlockSetElementInd.numel() );
        ...
        Mesh::get_sideset_cells_and_ords(
                const std::string&           aSetName,
                moris::Vector< Cell const * >& aCells,
                Matrix< IndexMat >&          aSidesetOrdinals ) const
        {
            ...
            aCells              = moris::Vector< Cell const * >( tNumCellsInSet );
        ...
        uint
        Mesh::get_sidesets_num_faces( moris::Vector< moris_index > aSideSetIndex ) const
        {
            uint tNumSideSetFaces = 0;

            moris::Vector< std::string > tSideSetsNames = this->get_set_names( this->get_facet_rank() );
        ...
        Mesh::get_vertices_in_vertex_set_no_aura( std::string aSetName ) const
        {
            ...
            return moris::Vector< Vertex const * >( 0 );
        }
        ...
        Mesh::get_mtk_cells(
                ...
                moris::Vector< Cell const * >& aCells )
        {
            aCells = moris::Vector< Cell const * >( aCellInds.numel() );
        ...
        uint
        Mesh::get_max_level( const moris_index aInterpolationIndex )
        {
        ...
        uint
        Mesh::get_num_basis( const moris_index aInterpolationIndex )
        {
        ...
        uint
        Mesh::get_num_coarse_basis_of_basis(
        ...
        uint
        Mesh::get_coarse_basis_index_of_basis(
                const moris_index aInterpolationIndex,
                const moris_index aBasisIndex,
        ...
        Matrix< DDSMat >
        Mesh::get_fine_basis_inds_of_basis(
                const moris_index aInterpolationIndex,
                const moris_index aBasisIndex )
        {
        ...
        Matrix< DDRMat >
        Mesh::get_fine_basis_weights_of_basis(
                const moris_index aInterpolationIndex,
                const moris_index aBasisIndex )
        {
        ...
        uint Mesh::get_order()
        {
            moris::Vector< std::string > tSetNames = this->get_set_names( EntityRank::ELEMENT );
        ...
        luint
        Mesh::get_num_active_bg_elements_on_discretization_mesh_index_including_aura( moris_index const aDiscretizationMeshIndex )
        {
        ...
        void
        Mesh::get_active_bg_element_indices_on_discretization_mesh_index_including_aura(
        ...
        Mesh::get_lagrange_elements_in_bspline_elements(
                ...
                moris::Vector< mtk::Cell* >&                 aBspCells )
        {
