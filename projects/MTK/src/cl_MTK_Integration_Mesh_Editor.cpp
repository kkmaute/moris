...
namespace moris::mtk
{
    ...
    Integration_Mesh_Editor::generate_cell_cluster_data()
    {
        ...
        for ( moris_index iCluster = 0; iCluster < (moris_index)tNumCellClusters; iCluster++ )
        {
            ...
            moris::Vector< moris::mtk::Cell const * > const & tPrimaryCells = tCluster.get_primary_cells_in_cluster();
            moris::Vector< moris::mtk::Cell const * > const & tVoidCells    = tCluster.get_void_cells_in_cluster();
            moris::Vector< moris::mtk::Vertex const * >       tVertex       = tCluster.get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::initialize_side_cluster_data()
    {
        ...
        moris::Vector< moris::mtk::Side_Set* > const & tSideSets = mInputMesh->get_side_sets();
        ...
        {
            moris::Vector< Cluster const * > tSideClusters = iSet->get_clusters_on_set();
    ...
    Integration_Mesh_Editor::generate_side_cluster_data()
    {
        ...
        for ( auto&& iSet : tSideSets.data() )
        {
            ...
            for ( const auto& iCluster : tSideClusters )
            {
                ...
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCells = iCluster->get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertex       = iCluster->get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::generate_double_sided_cluster_data()
    {
        ...
        moris::Vector< moris::mtk::Double_Side_Set* > const & tDoubleSideSets = mInputMesh->get_double_side_sets();
        ...
        for ( const auto& iSet : tDoubleSideSets.data() )
        {
            ...
            // get the clusters
            moris::Vector< Cluster const * > tClusters = iSet->get_clusters_on_set();
            ...
            for ( const auto& iCluster : tClusters )
            {
                ...
                moris::Vector< moris::mtk::Vertex const * > const & tVertexPairs = iCluster->get_leader_vertex_pairs();
    ...
    Integration_Mesh_Editor::initialize_double_sided_cluster_data()
    {
        ...
        moris::Vector< moris::mtk::Double_Side_Set* > const & tSideSets = mInputMesh->get_double_side_sets();
        ...
        for ( const auto& iSet : tDoubleSideSets.data() )
        {
            ...
            if ( std::strstr( iSet->get_set_name().c_str(), "ghost" ) )
            {
                ...
                for ( const auto& iCluster : tClusters )
                {
                    ...
                    moris::Vector< moris::mtk::Vertex const * > tGhostLeaderVertices   = tLeaderSideCluster.get_vertices_in_cluster();
                    moris::Vector< moris::mtk::Vertex const * > tGhostFollowerVertices = tFollowerSideCluster.get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::generate_block_set_data()
    {
        ...
        // get the block sets
        moris::Vector< moris::mtk::Block_Set* > const & tBlockSets = mInputMesh->get_block_sets();
        ...
        {
            // get the clusters
            moris::Vector< Cluster const * > tClustersOnBlock = iSet->get_clusters_on_set();
    ...
    Integration_Mesh_Editor::create_block_sets()
    {
        ...
        for ( const auto& iSet : tBlockSets.data() )
        {
            ...
            moris::Vector< Cluster const * > aBlockSetClusters;
    ...
    Integration_Mesh_Editor::create_side_sets()
    {
        ...
        for ( const auto& iSet : tSideSets.data() )
        {
            ...
            moris::Vector< Cluster const * > aSideSetClusters;
            ...
            moris::Vector< moris_index > tIndexOfSideClusters( mIGMeshInfo->mSideSetToSideClusterOffset( iCounter + 1 ) - mIGMeshInfo->mSideSetToSideClusterOffset( iCounter ) );
    ...
    Integration_Mesh_Editor::create_double_sided_clusters()
    {
        ...
        for ( size_t iDblCluster = 0; iDblCluster < tNumDblSideClusters; iDblCluster++ )
        {
            ...
            moris::Vector< mtk::Vertex const * > tVertexPair( mIGMeshInfo->mDoubleSidedClusterToVertexOffSet( iDblCluster + 1 ) - mIGMeshInfo->mDoubleSidedClusterToVertexOffSet( iDblCluster ) );
    ...
    Integration_Mesh_Editor::create_double_sided_sets()
    {
        ...
        for ( const auto& iSet : tDoubleSideSets.data() )
        {
            ...
            // create a cell with the indices of the side clusters within the set
            moris::Vector< moris_index > tIndexOfSideClusters( mIGMeshInfo->mDobleSideSetToDoubleSidedClusterOffset( iCounter + 1 ) - mIGMeshInfo->mDobleSideSetToDoubleSidedClusterOffset( iCounter ) );
    ...
    Integration_Mesh_Editor::create_ghost_clusters()
    {
        ...
        for ( uint iCluster = 0; iCluster < tNumGhostClusters; iCluster++ )
        {
            ...
            moris::Vector< mtk::Vertex const * > tVertexPair( mIGMeshInfo->mGhostToVertexOffset( iCluster + 1 ) - mIGMeshInfo->mGhostToVertexOffset( iCluster ) );
    ...
    Integration_Mesh_Editor::create_ghost_sets()
    {
        ...
        for ( const auto& iSet : tDoubleSideSets.data() )
        {
            ...
            // create a cell with the indices of the side clusters within the set
            moris::Vector< moris_index > tIndexOfSideClusters( mIGMeshInfo->mDoubleSidedGhostToSideClusterGhostOffset( iCounterLocal + 1 ) - mIGMeshInfo->mDoubleSidedGhostToSideClusterGhostOffset( iCounterLocal ) );
    ...
    void
    Integration_Mesh_Editor::add_vertices( moris::Vector< moris::Vector< moris_index > >& aSideClusterToVertexIndices, Matrix< DDRMat > aVerticesCoords )
    {
        ...
        uint tNumNewVertices2 = std::accumulate( aSideClusterToVertexIndices.data().begin(),
                ...
                tNumNewVertices,
                []( uint aContainter, moris::Vector< moris::moris_index > aVertexGroup ) -> uint { return aContainter + aVertexGroup.size(); } );
    ...
    void
    Integration_Mesh_Editor::add_cells( moris::Vector< moris::Vector< moris_index > >& aSideClusterToCells,
            moris::Vector< moris::Vector< moris_index > >&                             aCellToVertexIndices )
    {
        ...
        tNumNewCells      = std::accumulate( aSideClusterToCells.begin(),
                ...
                tNumNewCells,
                []( uint aContainter, moris::Vector< moris_index > aVertexGroup ) -> uint { return aContainter + aVertexGroup.size(); } );
    ...
    Integration_Mesh_Editor::construct_periodic_data_base(
            moris::Vector< moris::Vector< moris_index > >& aSideClusterToVertexIndices,
            Matrix< DDRMat >                           aVerticesCoords,
            moris::Vector< moris::Vector< moris_index > >& aSideClusterToCells,
            moris::Vector< moris::Vector< moris_index > >& aCellToVertexIndices,
            moris::Vector< moris_index >&                aSideClusterToIPCell,
            Matrix< DDRMat >&                          aVertexParametricCoords,
            moris::Vector< moris_index >&                aDoubleSidedClustersIndex,
    ...
    void
    Integration_Mesh_Editor::add_side_clusters( moris::Vector< moris::Vector< moris_index > >& aSideClusterToCells,
            moris::Vector< moris_index >&                                                    aSideClusterToIPCell,
            Matrix< DDRMat >&                                                              aVertexParametricCoords,
            moris::Vector< moris::Vector< moris_index > >&                                     aSideClusterToVertexIndices )
    {
        ...
        tNewSideClusterToIGCell      = std::accumulate( aSideClusterToCells.begin(),
                ...
                []( uint aContainter, moris::Vector< moris::moris_index > aCellGroup ) -> uint { return aContainter + aCellGroup.size(); } );
    ...
    void
    Integration_Mesh_Editor::add_double_sided_clusters( uint mNumDblSideCluster, moris::Vector< moris::Vector< moris_index > >& aSideClusterToVertexIndices )
    {
        ...
        {
            // initialize the vertex pairs of the double sided clusters with the right size
            moris::Vector< mtk::Vertex const * > tVertexPair( aSideClusterToVertexIndices( iDblSideCluster * 2 + 1 ).size() - 1 );
    ...
    void
    Integration_Mesh_Editor::add_double_sided_set( moris::Vector< moris_index >& aDoubleSidedClustersIndex, uint aNumGeometry )
    {
        // get number of geometries from the geometry engine
        moris::Vector< moris::Vector< moris_index > > tDoubleSideSetIndices( aNumGeometry * aNumGeometry );
        ...
        for ( uint iFirstPhase = 0; iFirstPhase < aNumGeometry; iFirstPhase++ )
        {
            for ( uint iSecondPhase = 0; iSecondPhase < aNumGeometry; iSecondPhase++ )
            {
                ...
                if ( tDoubleSideSetIndices( tPhaseToPhaseIndex ).size() > 0 )
                {
                    ...
                    // create the list of clusters inside the set
                    moris::Vector< Cluster const * > tDoubleSideSetClusters( tDoubleSideSetIndices( tPhaseToPhaseIndex ).size() );
    ...
    Integration_Mesh_Editor::merge_meshes()
    {
        ...
        // get the old double sided sets
        moris::Vector< moris::mtk::Double_Side_Set* > tDoubleSideSets = mOutputMesh->get_double_side_sets();
    ...
    {
        // call the old side sets
        moris::Vector< moris::mtk::Side_Set* > tSideSets = mOutputMesh->get_side_sets();
    ...
    Integration_Mesh_Editor::check_cell_clusters()
    {
        ...
        for ( size_t iCluster = 0; iCluster < tNumCells; iCluster++ )
        {
            ...
            moris::Vector< moris::mtk::Cell const * > const & tOldPrimaryCells = tCellClusterOldMesh.get_primary_cells_in_cluster();
            moris::Vector< moris::mtk::Cell const * > const & tNewPrimaryCells = tCellClusterNewMesh.get_primary_cells_in_cluster();

            // extract the void cells of the clusters
            moris::Vector< moris::mtk::Cell const * > const & tOldVoidCells = tCellClusterOldMesh.get_void_cells_in_cluster();
            moris::Vector< moris::mtk::Cell const * > const & tNewVoidCells = tCellClusterNewMesh.get_void_cells_in_cluster();

            // extract the vertices of the clusters
            moris::Vector< moris::mtk::Vertex const * > tOldVertices = tCellClusterOldMesh.get_vertices_in_cluster();
            moris::Vector< moris::mtk::Vertex const * > tNewVertices = tCellClusterNewMesh.get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::check_side_clusters()
    {
        ...
        for ( const auto& iSet : tSideSets.data() )
        {
            ...
            for ( const auto& iCluster : tSideClusters )
            {
                ...
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellsOld = iCluster->get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertexOld       = iCluster->get_vertices_in_cluster();
                ...
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellNew = mOutputMesh->mSideClusters( iCounter ).get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertexNew      = mOutputMesh->mSideClusters( iCounter ).get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::check_block_sets()
    {
        ...
        moris::Vector< moris::mtk::Block_Set* > const & tBlocks = mInputMesh->get_block_sets();

        // loop over the blocks
        for ( uint iSet = 0; iSet < mOutputMesh->mListOfBlocks.size(); iSet++ )
        {
            // clusters on the set from the mesh
            moris::Vector< Cluster const * > tClustersOldMesh = tBlocks( iSet )->get_clusters_on_set();
            moris::Vector< Cluster const * > tClustersNewMesh = mOutputMesh->mListOfBlocks( iSet )->get_clusters_on_set();
            ...
            if ( !std::strstr( tBlocks( iSet )->get_set_name().c_str(), "Vis" ) )
            {
                ...
                for ( uint iCluster = 0; iCluster < tClustersNewMesh.size(); iCluster++ )
                {
                    ...
                    moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellsOld  = tClustersOldMesh( iCluster )->get_primary_cells_in_cluster();
                    moris::Vector< moris::mtk::Vertex const * >       tVertexOld        = tClustersOldMesh( iCluster )->get_vertices_in_cluster();
                    ...
                    moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellNew   = tClustersNewMesh( iCluster )->get_primary_cells_in_cluster();
                    moris::Vector< moris::mtk::Vertex const * >       tVertexNew        = tClustersNewMesh( iCluster )->get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::check_side_sets()
    {
        ...
        for ( uint iSet = 0; iSet < mOutputMesh->mListOfSideSets.size(); iSet++ )
        {
            ...
            moris::Vector< Cluster const * > tSideClustersOldMesh = tSideSets( iSet )->get_clusters_on_set();
            moris::Vector< Cluster const * > tSideClustersNewMesh = mOutputMesh->mListOfSideSets( iSet )->get_clusters_on_set();

            for ( uint iCluster = 0; iCluster < tSideClustersNewMesh.size(); iCluster++ )
            {
                // moris::Matrix< moris::IndexMat >              tSideOrdinalOld  = iCluster->get_cell_side_ordinals();
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellsOld = tSideClustersOldMesh( iCluster )->get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertexOld       = tSideClustersOldMesh( iCluster )->get_vertices_in_cluster();
                ...
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellNew = tSideClustersNewMesh( iCluster )->get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertexNew      = tSideClustersNewMesh( iCluster )->get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::check_double_sided_clusters()
    {
        ...
        for ( auto&& iSet : tDoubleSideSets.data() )
        {
            ...
            for ( const auto& iCluster : tSideClusters )
            {
                ...
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellsOld = tLeaderSideCluster.get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertexOld       = tLeaderSideCluster.get_vertices_in_cluster();
                ...
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellNew = mOutputMesh->mDblSideClusters( iCounter ).get_leader_side_cluster().get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertexNew      = mOutputMesh->mDblSideClusters( iCounter ).get_leader_side_cluster().get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::check_ghost_clusters()
    {
        ...
        for ( const auto& iSet : tDoubleSideSets.data() )
        {
            ...
            for ( const auto& iCluster : tSideClusters )
            {
                ...
                moris::Vector< moris::mtk::Cell const * > const & tPrimaryCellNew = mOutputMesh->mGhostDblSidedSet( iCounter ).get_leader_side_cluster().get_primary_cells_in_cluster();
                moris::Vector< moris::mtk::Vertex const * >       tVertexNew      = mOutputMesh->mGhostDblSidedSet( iCounter ).get_leader_side_cluster().get_vertices_in_cluster();
    ...
    Integration_Mesh_Editor::check_double_sided_sets()
    {
        ...
        moris::Vector< moris::mtk::Double_Side_Set* > const & tSideSetsOld = mInputMesh->get_double_side_sets();
        moris::Vector< moris::mtk::Double_Side_Set* > const & tSideSetsNew = mOutputMesh->get_double_side_sets();
        ...
        for ( uint iSet = 0; iSet < tSideSetsOld.size(); iSet++ )
        {
            ...
            moris::Vector< Cluster const * > tSideClustersOldMesh = tSideSetsOld( iSet )->get_clusters_on_set();
            moris::Vector< Cluster const * > tSideClustersNewMesh = tSideSetsNew( iSet )->get_clusters_on_set();
