...
namespace moris
{
    ...
    FEMParameterList( moris::Cell< moris::Cell< ParameterList > >& tParameterList )
    {
        ...
        if ( tHaveStruct )
        {
            ...
            tParameterList( 2 )( tSPCounter ).set( "master_properties", "PropYoungsModulus,Material" );
            tSPCounter++;
        }
        ...
        if ( tHaveStruct )
        {
            ...
            tParameterList( 3 )( tIWGCounter ).set( "master_properties", std::string( "PropWeightCurrent,WeightCurrent;" ) + std::string( "PropWeightPrevious,WeightPrevious;" ) + std::string( "PropInitialConditionStruct,InitialCondition" ) );
            tParameterList( 3 )( tIWGCounter ).set( "mesh_set_names", tBulk );
    ...
    SOLParameterList( moris::Cell< moris::Cell< ParameterList > >& tParameterlist )
    {
        ...
        tParameterlist( 0 )( 0 ).set( "amesos: solver type", gPrecSolver );    // Amesos_Umfpack or Amesos_Pardiso

        // Ml Preconditioner parameters
        // tParameterlist( 0 )( 0 ).set( "ml_prec_type", "NSSA"); // options: SA, NSSA, DD, DD-ML

        // Preconditioner parameters
        tParameterlist( 0 )( 0 ).set( "overlap-level", 0 );
        tParameterlist( 0 )( 0 ).set( "schwarz: combine mode", "add" );    // for Amesos_Umfpack and Amesos_Pardiso provide this parameter with "add" mode
