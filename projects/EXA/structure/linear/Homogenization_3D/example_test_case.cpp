        ...
        uint                          aOuterNodeId )
{
    ...
    Matrix< DDRMat > tReferenceCoordinateInner = { { 0.4 }, { 0.6 }, { 0.4 } };
    Matrix< DDRMat > tReferenceCoordinateOuter = { { 0.2 }, { 0.8 }, { 1.0 } };
...
check_linear_results_serial()
{
    ...
    moris::mtk::Exodus_IO_Helper tExoIO( "Homogenization_3D.exo", 0, false, false );
    ...
    {
        REQUIRE( tNumDims == 3 );
        REQUIRE( tNumNodes == 976 );
        REQUIRE( tNumElems == 1731 );
    }

    // check results at node with most deformation
    uint tInnerNodeId = 33;
    uint tOuterNodeId = 584;
        ...
        "[moris],[example],[structure],[homogenization]" )
{
    ...
    char tString1[] = "";
    char tString2[] = "./Homogenization_3D.so";
