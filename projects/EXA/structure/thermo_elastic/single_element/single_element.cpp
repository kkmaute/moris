...
namespace moris
{
    ...
    // initial structure
    void Func_Initial_Condition_Struct(
            moris::Matrix< moris::DDRMat >                 & aPropMatrix,
            moris::Cell< moris::Matrix< moris::DDRMat > >  & aParameters,
            moris::fem::Field_Interpolator_Manager         * aFIManager )
    {
    ...
    void FEMParameterList( moris::Cell< moris::Cell< ParameterList > > & tParameterList )
    {
        ...
        if ( tHaveStruct )
        {
            ...
            if ( tHaveThermo )
            {
                ...
                tParameterList( 1 )( tCMCounter ).set( "properties",
                        std::string("PropYoungsModulus,    YoungsModulus;") +
                        std::string("PropPoissonRatio,     PoissonRatio;") +
                        std::string("PropThermalExpansion, CTE;") +
        ...
        if ( tHaveThermo ) {
            ...
            tParameterList( 2 )( tSPCounter ).set( "master_properties",
                    ...
                    std::string("PropPhaseStateFnct    , PhaseStateFunction;") +
                    std::string("PropPCconst_Dummy     , PhaseChangeConst")    );
            tSPCounter++;
        }
