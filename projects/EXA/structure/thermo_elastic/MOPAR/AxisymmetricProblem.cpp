...
namespace moris
{
    ...
    void Func_Thermal_Modulus_Block(
            moris::Matrix< moris::DDRMat >                 & aPropMatrix,
            moris::Cell< moris::Matrix< moris::DDRMat > >  & aParameters,
            moris::fem::Field_Interpolator_Manager         * aFIManager )
    {
        // get temperature
        Matrix< DDRMat > tTemperature = aFIManager->
   ...
   // Young's Modulus Interpolator
            ...
            moris::fem::Field_Interpolator_Manager         * aFIManager )
    {
        ...
        for( uint i = 0; i < tNumRows; i++ )
        {
            ...
            {
                aPropMatrix(0) = (tDataMatrix(i,14) - tDataMatrix(i,13)) * tDataMatrix(i,3) +
                        tDataMatrix(i,15)*tDataMatrix(i,4);
                aPropMatrix(1) = (tDataMatrix(i,17) - tDataMatrix(i,13)) * tDataMatrix(i,4) +
                        tDataMatrix(i,15)*tDataMatrix(i,3);
                break;
            }
            ...
            {
                tTractionX0 = (tDataMatrix(i-1,14) - tDataMatrix(i-1,13)) * tDataMatrix(i-1,3) +
                        tDataMatrix(i-1,15)*tDataMatrix(i-1,4);
                tTractionY0 = (tDataMatrix(i-1,17) - tDataMatrix(i-1,13)) * tDataMatrix(i-1,4) +
                        tDataMatrix(i-1,15)*tDataMatrix(i-1,3);
                tTractionX1 = (tDataMatrix(i,14) - tDataMatrix(i,13)) * tDataMatrix(i,3) +
                        tDataMatrix(i,15)*tDataMatrix(i,4);
                tTractionY1 = (tDataMatrix(i,17) - tDataMatrix(i,13)) * tDataMatrix(i,4) +
                        tDataMatrix(i,15)*tDataMatrix(i,3);

                // getting x and y traction
                aPropMatrix(0) = tTractionX0 + ( tTractionX1 - tTractionX0 ) *
                        ( tX(1) - tDataMatrix( i-1 ,2 ) ) / (tDataMatrix(i ,2) - tDataMatrix( i-1 ,2 ) );

                aPropMatrix(1) = tTractionY0 + ( tTractionY1 - tTractionY0 ) *
                        ( tX(1) - tDataMatrix( i-1 ,2 ) ) / (tDataMatrix(i ,2) - tDataMatrix( i-1 ,2 ) );
                break;
            }
        }
