...
for ib=1:numblk   
    ...
    maxnodesele = max(maxnodesele,size(fetopo_tmp,2));
    numfele     = numfele + size(fetopo_tmp,1);
end

fetopo = zeros(numfele,maxnodesele+3);
feprop = zeros(numfele,2);
numfele = 0;
...
for ib=1:numblk
    ...
    fetopo(numfele+1:numfele+numfele_tmp,2) = ib;
    fetopo(numfele+1:numfele+numfele_tmp,3) = size(fetopo_tmp,2);
    fetopo(numfele+1:numfele+numfele_tmp,4:size(fetopo_tmp,2)+3)=fetopo_tmp;
    feprop(numfele+1:numfele+numfele_tmp,:) = [zeros(numfele_tmp,1),ib*ones(numfele_tmp,1)];
