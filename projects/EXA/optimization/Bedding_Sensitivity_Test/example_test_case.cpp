...
//    }
    ...
    for (uint tFDIndex = 0; tFDIndex < tFDTypes.size(); tFDIndex++)
    {
        ...
        for (uint tADVIndex = 0; tADVIndex < tConstraintsAnalytical.n_cols(); tADVIndex++)
        {
            ...
            {
                // skip comparison if analytical and FD sensitivities are smaller than MORIS_REAL_EPS
                if (    std::abs(tConstraintsAnalytical(tConIndex,tADVIndex)) > MORIS_REAL_EPS ||
                        std::abs(tConstraintsFD(tConIndex,tADVIndex))         > MORIS_REAL_EPS )
                {
