...
namespace moris
{
    ...
    std::string tFluidKinViscosity   = moris_to_string(43.32e-6 * tLengthScale * tLengthScale / 0.3289 / tTimeScale );   // m2/s air at 800 C
    ...
    std::string tFluidTurbulencePrandtl = "0.9";
            ...
            const moris::Cell< moris::real* > & aGeometryParameters )
    {
        ...
        real tReturnValue = tRadius - std::pow(
                std::pow( tX - tSpherePosX, tSphereExpon ) +
                std::pow( tY - tSpherePosY, tSphereExpon ) , 1.0/tSphereExpon );

        return std::abs(tReturnValue) < tMinLevs ? tMinLevs : tReturnValue;
    }

    void Func_Sphere_Deriv(
    ...
    uint get_phase_index( const Bitset<5>& aGeometrySigns )
    {
        // by default in fluid
        uint tPhaseIndex = 0;
...
*/
    ...
    void GENParameterList( moris::Cell< moris::Cell< ParameterList > > & tParameterlist )
    {
        ...
        tParameterlist( 1 )( tGeoCounter ).set( "constant_parameters",
                "1,0," +  moris_to_string(tGeoShift) + ",0");
        tGeoCounter++;
        ...
        tParameterlist( 1 )( tGeoCounter ).set( "refinement_mesh_index", "0");
        tParameterlist( 1 )( tGeoCounter ).set( "constant_parameters",
                "-1,0," + moris_to_string(tChannelLength+tGeoShift) + ",0");
        tGeoCounter++;
        ...
        tParameterlist( 1 )( tGeoCounter ).set( "refinement_mesh_index", "0");
        tParameterlist( 1 )( tGeoCounter ).set( "constant_parameters",
                "0,1,0," + moris_to_string(tGeoShift));
        tGeoCounter++;
        ...
        tParameterlist( 1 )( tGeoCounter ).set( "refinement_mesh_index", "0");
        tParameterlist( 1 )( tGeoCounter ).set( "constant_parameters",
                "0,-1,0," + moris_to_string(tChannelHeight+tGeoShift));
        tGeoCounter++;
    }
    ...
    void FEMParameterList( moris::Cell< moris::Cell< ParameterList > > & tParameterList )
    {
        ...
        tParameterList( tCMIndex )( tCMCounter ).set( "constitutive_type", (uint) fem::Constitutive_Type::FLUID_TURBULENCE );
        tParameterList( tCMIndex )( tCMCounter ).set( "dof_dependencies",
                std::pair< std::string, std::string >( "VX,VY;P;VISCOSITY", "Velocity,Pressure,Viscosity" ) );
        ...
        tParameterList( tCMIndex )( tCMCounter ).set( "phase_name",        "PhaseFluid") ;
        tParameterList( tCMIndex )( tCMCounter ).set( "constitutive_type", (uint) fem::Constitutive_Type::SPALART_ALLMARAS_TURBULENCE );
        tParameterList( tCMIndex )( tCMCounter ).set( "dof_dependencies",
                std::pair< std::string, std::string >( "VX,VY;VISCOSITY", "Velocity,Viscosity" ) );
        tParameterList( tCMIndex )( tCMCounter ).set( "properties",
                "PropFluidKinViscosity,KinViscosity;"
        ...
        tParameterList( tCMIndex )( tCMCounter ).set( "phase_name",        "PhaseFluid") ;
        tParameterList( tCMIndex )( tCMCounter ).set( "constitutive_type",  (uint) fem::Constitutive_Type::DIFF_LIN_ISO_TURBULENCE );
        tParameterList( tCMIndex )( tCMCounter ).set( "dof_dependencies",
                std::pair< std::string, std::string >( "TEMP", "Temperature" ) );
        tParameterList( tCMIndex )( tCMCounter ).set( "properties",
        ...
        tParameterList( tSPIndex )( tSPCounter ).set( "master_dof_dependencies",
                std::pair< std::string, std::string >( "VX,VY;P", "Velocity,Pressure" ) );
        ...
        tParameterList( tSPIndex )( tSPCounter ).set( "master_dof_dependencies",
                std::pair< std::string, std::string >( "VX,VY", "Velocity" ) );
        tParameterList( tSPIndex )( tSPCounter ).set( "master_properties",
                "PropFluidConductivity,Conductivity" );
        tSPCounter++;
        ...
        tParameterList( tSPIndex )( tSPCounter ).set( "master_properties",   "PropFluidViscosity,Viscosity" );
        tSPCounter++;
    ...
    void SOLParameterList( moris::Cell< moris::Cell< ParameterList > > & tParameterlist )
    {
        ...
        */
        ...
        tParameterlist( 3 )( 0 ).set("NLA_Nonlinear_solver_algorithms", "1");            // set nonlinear algorithm with index 0
        ...
        tParameterlist( 3 )( 2 ) = moris::prm::create_nonlinear_solver_parameter_list(); // 0: fluid subproblem
        tParameterlist( 3 )( 2 ).set("NLA_Nonlinear_solver_algorithms", "0");            // set nonlinear algorithm with index 0
        tParameterlist( 3 )( 2 ).set("NLA_Solver_Implementation",       (uint) moris::NLA::NonlinearSolverType::NEWTON_SOLVER );
        tParameterlist( 3 )( 2 ).set("NLA_DofTypes",                    "VX,VY,P,VISCOSITY");

        tParameterlist( 3 )( 3 ) = moris::prm::create_nonlinear_solver_parameter_list(); // 1: thermal subproblem
        tParameterlist( 3 )( 3 ).set("NLA_Nonlinear_solver_algorithms", "1");            // set nonlinear algorithm with index 0
        tParameterlist( 3 )( 3 ).set("NLA_Solver_Implementation",       (uint) moris::NLA::NonlinearSolverType::NEWTON_SOLVER );
        tParameterlist( 3 )( 3 ).set("NLA_DofTypes",                    "TEMP");

        tParameterlist( 3 )( 4 ) = moris::prm::create_nonlinear_solver_parameter_list(); // 2: one-way coupling via NLBGS
        tParameterlist( 3 )( 4 ).set("NLA_Nonlinear_solver_algorithms", "2");            // set nonlinear algorithm with index 1.
        tParameterlist( 3 )( 4 ).set("NLA_Solver_Implementation",       (uint) moris::NLA::NonlinearSolverType::NLBGS_SOLVER );
        ...
        tParameterlist( 3 )( 5 ) = moris::prm::create_nonlinear_solver_parameter_list(); // 0: fluid subproblem
        tParameterlist( 3 )( 5 ).set("NLA_Nonlinear_solver_algorithms", "1");            // set nonlinear algorithm with index 0
        tParameterlist( 3 )( 5 ).set("NLA_Solver_Implementation",       (uint) moris::NLA::NonlinearSolverType::NEWTON_SOLVER );
        ...
        tParameterlist( 3 )( 6 ).set("NLA_Solver_Implementation",       (uint) moris::NLA::NonlinearSolverType::NLBGS_SOLVER );
        tParameterlist( 3 )( 6 ).set("NLA_Sub_Nonlinear_Solver",        "0,1,5,3");          // set sub nonlinear solvers with index 0 and 1
    ...
    void VISParameterList( moris::Cell< moris::Cell< ParameterList > > & tParameterlist )
    {
        ...
        tParameterlist( 0 )( 0 ).set( "IQI_Names"  , "IQIBulkTHETA,IQIBulkPHID,IQIBulkVX,IQIBulkVY,IQIBulkP,IQIBulkVISCOSITY,IQIBulkTEMP,"
