        ...
        "[moris],[example],[optimization],[sweep]" )
{
    ...
    if ( par_size() == 1 )
    {
        ...
        REQUIRE( tObjectiveAnalytical.numel() == tConstraintsAnalytical.numel() );    // one objective and one constraint for this problem only

        print( tObjectiveAnalytical, "tObjectiveAnalytical" );
        print( tConstraintsAnalytical, "tConstraintsAnalytical" );

        // Read FD sensitivities and compare
        Vector< std::string > tFDTypes = { "fd_forward", "fd_backward", "fd_central" };
        print( tFDTypes, "tFDTypes" );
        ...
        for ( uint tFDIndex = 0; tFDIndex < tFDTypes.size(); tFDIndex++ )
        {
            ...
            print( tObjectiveFD, "tObjectiveFD" );
            print( tConstraintsFD, "tConstraintsFD" );
